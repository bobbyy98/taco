...
namespace taco {
  ...
  public:
    ...
    CompressedModeFormat(bool isFull, bool isOrdered, bool isUnique, bool isZeroless, long long allocSize = DEFAULT_ALLOC_SIZE);
    ...
    std::vector<AttrQuery> attrQueries(std::vector<IndexVar> parentCoords, std::vector<IndexVar> childCoords) const override;
    ...
    ir::Stmt     getSeqInitEdges(ir::Expr prevSize, std::vector<AttrQueryResult> queries, Mode mode) const override;
    ir::Stmt     getSeqInsertEdge(ir::Expr parentPos, std::vector<ir::Expr> coords, std::vector<AttrQueryResult> queries, Mode mode) const override;
    ir::Stmt     getInitCoords(ir::Expr prevSize, std::vector<AttrQueryResult> queries, Mode mode) const override;
    ir::Stmt     getInitYieldPos(ir::Expr prevSize, Mode mode) const override;
    ModeFunction getYieldPos(ir::Expr parentPos, std::vector<ir::Expr> coords, Mode mode) const override;
    ir::Stmt     getInsertCoord(ir::Expr parentPos, ir::Expr pos, std::vector<ir::Expr> coords, Mode mode) const override;
