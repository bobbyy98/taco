...
namespace taco {
  ...
  public:
    ...
    ir::Stmt lower(IndexStmt stmt, std::string name, bool assemble, bool compute, bool pack, bool unpack);
  ...
  protected:
    ...
    virtual ir::Stmt lowerForallDimension(Forall forall, std::vector<Iterator> locaters, std::vector<Iterator> inserters, std::vector<Iterator> appenders, MergeLattice caseLattice, std::set<Access> reducedAccesses, ir::Stmt recoveryStmt);

    /// Lower a forall that iterates over all the coordinates in the forall index
    /// var's dimension, and locates tensor positions from the locate iterators.
    virtual ir::Stmt lowerForallDenseAcceleration(Forall forall, std::vector<Iterator> locaters, std::vector<Iterator> inserters, std::vector<Iterator> appenders, MergeLattice caseLattice, std::set<Access> reducedAccesses, ir::Stmt recoveryStmt);

    /// Lower a forall that iterates over the coordinates in the iterator, and
    /// locates tensor positions from the locate iterators.
    virtual ir::Stmt lowerForallCoordinate(Forall forall, Iterator iterator, std::vector<Iterator> locaters, std::vector<Iterator> inserters, std::vector<Iterator> appenders, MergeLattice caseLattice, std::set<Access> reducedAccesses, ir::Stmt recoveryStmt);
    ...
    /// iterators.
    virtual ir::Stmt lowerForallPosition(Forall forall, Iterator iterator, std::vector<Iterator> locaters, std::vector<Iterator> inserters, std::vector<Iterator> appenders, MergeLattice caseLattice, std::set<Access> reducedAccesses, ir::Stmt recoveryStmt);

    virtual ir::Stmt lowerForallFusedPosition(Forall forall, Iterator iterator, std::vector<Iterator> locaters, std::vector<Iterator> inserters, std::vector<Iterator> appenders, MergeLattice caseLattice, std::set<Access> reducedAccesses, ir::Stmt recoveryStmt);
     ...
     */
    virtual ir::Stmt lowerMergeLattice(MergeLattice lattice, IndexVar coordinateVar, IndexStmt statement, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy);
     ...
     */
    virtual ir::Stmt lowerMergePoint(MergeLattice pointLattice, ir::Expr coordinate, IndexVar coordinateVar, IndexStmt statement, const std::set<Access> &reducedAccesses, bool resolvedCoordDeclared, MergeStrategy mergestrategy);

    /// Lower a merge lattice to cases.
    virtual ir::Stmt lowerMergeCases(ir::Expr coordinate, IndexVar coordinateVar, IndexStmt stmt, MergeLattice lattice, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy);

    /// Lower a forall loop body.
    virtual ir::Stmt lowerForallBody(ir::Expr coordinate, IndexStmt stmt, std::vector<Iterator> locaters, std::vector<Iterator> inserters, std::vector<Iterator> appenders, MergeLattice caseLattice, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy);
    ...
    ir::Stmt initResultArrays(std::vector<Access> writes, std::set<Access> reducedAccesses);
    ...
    ir::Stmt initResultArrays(IndexVar var, std::vector<Access> writes, std::set<Access> reducedAccesses);

    ir::Stmt resizeAndInitValues(const std::vector<Iterator> &appenders, const std::set<Access> &reducedAccesses);
    /**
    ...
    ir::Stmt reduceDuplicateCoordinates(ir::Expr coordinate, std::vector<Iterator> iterators, bool alwaysReduce);
    ...
    ir::Stmt codeToIncIteratorVars(ir::Expr coordinate, IndexVar coordinateVar, std::vector<Iterator> iterators, std::vector<Iterator> mergers, MergeStrategy strategy);
    ...
    std::pair<std::vector<Iterator>, std::vector<Iterator>> splitAppenderAndInserters(const std::vector<Iterator> &results);
    ...
    ir::Stmt lowerMergeCasesWithExplicitZeroChecks(ir::Expr coordinate, IndexVar coordinateVar, IndexStmt stmt, MergeLattice lattice, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy);
    ...
    template <typename C>
    std::vector<ir::Expr> compareToResolvedCoordinate(const std::vector<Iterator> &iters, ir::Expr resolvedCoordinate, IndexVar coordinateVar) {
    ...
    std::vector<ir::Stmt> constructInnerLoopCasePreamble(ir::Expr coordinate, IndexVar coordinateVar, MergeLattice lattice, std::map<Iterator, ir::Expr> &iteratorToConditionMap);
    ...
    std::vector<ir::Stmt> lowerCasesFromMap(std::map<Iterator, ir::Expr> iteratorToCondition, ir::Expr coordinate, IndexStmt stmt, const MergeLattice &lattice, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy);
