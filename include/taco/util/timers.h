...
namespace taco { namespace util {
  ...
  struct TimeResults {
    ...
    friend std::ostream &operator<<(std::ostream &os, const TimeResults &t) {
        if (t.size == 1) {
          ...
          return os << "  mean:   " << t.mean << endl << "  stdev:  " << t.stdev << endl << "  median: " << t.median;
        }
    }
  };
  ...
  public:
    ...
    TimeResults getResult() {
      ...
      transform(times.begin() + truncate, times.end() - truncate, diff.begin(), [mean](double x) {
        return x - mean;
      });
  ...
  public:
    ...
    void stop() {
      auto end = std::chrono::steady_clock::now();
      taco_iassert(isTiming) << "Called PrintTimer::stop without first calling start";
        if (timerGroup) {
          std::cout << "  ";
      }
      auto diff = std::chrono::duration<double, std::milli>(end - begin).count();
