...
namespace taco { namespace util {
  ...
  inline std::string getTmpdir() {
      if (cachedtmpdir == "") {
        ...
        taco_uassert(tmpdir.front() == '/') << "The TMPDIR environment variable must be an absolute path";

        taco_uassert(access(tmpdir.c_str(), W_OK) == 0) << "Unable to write to temporary directory for code generation. "
        ...
        taco_uassert(ctacotmpdir != NULL) << "Unable to create taco temporary directory for code generation. Please set"
