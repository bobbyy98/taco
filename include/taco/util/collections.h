...
namespace taco { namespace util {
  ...
  template <typename V>
  std::vector<V> combine(const std::vector<V> &a, const std::vector<V> &b, const std::vector<V> &c) {
    std::vector<V> result;
    append(result, a);
    append(result, b);
    append(result, c);
  ...
  template <typename V>
  std::vector<V> remove(const std::vector<V> &vector, const std::vector<V> &filter) {
    std::vector<V> result;
   ...
   */
  template <typename V, typename T>
  std::pair<std::vector<V>, std::vector<V>> split(const std::vector<V> &vector, T test) {
    std::vector<V> first;
    std::vector<V> second;
      for (auto &element: vector) {
          if (test(element)) {
  ...
  size_t locate(const Collection &collection, const Value &value) {
    ...
    return std::distance(collection.begin(), std::find(collection.begin(), collection.end(), value));
  }
  ...
  std::vector<ResultValue> map(const Collection &collection, std::function<ResultValue(ParamValue)> f) {
  ...
  public:
    ...
    iterator begin() {
      return (c.begin() == c.end() ? c.end() : ++c.begin());
    }
    iterator end() {
      return c.end();
    }
  ...
  public:
    ...
    friend std::ostream &           operator<<(std::ostream &os, const std::pair<value_type1, value_type2> &p) {
    ...
    public:
      ...
      ZipConstIterator(const_iterator1 c1it, const_iterator2 c2it): c1it(c1it), c2it(c2it) {
      }
      ZipConstIterator(const ZipConstIterator &zit): c1it(zit.c1it), c2it(zit.c2it) {
      }
      ...
      friend bool operator==(const ZipConstIterator &l, const ZipConstIterator &r) {
