...
namespace taco { namespace util {
  ...
  enum class FillMethod { Dense, Uniform, Random, Sparse, SlicingH, SlicingV, FEM, HyperSparse, Blocked };

  // Some parameters
  const std::map<FillMethod, double> fillFactors = {
      {FillMethod::Dense, 1.0},         {FillMethod::Uniform, 1.0},   {FillMethod::Random, 1.0},    {FillMethod::Sparse, 0.07},
      {FillMethod::HyperSparse, 0.005}, {FillMethod::SlicingH, 0.01}, {FillMethod::SlicingV, 0.01}, {FillMethod::FEM, 0.0}};
  ...
  void fillTensor(TensorBase &tens, const FillMethod &fill, double fillValue /*=-1.0*/) {
    ...
    if (fillValue == -1)
      ...
      switch (tens.getOrder()) {
        ...
        default:
          taco_uerror << "Impossible to fill tensor " << tens.getName() << " of order " << tens.getOrder() << std::endl;
      }
  }

  void fillVector(TensorBase &tensor, const FillMethod &fill, double fillValue) {
    // Random values
    std::uniform_real_distribution<double> unif(doubleLowerBound, doubleUpperBound);
      ...
      switch (fill) {
        ...
        case FillMethod::Sparse:
          ...
          case FillMethod::Blocked: {
            ...
            Tensor<double> BaseTensor(tens.getName(), dimensions, tens.getFormat());
    ...
    std::vector<std::vector<int>> positions(tens.getOrder());
      ...
      switch (fill) {
        ...
        case FillMethod::Sparse:
          ...
          default: {
            taco_uerror << "FillMethod not available for tensors of order 3" << std::endl;
            break;
          }
      }
  }
