...
namespace taco { namespace ir {
  ...
  enum class TensorProperty { Order, Dimension, ComponentSize, ModeOrdering, ModeTypes, Indices, Values, FillValue, ValuesSize };
  ...
  struct Var: public ExprNode<Var> {
    ...
    static Expr make(std::string name, Datatype type, bool is_ptr = false, bool is_tensor = false, bool is_parameter = false);
  ...
  struct Call: public ExprNode<Call> {
    ...
    static Expr make(const std::string &func, const std::vector<Expr> &args, Datatype type);
  ...
  struct For: public StmtNode<For> {
    ...
    static Stmt make(Expr var, Expr start, Expr end, Expr increment, Stmt contents, LoopKind kind = LoopKind::Serial, ParallelUnit parallel_unit = ParallelUnit::NotParallel, size_t unrollFactor = 0, int vec_width = 0);
  ...
  struct While: public StmtNode<While> {
    ...
    static Stmt make(Expr cond, Stmt contents, LoopKind kind = LoopKind::Serial, int vec_width = 0);
  ...
  struct Function: public StmtNode<Function> {
    ...
    static Stmt make(std::string name, std::vector<Expr> outputs, std::vector<Expr> inputs, Stmt body);
  ...
  struct Allocate: public StmtNode<Allocate> {
    ...
    static Stmt make(Expr var, Expr num_elements, bool is_realloc = false, Expr old_elements = Expr(), bool clear = false);
  ...
  struct GetProperty: public ExprNode<GetProperty> {
    ...
    static Expr make(Expr tensor, TensorProperty property, int mode, int index, std::string name);
  ...
  inline const S *to(Stmt s) {
    taco_iassert(isa<S>(s)) << "Cannot convert " << typeid(s).name() << " to " << typeid(S).name();
