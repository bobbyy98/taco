...
namespace taco {
  ...
  public:
    ...
    Format(const std::vector<ModeFormatPack> &modeFormatPacks, const std::vector<int> &modeOrdering);
  ...
  public:
    ...
    std::vector<AttrQuery> getAttrQueries(std::vector<IndexVar> parentCoords, std::vector<IndexVar> childCoords) const;
  ...
  const Format COO(int order, bool isUnique = true, bool isOrdered = true, bool isAoS = false, const std::vector<int> &modeOrdering = {});
  /// @}
