...
namespace taco {
  ...
  TensorBase readMTX(std::string filename, const ModeFormat &modetype, bool pack = true);

  /// Read an mtx matrix from a file.
  TensorBase readMTX(std::string filename, const Format &format, bool pack = true);

  /// Read an mtx matrix from a stream.
  TensorBase readMTX(std::istream &stream, const ModeFormat &modetype, bool pack = true);

  /// Read an mtx matrix from a stream.
  TensorBase readMTX(std::istream &stream, const Format &format, bool pack = true);

  TensorBase readSparse(std::istream &stream, const ModeFormat &modetype, bool symm = false);
  TensorBase readDense(std::istream &stream, const ModeFormat &modetype, bool symm = false);

  TensorBase readSparse(std::istream &stream, const Format &format, bool symm = false);
  TensorBase readDense(std::istream &stream, const Format &format, bool symm = false);
