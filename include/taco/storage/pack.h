...
namespace taco {
  ...
  TensorStorage pack(Datatype datatype, const std::vector<int> &dimensions, const Format &format, const std::vector<TypedIndexVector> &coordinates, const void *values, const Literal &fill);

  template <typename V, size_t O, typename C>
  TensorStorage pack(std::vector<int> dimensions, Format format, const std::vector<std::pair<Coordinates<O, C>, V>> &components, const Literal &fill) {
    size_t order = dimensions.size();
    size_t nnz   = components.size();

    std::vector<TypedIndexVector> coordinates(order, TypedIndexVector(type<C>(), nnz));
