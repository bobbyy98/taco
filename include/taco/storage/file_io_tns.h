...
namespace taco {
  ...
  TensorBase readTNS(std::string filename, const ModeFormat &modetype, bool pack = true);

  /// Read a tns tensor from a file.
  TensorBase readTNS(std::string filename, const Format &format, bool pack = true);

  /// Read a tns tensor from a stream.
  TensorBase readTNS(std::istream &stream, const ModeFormat &modetype, bool pack = true);
