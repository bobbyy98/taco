...
namespace taco {
  ...
  public:
    ...
    /// in every mode.
    TensorBase(Datatype ctype, std::vector<int> dimensions, ModeFormat modeType = ModeFormat::compressed, Literal fill = Literal());
    ...
    TensorBase(std::string name, Datatype ctype, std::vector<int> dimensions, ModeFormat modeType = ModeFormat::compressed, Literal fill = Literal());
    ...
    TensorBase(std::string name, Datatype ctype, std::vector<int> dimensions, Format format, Literal fill = Literal());
    ...
    public:
      ...
      bool operator==(const const_iterator &rhs) {
        ...
        return ((bufferSize == 0) == (rhs.bufferSize == 0)) && ((bufferSize == 0) || (valsIterated() == rhs.valsIterated())) && (tensor == rhs.tensor);
      }
    ...
    private:
      ...
      const_iterator(const TensorBase *tensor, bool isEnd = false):
          ...
          if (!isEnd) {
            const auto helperFuncs                = tensor->getHelperFunctions(tensor->getFormat(), tensor->getComponentType(), tensor->getDimensions());
            *reinterpret_cast<void **>(&iterFunc) = helperFuncs->getFuncPtr("_shim_iterate");
            ++(*this);
        }
      }

      static std::shared_ptr<Context> makeContext(int tensorOrder, int bufferCapacity) {
      ...
      void fillBuffer() {
        std::array<void *, 5> args = {&ctx->iterCtx, ctx->coordBuffer, (void *) valBuffer, (void *) &bufferCapacity, (void *) tensorStorage};
    ...
    private:
      ...
      iterator_wrapper(const TensorBase *tensor, bool iterateAll = true): tensor(tensor) {
          if (iterateAll) {
    ...
    void printComputeIR(std::ostream &stream, bool color = false, bool simplify = false) const;

    /// Print the IR loops that assemble the tensor's expression.
    void printAssembleIR(std::ostream &stream, bool color = false, bool simplify = false) const;
  ...
  private:
    static std::shared_ptr<ir::Module> getHelperFunctions(const Format &format, Datatype ctype, const std::vector<int> &dimensions);
    static std::shared_ptr<ir::Module> getComputeKernel(const IndexStmt stmt);
    static void                        cacheComputeKernel(const IndexStmt stmt, const std::shared_ptr<ir::Module> kernel);
  ...
  protected:
    template <typename T, typename CType>
    void insertUnchecked(const typename const_iterator<T, CType>::Coordinates &coordinate, CType value);
  ...
  private:
    ...
    typedef std::vector<std::tuple<Format, Datatype, std::vector<int>, std::shared_ptr<ir::Module>>> HelperFuncsCache;
    static HelperFuncsCache                                                                          helperFunctions;
    static std::mutex                                                                                helperFunctionsMutex;

    typedef std::vector<std::pair<IndexStmt, std::shared_ptr<ir::Module>>> KernelsCache;
  ...
  public:
    ...
    /// Create a tensor with the given dimensions. The format defaults to sparse
    /// in every mode.
    Tensor(std::vector<int> dimensions, ModeFormat modeType = ModeFormat::compressed, CType fill = CType());
    ...
    /// defaults to sparse in every mode.
    Tensor(std::string name, std::vector<int> dimensions, ModeFormat modeType = ModeFormat::compressed, CType fill = CType());
    ...
    Tensor(std::string name, std::vector<int> dimensions, Format format, CType fill = CType());
  ...
  template <typename CType>
  struct ScalarAccess {
    ScalarAccess(TensorBase *tensor, const std::vector<int> &indices): tensor(tensor), indices(indices) {
    }
  ...
  TensorBase read(std::string filename, FileType filetype, ModeFormat modetype, bool pack = true);

  /// Read a tensor from a file of the given file format and the tensor is
  /// returned packed by default.
  TensorBase read(std::string filename, FileType filetype, Format format, bool pack = true);

  /// Read a tensor from a stream of the given file format. The tensor is returned
  /// packed by default.
  TensorBase read(std::istream &stream, FileType filetype, ModeFormat modetype, bool pack = true);

  /// Read a tensor from a stream of the given file format. The tensor is returned
  /// packed by default.
  TensorBase read(std::istream &stream, FileType filetype, Format format, bool pack = true);
  ...
  template <typename CType>
  TensorBase makeCSR(const std::string &name, const std::vector<int> &dimensions, int *rowptr, int *colidx, CType *vals, Array::Policy policy = Array::UserOwns) {
  ...
  template <typename T>
  TensorBase makeCSR(const std::string &name, const std::vector<int> &dimensions, const std::vector<int> &rowptr, const std::vector<int> &colidx, const std::vector<T> &vals) {
    taco_uassert(dimensions.size() == 2) << error::requires_matrix;
  ...
  template <typename T>
  void getCSRArrays(const TensorBase &tensor, int **rowptr, int **colidx, T **vals) {
    taco_uassert(tensor.getFormat() == CSR) << "The tensor " << tensor.getName() << " is not defined in the CSR format";
    auto storage = tensor.getStorage();
  ...
  /// arrays remain owned by the user and will not be freed by taco.
  template <typename T>
  TensorBase makeCSC(const std::string &name, const std::vector<int> &dimensions, int *colptr, int *rowidx, T *vals, Array::Policy policy = Array::UserOwns) {
    taco_uassert(dimensions.size() == 2) << error::requires_matrix;
  ...
  template <typename T>
  TensorBase makeCSC(const std::string &name, const std::vector<int> &dimensions, const std::vector<int> &colptr, const std::vector<int> &rowidx, const std::vector<T> &vals) {
    taco_uassert(dimensions.size() == 2) << error::requires_matrix;
    Tensor<T> tensor(name, dimensions, CSC);
    auto      storage = tensor.getStorage();
  ...
  /// function does not change the ownership of the arrays.
  template <typename T>
  void getCSCArrays(const TensorBase &tensor, int **colptr, int **rowidx, T **vals) {
    taco_uassert(tensor.getFormat() == CSC) << "The tensor " << tensor.getName() << " is not defined in the CSC format";
    auto storage = tensor.getStorage();
    auto index   = storage.getIndex();
  ...
  struct TensorBase::Content {
    ...
    Content(std::string name, Datatype dataType, const std::vector<int> &dimensions, Format format, Literal fill):
        dataType(dataType),
  ...
  void TensorBase::insert(const std::initializer_list<int> &coordinate, CType value) {
    taco_uassert(coordinate.size() == (size_t) getOrder()) << "Wrong number of indices";
    taco_uassert(getComponentType() == type<CType>()) << "Cannot insert a value of type '" << type<CType>() << "' "
  ...
  template <typename T, typename CType>
  void TensorBase::insertUnchecked(const typename TensorBase::const_iterator<T, CType>::Coordinates &coordinate, CType value) {
      if ((content->coordinateBuffer->size() - content->coordinateBufferUsed) < content->coordinateSize) {
        content->coordinateBuffer->resize(content->coordinateBuffer->size() + content->coordinateSize);
    }
    int *coordLoc = (int *) &content->coordinateBuffer->data()[content->coordinateBufferUsed];
  ...
  CType TensorBase::at(const std::vector<int> &coordinate) {
    taco_uassert(coordinate.size() == (size_t) getOrder()) << "Wrong number of indices";
    taco_uassert(getComponentType() == type<CType>()) << "Cannot get a value of type '" << type<CType>() << "' "
  ...
  template <typename CType>
  Tensor<CType>::Tensor(std::vector<int> dimensions, ModeFormat modeType, CType fill): TensorBase(type<CType>(), dimensions, modeType, fill) {
  }

  template <typename CType>
  Tensor<CType>::Tensor(std::vector<int> dimensions, Format format, CType fill): TensorBase(type<CType>(), dimensions, format, fill) {
  }

  template <typename CType>
  Tensor<CType>::Tensor(std::string name, std::vector<int> dimensions, ModeFormat modeType, CType fill):
      TensorBase(name, type<CType>(), dimensions, modeType, fill) {
  }

  template <typename CType>
  Tensor<CType>::Tensor(std::vector<int> dimensions, CType fill): TensorBase(type<CType>(), dimensions, fill) {
  }

  template <typename CType>
  Tensor<CType>::Tensor(std::string name, std::vector<int> dimensions, CType fill): TensorBase(name, type<CType>(), dimensions, fill) {
  }

  template <typename CType>
  Tensor<CType>::Tensor(std::string name, std::vector<int> dimensions, Format format, CType fill): TensorBase(name, type<CType>(), dimensions, format, fill) {
  }

  template <typename CType>
  Tensor<CType>::Tensor(const TensorBase &tensor): TensorBase(tensor) {
    taco_uassert(tensor.getComponentType() == type<CType>()) << "Assigning TensorBase with " << tensor.getComponentType() << " components to a Tensor<" << type<CType>() << ">";
  }
  ...
  ScalarAccess<CType> Tensor<CType>::operator()(const std::vector<int> &indices) {
    taco_uassert(indices.size() == (size_t) getOrder()) << "A tensor of order " << getOrder() << " must be indexed with " << getOrder() << " variables, but is indexed with:  " << util::join(indices);
