...
/// avoid templates, or you can use the templated `Tensor<T>` that inherits
/// from `TensorBase`.
class TensorBase {
 public:
  /* --- Constructor Methods --- */
  ...
  /// Get a vector with the strides of the tensor.
  const std::vector<int> getStrides() const;

  /// Get the product of the dimensions of the tensor.
  int getSize() const;
  ...
  /// duplicated elements. The result is a Tensor where the duplicates have been
  /// summed up. The InputIterators value_type must provide the following
  /// interface:
  ///
  ...
  void setFromComponents(const InputIterators &begin,
  ...
  class const_iterator {
   public:
    class Coordinates {
     public:
      const T &operator[](size_t idx) const { return coordinates[idx]; }
     ...
     private:
      ...
      Coordinates(size_t order) : coordinates(nullptr), order(order) {}
    ...
    const_iterator &operator++() {
      ...
      // If iterator has iterated over every element in the buffer, refill
      // the buffer with additional tensor elements.
    ...
    const value_type &operator*() const { return curVal; }
   ...
   private:
    ...
    const_iterator(const TensorBase *tensor, bool isEnd = false)
      ...
      if (!isEnd) {
        const auto helperFuncs = tensor->getHelperFunctions(
            tensor->getFormat(), tensor->getComponentType(),
  ...
  /// Create a possibly windowed index expression that accesses (reads or
  /// writes) this tensor.
  Access operator()(
  ...
  /// Create an index expression that accesses (reads or writes) this (scalar)
  /// tensor.
  ...
  /// Compile the source code of the kernel functions. This function is
  /// optional and mainly intended for experimentation. If the source code is
  /// not set then it will will be created it from the given expression.
  ...
  /// address of b.  This is arbitrary and non-deterministic, but necessary
  /// for tensor to be placed in maps.
  friend bool operator<(const TensorBase &a, const TensorBase &b);
  friend bool operator>(const TensorBase &a, const TensorBase &b);
  friend bool operator<=(const TensorBase &a, const TensorBase &b);
  friend bool operator>=(const TensorBase &a, const TensorBase &b);
 ...
 private:
  ...
  typedef std::vector<std::tuple<Format, Datatype, std::vector<int>,
                                 std::shared_ptr<ir::Module>>>
      HelperFuncsCache;
  static HelperFuncsCache helperFunctions;
  static std::mutex helperFunctionsMutex;

  typedef std::vector<std::pair<IndexStmt, std::shared_ptr<ir::Module>>>
      KernelsCache;
...
class Tensor : public TensorBase {
 public:
  ...
  Tensor(std::vector<int> dimensions,
         ModeFormat modeType = ModeFormat::compressed, CType fill = CType());
  ...
  /// Simple transpose that packs a new tensor from the values in the current
  /// tensor.
  Tensor<CType> transpose(std::string name,
                          std::vector<int> newModeOrdering) const;
  Tensor<CType> transpose(std::vector<int> newModeOrdering) const;
  Tensor<CType> transpose(std::vector<int> newModeOrdering,
                          Format format) const;
  Tensor<CType> transpose(std::string name, std::vector<int> newModeOrdering,
                          Format format) const;
 ...
 private:
  ...
  /// Access() expressions containing mixes of IndexVar and WindowedIndexVar
  /// objects.
  template <typename First, typename... Rest>
  std::vector<std::shared_ptr<IndexVarInterface>> _access(const First &first,
                                                          const Rest &...rest);
...
struct ScalarAccess {
  ...
  void operator=(CType scalar) { tensor->insert<CType>(indices, scalar); }

  void operator+=(CType scalar) { tensor->insert<CType>(indices, scalar); }

  operator CType() { return tensor->at<CType>(indices); }
...
/// Read a tensor from a stream of the given file format. The tensor is
/// returned packed by default.
...
TensorBase makeCSR(const std::string &name, const std::vector<int> &dimensions,
                   ...
                   Array::Policy policy = Array::UserOwns) {
...
TensorBase makeCSR(const std::string &name, const std::vector<int> &dimensions,
                   ...
                   const std::vector<int> &colidx, const std::vector<T> &vals) {
  taco_uassert(dimensions.size() == 2) << error::requires_matrix;
...
template <typename T>
void getCSRArrays(const TensorBase &tensor, int **rowptr, int **colidx,
                  T **vals) {
  taco_uassert(tensor.getFormat() == CSR)
      << "The tensor " << tensor.getName()
      << " is not defined in the CSR format";
  auto storage = tensor.getStorage();
...
/// The arrays remain owned by the user and will not be freed by taco.
template <typename T>
...
TensorBase makeCSC(const std::string &name, const std::vector<int> &dimensions,
                   ...
                   const std::vector<int> &rowidx, const std::vector<T> &vals) {
  taco_uassert(dimensions.size() == 2) << error::requires_matrix;
  Tensor<T> tensor(name, dimensions, CSC);
  auto storage = tensor.getStorage();
...
/// Get the arrays that makes up a compressed sparse columns (CSC) tensor.
/// This function does not change the ownership of the arrays.
template <typename T>
void getCSCArrays(const TensorBase &tensor, int **colptr, int **rowidx,
                  T **vals) {
...
struct TensorBase::Content {
  ...
  Content(std::string name, Datatype dataType,
          const std::vector<int> &dimensions, Format format, Literal fill)
      : dataType(dataType),
        dimensions(dimensions),
        storage(TensorStorage(dataType, dimensions, format, fill)),
        tensorVar(TensorVar(util::getUniqueId(), name,
                            Type(dataType, convert(dimensions)), format,
                            fill)) {
...
template <typename CType>
void TensorBase::insert(const std::initializer_list<int> &coordinate,
                        CType value) {
  ...
  if ((content->coordinateBuffer->size() - content->coordinateBufferUsed) <
      content->coordinateSize) {
    content->coordinateBuffer->resize(content->coordinateBuffer->size() +
                                      content->coordinateSize);
  }
  int *coordLoc =
      (int *)&content->coordinateBuffer->data()[content->coordinateBufferUsed];
...
template <typename CType>
void TensorBase::insertUnsynced(const std::vector<int> &coordinate,
                                CType value) {
  taco_uassert(coordinate.size() == (size_t)getOrder())
      << "Wrong number of indices";
  taco_uassert(getComponentType() == type<CType>())
...
void TensorBase::insertUnchecked(
    const typename TensorBase::const_iterator<T, CType>::Coordinates
        &coordinate,
    CType value) {
  if ((content->coordinateBuffer->size() - content->coordinateBufferUsed) <
      content->coordinateSize) {
...
const Access TensorBase::operator()(const IndexVars &...indices) const {
  return static_cast<const TensorBase *>(this)->operator()(
...
template <typename InputIterators>
void TensorBase::setFromComponents(const InputIterators &begin,
                                   const InputIterators &end) {
...
/// The _access() methods perform primitive recursion on the input variadic
/// template. This means that each instance of the _access method matches on
/// the first element of the variadic template parameter pack, performs an
/// "action", then recurses with the remaining elements in the parameter pack
/// through a recursive call to _access. Since this is recursion, we need a
/// base case. The empty argument instance of _access returns an empty value
/// of the desired type, in this case a vector of IndexVarInterface.
template <typename CType>
std::vector<std::shared_ptr<IndexVarInterface>> Tensor<CType>::_access() {
  return std::vector<std::shared_ptr<IndexVarInterface>>{};
}

/// The recursive case of _access matches on the first element, and attempts
/// to create a shared_ptr out of it. It then makes a recursive call to get a
/// vector with the rest of the elements. Then, it pushes the first element
/// onto the back of the vector -- this check ensures that the type First is
/// indeed a member of IndexVarInterface.
template <typename CType>
template <typename First, typename... Rest>
std::vector<std::shared_ptr<IndexVarInterface>> Tensor<CType>::_access(
...
/// _access_wrapper just calls into _access and reverses the result to get the
/// initial order of the arguments.
template <typename CType>
template <typename... Args>
...
/// We have to case on whether the first argument is an IndexVar or a
/// WindowedIndexVar so that the template engine can differentiate between the
/// two versions.
template <typename CType>
template <typename... IndexVars>
Access Tensor<CType>::operator()(const IndexVar &first,
                                 const IndexVars &...indices) {
  return this->_access_wrapper(first, indices...);
}
template <typename CType>
template <typename... IndexVars>
Access Tensor<CType>::operator()(const WindowedIndexVar &first,
                                 const IndexVars &...indices) {
  return this->_access_wrapper(first, indices...);
}
template <typename CType>
template <typename... IndexVars>
Access Tensor<CType>::operator()(const IndexSetVar &first,
                                 const IndexVars &...indices) {
  return this->_access_wrapper(first, indices...);
}
...
template <typename CType>
template <typename... Ints>
ScalarAccess<CType> Tensor<CType>::operator()(const int index,
                                              const Ints &...indices) {
  return this->operator()(std::vector<int>{index, indices...});
}

template <typename CType>
Tensor<CType> Tensor<CType>::transpose(std::string name,
                                       std::vector<int> newModeOrdering) const {
...
template <typename CType>
Tensor<CType> Tensor<CType>::transpose(std::vector<int> newModeOrdering,
                                       Format format) const {
...
template <typename CType>
Tensor<CType> Tensor<CType>::removeExplicitFillValues(Format format,
                                                      int value) const {
...
template <typename CType>
void Tensor<CType>::operator=(const IndexExpr &expr) {
  TensorBase::operator=(expr);
}

enum class ParallelSchedule { Static, Dynamic };
