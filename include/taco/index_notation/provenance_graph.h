...
namespace taco {
  ...
  struct IndexVarRelNode: public util::Manageable<IndexVarRelNode>, private util::Uncopyable {
    ...
    virtual std::vector<ir::Expr> deriveIterBounds(IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const;
  ...
  private:
    ...
    std::vector<ir::Expr> locateBounds(std::vector<ir::Expr> coordBounds, Datatype boundType, Iterators iterators, ProvenanceGraph provGraph) const;
    struct Content;
    std::shared_ptr<Content> content;
  };
