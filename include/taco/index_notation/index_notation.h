...
namespace taco {
  ...
  public:
    ...
    Access(const TensorVar &tensorVar, const std::vector<IndexVar> &indices = {}, const std::map<int, std::shared_ptr<IndexVarIterationModifier>> &modifiers = {}, bool isAccessingStructure = false);
  ...
  public:
    ...
    CallIntrinsic(const std::shared_ptr<Intrinsic> &func, const std::vector<IndexExpr> &args);
  ...
  public:
    ...
    ///  allows us to leverage scratchpad memories and
    ///  reorder computations to increase locality
    IndexStmt precompute(IndexExpr expr, std::vector<IndexVar> i_vars, std::vector<IndexVar> iw_vars, TensorVar workspace) const;
    ...
    IndexStmt assemble(TensorVar result, AssembleStrategy strategy, bool separately_schedulable = false) const;
  ...
  public:
    ...
    Assignment(TensorVar tensor, std::vector<IndexVar> indices, IndexExpr rhs, IndexExpr op = IndexExpr(), const std::map<int, std::shared_ptr<IndexVarIterationModifier>> &modifiers = {});
  ...
  public:
    ...
    static void match(std::shared_ptr<IndexVarInterface> ptr, std::function<void(std::shared_ptr<IndexVar>)> ivarFunc, std::function<void(std::shared_ptr<WindowedIndexVar>)> wvarFunc, std::function<void(std::shared_ptr<IndexSetVar>)> isetVarFunc) {
  ...
  public:
    ...
    TensorVar(const int &id, const std::string &name, const Type &type, const Format &format, const Literal &fill = Literal());
  ...
  std::vector<ir::Expr> createVars(const std::vector<TensorVar> &tensorVars, std::map<TensorVar, ir::Expr> *vars, bool isParameter = false);
