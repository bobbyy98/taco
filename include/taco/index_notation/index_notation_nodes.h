...
namespace taco {
  ...
  struct IndexVarIterationModifier {
    ...
    static void match(std::shared_ptr<IndexVarIterationModifier> ptr, std::function<void(std::shared_ptr<AccessWindow>)> windowFunc, std::function<void(std::shared_ptr<IndexSet>)> indexSetFunc) {
  ...
  struct AccessNode: public IndexExprNode {
    AccessNode(TensorVar tensorVar, const std::vector<IndexVar> &indices, const std::map<int, std::shared_ptr<IndexVarIterationModifier>> &modifiers, bool isAccessingStructure):
        ...
        for (auto &it: modifiers) {
          IndexVarIterationModifier::match(
              it.second,
              [&](std::shared_ptr<AccessWindow> w) {
  ...
  protected:
    ...
    explicit AccessNode(TensorVar tensorVar): IndexExprNode(tensorVar.getType().getDataType()), tensorVar(tensorVar), isAccessingStructure(false) {
    }
  };
  ...
  struct LiteralNode: public IndexExprNode {
    ...
    template <typename T>
    T getVal() const {
      taco_iassert(getDataType() == type<T>()) << "Attempting to get data of wrong type";
  ...
  protected:
    ...
    BinaryExprNode(IndexExpr a, IndexExpr b): IndexExprNode(max_type(a.getDataType(), b.getDataType())), a(a), b(b) {
    }
  };
  ...
  struct CallIntrinsicNode: public IndexExprNode {
    CallIntrinsicNode(const std::shared_ptr<Intrinsic> &func, const std::vector<IndexExpr> &args);
  ...
  struct CallNode: public IndexExprNode {
    ...
    CallNode(std::string name, const std::vector<IndexExpr> &args, OpImpl lowerFunc, const IterationAlgebra &iterAlg, const std::vector<Property> &properties, const std::map<std::vector<int>, OpImpl> &regionDefinitions, const std::vector<int> &definedRegions);

    CallNode(std::string name, const std::vector<IndexExpr> &args, OpImpl lowerFunc, const IterationAlgebra &iterAlg, const std::vector<Property> &properties, const std::map<std::vector<int>, OpImpl> &regionDefinitions);
  ...
  private:
    static Datatype inferReturnType(OpImpl f, const std::vector<IndexExpr> &inputs) {
      std::function<ir::Expr(IndexExpr)> getExprs = [](IndexExpr arg) {
        return ir::Var::make("t", arg.getDataType());
      };
  ...
  struct AssignmentNode: public IndexStmtNode {
    AssignmentNode(const Access &lhs, const IndexExpr &rhs, const IndexExpr &op): lhs(lhs), rhs(rhs), op(op) {
    }
  ...
  struct YieldNode: public IndexStmtNode {
    YieldNode(const std::vector<IndexVar> &indexVars, IndexExpr expr): indexVars(indexVars), expr(expr) {
    }
  ...
  struct WhereNode: public IndexStmtNode {
    WhereNode(IndexStmt consumer, IndexStmt producer): consumer(consumer), producer(producer) {
    }
  ...
  struct SequenceNode: public IndexStmtNode {
    SequenceNode(IndexStmt definition, IndexStmt mutation): definition(definition), mutation(mutation) {
    }
  ...
  struct AssembleNode: public IndexStmtNode {
    AssembleNode(IndexStmt queries, IndexStmt compute, Assemble::AttrQueryResults results): queries(queries), compute(compute), results(results) {
    }
