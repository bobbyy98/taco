...
namespace taco {
  ...
  public:
    // Full construction
    Func(FuncBodyGenerator lowererFunc, FuncAlgebraGenerator algebraFunc, std::vector<Property> properties, std::map<std::vector<int>, FuncBodyGenerator> specialDefinitions = {});

    Func(std::string name, FuncBodyGenerator lowererFunc, FuncAlgebraGenerator algebraFunc, std::vector<Property> properties, std::map<std::vector<int>, FuncBodyGenerator> specialDefinitions = {});

    // Construct without specifying algebra
    Func(std::string name, FuncBodyGenerator lowererFunc, std::vector<Property> properties, std::map<std::vector<int>, FuncBodyGenerator> specialDefinitions = {});

    Func(FuncBodyGenerator lowererFunc, std::vector<Property> properties, std::map<std::vector<int>, FuncBodyGenerator> specialDefinitions = {});

    // Construct without properties
    Func(std::string name, FuncBodyGenerator lowererFunc, FuncAlgebraGenerator algebraFunc, std::map<std::vector<int>, FuncBodyGenerator> specialDefinitions = {});
    ...
    Call operator()(IndexExprs &&...exprs) {
      ...
      CallNode *op = new CallNode(name, actualArgs, lowererFunc, nodeAlgebra, properties, regionDefinitions);
