...
namespace taco {
  ...
  struct Isomorphic: public IndexNotationVisitorStrict {
    ...
    void visit(const AccessNode *anode) {
      ...
      auto bnode = to<AccessNode>(bExpr.ptr);
        ...
        if (anode->isAccessingStructure != bnode->isAccessingStructure || anode->windowedModes != bnode->windowedModes) {
          eq = false;
          return;
      }
    ...
    void visit(const CastNode *anode) {
      ...
      auto bnode = to<CastNode>(bExpr.ptr);
        if (anode->getDataType() != bnode->getDataType() || !check(anode->a, bnode->a)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const CallIntrinsicNode *anode) {
      ...
      auto bnode = to<CallIntrinsicNode>(bExpr.ptr);
        if (anode->func->getName() != bnode->func->getName() || anode->args.size() != bnode->args.size()) {
          eq = false;
          return;
      }
    ...
    void visit(const ReductionNode *anode) {
      ...
      auto bnode = to<ReductionNode>(bExpr.ptr);
        if (!check(anode->op, bnode->op) || !check(anode->var, bnode->var) || !check(anode->a, bnode->a)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const AssignmentNode *anode) {
      ...
      auto bnode = to<AssignmentNode>(bStmt.ptr);
        if (!check(anode->lhs, bnode->lhs) || !check(anode->rhs, bnode->rhs) || !check(anode->op, bnode->op)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const ForallNode *anode) {
      ...
      auto bnode = to<ForallNode>(bStmt.ptr);
        if (!check(anode->indexVar, bnode->indexVar) || !check(anode->stmt, bnode->stmt) || anode->parallel_unit != bnode->parallel_unit ||
            anode->output_race_strategy != bnode->output_race_strategy || anode->unrollFactor != bnode->unrollFactor) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const WhereNode *anode) {
      ...
      auto bnode = to<WhereNode>(bStmt.ptr);
        if (!check(anode->consumer, bnode->consumer) || !check(anode->producer, bnode->producer)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const SequenceNode *anode) {
      ...
      auto bnode = to<SequenceNode>(bStmt.ptr);
        if (!check(anode->definition, bnode->definition) || !check(anode->mutation, bnode->mutation)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const AssembleNode *anode) {
      ...
      auto bnode = to<AssembleNode>(bStmt.ptr);
        if (!check(anode->queries, bnode->queries) || !check(anode->compute, bnode->compute)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const MultiNode *anode) {
      ...
      auto bnode = to<MultiNode>(bStmt.ptr);
        if (!check(anode->stmt1, bnode->stmt1) || !check(anode->stmt2, bnode->stmt2)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const SuchThatNode *anode) {
      ...
      auto bnode = to<SuchThatNode>(bStmt.ptr);
        if (!check(anode->stmt, bnode->stmt) || anode->predicate != bnode->predicate) {
          eq = false;
          return;
      }
      eq = true;
  ...
  struct Equals: public IndexNotationVisitorStrict {
    ...
    void visit(const AccessNode *anode) {
      ...
      auto bnode = to<AccessNode>(bExpr.ptr);
        ...
        if (anode->isAccessingStructure != bnode->isAccessingStructure || anode->windowedModes != bnode->windowedModes || anode->indexSetModes != bnode->indexSetModes) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const CastNode *anode) {
      ...
      auto bnode = to<CastNode>(bExpr.ptr);
        if (anode->getDataType() != bnode->getDataType() || !equals(anode->a, bnode->a)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const ReductionNode *anode) {
      ...
      auto bnode = to<ReductionNode>(bExpr.ptr);
        if (!equals(anode->op, bnode->op) || anode->var != bnode->var || !equals(anode->a, bnode->a)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const AssignmentNode *anode) {
      ...
      auto bnode = to<AssignmentNode>(bStmt.ptr);
        if (!equals(anode->lhs, bnode->lhs) || !equals(anode->rhs, bnode->rhs) || !equals(anode->op, bnode->op)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const ForallNode *anode) {
      ...
      auto bnode = to<ForallNode>(bStmt.ptr);
        if (anode->indexVar != bnode->indexVar || !equals(anode->stmt, bnode->stmt) || anode->parallel_unit != bnode->parallel_unit ||
            anode->output_race_strategy != bnode->output_race_strategy || anode->unrollFactor != bnode->unrollFactor) {
          eq = false;
          return;
      }
    ...
    void visit(const WhereNode *anode) {
      ...
      auto bnode = to<WhereNode>(bStmt.ptr);
        if (!equals(anode->consumer, bnode->consumer) || !equals(anode->producer, bnode->producer)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const SequenceNode *anode) {
      ...
      auto bnode = to<SequenceNode>(bStmt.ptr);
        if (!equals(anode->definition, bnode->definition) || !equals(anode->mutation, bnode->mutation)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const AssembleNode *anode) {
      ...
      auto bnode = to<AssembleNode>(bStmt.ptr);
        if (!equals(anode->queries, bnode->queries) || !equals(anode->compute, bnode->compute)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const MultiNode *anode) {
      ...
      auto bnode = to<MultiNode>(bStmt.ptr);
        if (!equals(anode->stmt1, bnode->stmt1) || !equals(anode->stmt2, bnode->stmt2)) {
          eq = false;
          return;
      }
      eq = true;
    ...
    void visit(const SuchThatNode *anode) {
      ...
      auto bnode = to<SuchThatNode>(bStmt.ptr);
        if (anode->predicate != bnode->predicate || !equals(anode->stmt, bnode->stmt)) {
          eq = false;
          return;
      }
      eq = true;
  ...
  Access::Access(const TensorVar &tensor, const std::vector<IndexVar> &indices, const std::map<int, std::shared_ptr<IndexVarIterationModifier>> &modifiers, bool isAccessingStructure):
  ...
  template <>
  bool isa<Access>(IndexExpr e) {
    return isa<AccessNode>(e.ptr);
  }

  template <>
  Access to<Access>(IndexExpr e) {
  ...
  Literal Literal::zero(Datatype type) {
      switch (type.getKind()) {
        ...
        case Datatype::UInt8:
          return Literal(uint8_t(0));
        case Datatype::UInt16:
          return Literal(uint16_t(0));
        case Datatype::UInt32:
          return Literal(uint32_t(0));
        case Datatype::UInt64:
          return Literal(uint64_t(0));
        case Datatype::Int8:
          return Literal(int8_t(0));
        case Datatype::Int16:
          return Literal(int16_t(0));
        case Datatype::Int32:
          return Literal(int32_t(0));
        case Datatype::Int64:
          return Literal(int64_t(0));
        case Datatype::Float32:
          return Literal(float(0.0));
        case Datatype::Float64:
          return Literal(double(0.0));
        case Datatype::Complex64:
          return Literal(std::complex<float>());
        case Datatype::Complex128:
          return Literal(std::complex<double>());
        default:
  ...
  template <>
  bool isa<Literal>(IndexExpr e) {
    return isa<LiteralNode>(e.ptr);
  }

  template <>
  Literal to<Literal>(IndexExpr e) {
  ...
  template <>
  bool isa<Neg>(IndexExpr e) {
    return isa<NegNode>(e.ptr);
  }

  template <>
  Neg to<Neg>(IndexExpr e) {
  ...
  template <>
  bool isa<Add>(IndexExpr e) {
    return isa<AddNode>(e.ptr);
  }

  template <>
  Add to<Add>(IndexExpr e) {
  ...
  template <>
  bool isa<Sub>(IndexExpr e) {
    return isa<SubNode>(e.ptr);
  }

  template <>
  Sub to<Sub>(IndexExpr e) {
  ...
  template <>
  bool isa<Mul>(IndexExpr e) {
    return isa<MulNode>(e.ptr);
  }

  template <>
  Mul to<Mul>(IndexExpr e) {
  ...
  template <>
  bool isa<Div>(IndexExpr e) {
    return isa<DivNode>(e.ptr);
  }

  template <>
  Div to<Div>(IndexExpr e) {
  ...
  template <>
  bool isa<Sqrt>(IndexExpr e) {
    return isa<SqrtNode>(e.ptr);
  }

  template <>
  Sqrt to<Sqrt>(IndexExpr e) {
  ...
  template <>
  bool isa<Cast>(IndexExpr e) {
    return isa<CastNode>(e.ptr);
  }

  template <>
  Cast to<Cast>(IndexExpr e) {
  ...
  template <>
  bool isa<Call>(IndexExpr e) {
    return isa<CallNode>(e.ptr);
  }

  template <>
  Call to<Call>(IndexExpr e) {
  ...
  CallIntrinsic::CallIntrinsic(const std::shared_ptr<Intrinsic> &func, const std::vector<IndexExpr> &args): CallIntrinsic(new CallIntrinsicNode(func, args)) {
  }
  ...
  template <>
  bool isa<CallIntrinsic>(IndexExpr e) {
    return isa<CallIntrinsicNode>(e.ptr);
  }

  template <>
  CallIntrinsic to<CallIntrinsic>(IndexExpr e) {
  ...
  Reduction::Reduction(IndexExpr op, IndexVar var, IndexExpr expr): Reduction(new ReductionNode(op, var, expr)) {
  }
  ...
  template <>
  bool isa<Reduction>(IndexExpr s) {
    return isa<ReductionNode>(s.ptr);
  }

  template <>
  Reduction to<Reduction>(IndexExpr s) {
  ...
  IndexStmt::IndexStmt(const IndexStmtNode *n): util::IntrusivePtr<const IndexStmtNode>(n) {
  }
  ...
  std::vector<IndexVar> IndexStmt::getIndexVars() const {
    ...
    ;
    set<IndexVar> seen;
    match(*this, std::function<void(const AssignmentNode *, Matcher *)>([&](const AssignmentNode *op, Matcher *ctx) {
            ...
            auto &vars = op->indexVars;
              for (size_t i = 0; i < vars.size(); i++) {
                  if (!util::contains(indexVarDomains, vars[i])) {
                    ...
                    taco_iassert(indexVarDomains.at(vars[i]) == type.getShape().getDimension(i)) << "Index variable used to index incompatible dimensions";
                  }
              }
          }));
  ...
  IndexStmt IndexStmt::precompute(IndexExpr expr, std::vector<IndexVar> i_vars, std::vector<IndexVar> iw_vars, TensorVar workspace) const {
  ...
  IndexStmt IndexStmt::assemble(TensorVar result, AssembleStrategy strategy, bool separatelySchedulable) const {
    string    reason;
    IndexStmt transformed = SetAssembleStrategy(result, strategy, separatelySchedulable).apply(*this, &reason);
      if (!transformed.defined()) {
        taco_uerror << reason;
    }
    return transformed;
  ...
  Assignment::Assignment(Access lhs, IndexExpr rhs, IndexExpr op): Assignment(new AssignmentNode(lhs, rhs, op)) {
  }

  Assignment::Assignment(TensorVar tensor, vector<IndexVar> indices, IndexExpr rhs, IndexExpr op, const std::map<int, std::shared_ptr<IndexVarIterationModifier>> &modifiers):
  ...
  std::vector<IndexVar> Assignment::getReductionVars() const {
    ...
    match(getRhs(), std::function<void(const AccessNode *)>([&](const AccessNode *op) {
            for (auto &var: op->indexVars) {
                if (!util::contains(seen, var)) {
  ...
  template <>
  bool isa<Assignment>(IndexStmt s) {
    return isa<AssignmentNode>(s.ptr);
  }

  template <>
  Assignment to<Assignment>(IndexStmt s) {
  ...
  Yield::Yield(const std::vector<IndexVar> &indexVars, IndexExpr expr): Yield(new YieldNode(indexVars, expr)) {
  }
  ...
  template <>
  bool isa<Forall>(IndexStmt s) {
    return isa<ForallNode>(s.ptr);
  }

  template <>
  Forall to<Forall>(IndexStmt s) {
  ...
  Where::Where(IndexStmt consumer, IndexStmt producer): Where(new WhereNode(consumer, producer)) {
  }
  ...
  template <>
  bool isa<Where>(IndexStmt s) {
    return isa<WhereNode>(s.ptr);
  }

  template <>
  Where to<Where>(IndexStmt s) {
  ...
  Sequence::Sequence(IndexStmt definition, IndexStmt mutation): Sequence(new SequenceNode(definition, mutation)) {
  }
  ...
  template <>
  bool isa<Sequence>(IndexStmt s) {
    return isa<SequenceNode>(s.ptr);
  }

  template <>
  Sequence to<Sequence>(IndexStmt s) {
  ...
  Assemble::Assemble(IndexStmt queries, IndexStmt compute, AttrQueryResults results): Assemble(new AssembleNode(queries, compute, results)) {
  }
  ...
  Assemble assemble(IndexStmt queries, IndexStmt compute, Assemble::AttrQueryResults results) {
    return Assemble(queries, compute, results);
  }

  template <>
  bool isa<Assemble>(IndexStmt s) {
    return isa<AssembleNode>(s.ptr);
  }

  template <>
  Assemble to<Assemble>(IndexStmt s) {
  ...
  Multi::Multi(IndexStmt stmt1, IndexStmt stmt2): Multi(new MultiNode(stmt1, stmt2)) {
  }
  ...
  template <>
  bool isa<Multi>(IndexStmt s) {
    return isa<MultiNode>(s.ptr);
  }

  template <>
  Multi to<Multi>(IndexStmt s) {
  ...
  SuchThat::SuchThat(IndexStmt stmt, std::vector<IndexVarRel> predicate): SuchThat(new SuchThatNode(stmt, predicate)) {
  }
  ...
  template <>
  bool isa<SuchThat>(IndexStmt s) {
    return isa<SuchThatNode>(s.ptr);
  }

  template <>
  SuchThat to<SuchThat>(IndexStmt s) {
  ...
  template <>
  bool isa<IndexVar>(IndexExpr e) {
    return isa<IndexVarNode>(e.ptr);
  }

  template <>
  IndexVar to<IndexVar>(IndexExpr e) {
  ...
  std::ostream &operator<<(std::ostream &os, const std::shared_ptr<IndexVarInterface> &var) {
    std::stringstream ss;
    IndexVarInterface::match(
        var,
        [&](std::shared_ptr<IndexVar> ivar) {
          ss << *ivar;
        },
  ...
  TensorVar::TensorVar(const Type &type, const Literal &fill): TensorVar(type, createDenseFormat(type), fill) {
  }

  TensorVar::TensorVar(const std::string &name, const Type &type, const Literal &fill): TensorVar(-1, name, type, createDenseFormat(type), fill) {
  }

  TensorVar::TensorVar(const Type &type, const Format &format, const Literal &fill): TensorVar(-1, util::uniqueName('A'), type, format, fill) {
  }

  TensorVar::TensorVar(const string &name, const Type &type, const Format &format, const Literal &fill): TensorVar(-1, name, type, format, fill) {
  }

  TensorVar::TensorVar(const int &id, const string &name, const Type &type, const Format &format, const Literal &fill): content(new Content) {
    content->id     = id;
    content->name   = name;
    content->type   = type;
  ...
  const Access TensorVar::operator()(const std::vector<IndexVar> &indices) const {
    taco_uassert((int) indices.size() == getOrder()) << "A tensor of order " << getOrder() << " must be indexed with " << getOrder() << " variables, but is indexed with:  " << util::join(indices);
  ...
  Assignment TensorVar::operator=(IndexExpr expr) {
    taco_uassert(getOrder() == 0) << "Must use index variable on the left-hand-side when assigning an "
  ...
  bool isEinsumNotation(IndexStmt stmt, std::string *reason) {
    ...
    match(stmt, std::function<void(const AddNode *, Matcher *)>([&](const AddNode *op, Matcher *ctx) {
            if (mulnodeVisited) {
          ...
          std::function<void(const BinaryExprNode *)>([&](const BinaryExprNode *op) {
            *reason  = "einsum notation may not contain " + op->getOperatorString() + " operations";
            isEinsum = false;
          }),
  ...
  bool isReductionNotation(IndexStmt stmt, std::string *reason) {
    ...
    match(stmt, std::function<void(const ReductionNode *, Matcher *)>([&](const ReductionNode *op, Matcher *ctx) {
            boundVars.scope();
  ...
  bool isConcreteNotation(IndexStmt stmt, std::string *reason) {
    ...
    match(stmt, std::function<void(const ForallNode *, Matcher *)>([&](const ForallNode *op, Matcher *ctx) {
            boundVars.scope();
          ...
          std::function<void(const WhereNode *, Matcher *)>([&](const WhereNode *op, Matcher *ctx) {
            ...
            if (!alreadyInProducer)
              inWhereProducer = false;
            bool alreadyInConsumer = inWhereConsumer;
            inWhereConsumer        = true;
            ctx->match(op->consumer);
            if (!alreadyInConsumer)
              inWhereConsumer = false;
          }),
          std::function<void(const AssignmentNode *, Matcher *)>([&](const AssignmentNode *op, Matcher *ctx) {
          ...
          int numReductionVars = 0;
            ...
            if (numReductionVars > 0 && op->op == IndexExpr() && !inWhereProducer) {
  ...
  Assignment makeReductionNotation(Assignment assignment) {
    ...
    struct MakeReductionNotation: IndexNotationRewriter {
      MakeReductionNotation(const std::vector<IndexVar> &free): free(free.begin(), free.end()) {
      }
    ...
    return Assignment(assignment.getLhs(), MakeReductionNotation(free).einsum(expr), assignment.getOperator());
  }
  ...
  struct ReplaceReductionsWithWheres: IndexNotationRewriter {
    ...
    void visit(const AssignmentNode *node) {
      ...
      IndexStmt producer = forall(reduction.getVar(), Assignment(t, reduction.getExpr(), reduction.getOp()));
    ...
    void visit(const ReductionNode *node) {
      ...
      reduction = node;
      t         = TensorVar("t" + util::toString(node->var), node->getDataType());
      expr      = t;
    }
  };
  ...
  IndexStmt makeConcreteNotation(IndexStmt stmt) {
    std::string reason;
    taco_iassert(isReductionNotation(stmt, &reason)) << "Not reduction notation: " << stmt << std::endl << reason;
  ...
  Assignment makeReductionNotationScheduled(Assignment assignment, ProvenanceGraph provGraph) {
    ...
    struct MakeReductionNotation: IndexNotationRewriter {
      MakeReductionNotation(const std::vector<IndexVar> &free, ProvenanceGraph provGraph): free(free.begin(), free.end()), provGraph(provGraph) {
      }
    ...
    return Assignment(assignment.getLhs(), MakeReductionNotation(free, provGraph).einsum(expr), assignment.getOperator());
  }
  ...
  IndexStmt makeConcreteNotationScheduled(IndexStmt stmt, ProvenanceGraph provGraph, vector<IndexVar> forallIndexVars) {
    std::string reason;
    taco_iassert(isReductionNotationScheduled(stmt, provGraph, &reason)) << "Not reduction notation: " << stmt << std::endl << reason;
    taco_iassert(isa<Assignment>(stmt));
    ...
    match(stmt, function<void(const AccessNode *, Matcher *)>([&](const AccessNode *op, Matcher *ctx) {
  ...
  std::vector<TensorVar> getTemporaries(IndexStmt stmt) {
    ...
    match(stmt, function<void(const AssignmentNode *)>([&](const AssignmentNode *op) {
          ...
          function<void(const SequenceNode *, Matcher *)>([&](const SequenceNode *op, Matcher *ctx) {
            if (firstAssignment) {
          ...
          function<void(const MultiNode *, Matcher *)>([&](const MultiNode *op, Matcher *ctx) {
            if (firstAssignment) {
              ctx->match(op->stmt1);
              firstAssignment = true;
              ctx->match(op->stmt2);
          ...
          function<void(const AssembleNode *, Matcher *)>([&](const AssembleNode *op, Matcher *ctx) {
  ...
  std::vector<TensorVar> getAttrQueryResults(IndexStmt stmt) {
    ...
    match(stmt, function<void(const AssembleNode *, Matcher *)>([&](const AssembleNode *op, Matcher *ctx) {
  ...
  std::vector<Access> getArgumentAccesses(IndexStmt stmt) {
    ...
    match(stmt, function<void(const AccessNode *)>([&](const AccessNode *n) {
            if (util::contains(temporaries, n->tensorVar)) {
              return;
          }
          result.push_back(n);
          }),
          function<void(const AssignmentNode *, Matcher *)>([&](const AssignmentNode *n, Matcher *ctx) {
            ctx->match(n->rhs);
          }));
  ...
  std::vector<IndexVar> getReductionVars(IndexStmt stmt) {
    ...
    std::vector<IndexVar> reductionVars, scopedVars, producerScopedVars, consumerScopedVars;
    match(stmt, function<void(const ForallNode *, Matcher *)>([&](const ForallNode *op, Matcher *ctx) {
          ...
          function<void(const AssignmentNode *)>([&](const AssignmentNode *op) {
            ...
            match(op->rhs, std::function<void(const AccessNode *)>([&](const AccessNode *op) {
  ...
  vector<ir::Expr> createVars(const vector<TensorVar> &tensorVars, map<TensorVar, ir::Expr> *vars, bool isParameter) {
    taco_iassert(vars != nullptr);
    vector<ir::Expr> irVars;
      for (auto &var: tensorVars) {
        ir::Expr irVar = ir::Var::make(var.getName(), var.getType().getDataType(), true, true, isParameter);
  ...
  private:
    ...
    void visit(const AccessNode *op) {
        if (util::contains(zeroed, op) || util::contains(zeroedVars, op->tensorVar)) {
          expr = IndexExpr();
        } else {
          expr = op;
        }
    ...
    void visit(const CallNode *op) {
      ...
      Identity identity = findProperty<Identity>(op->properties);
        ...
        if (rewritten) {
          ...
          expr = new CallNode(op->name, args, op->defaultLowerFunc, newAlg, op->properties, op->regionDefinitions, definedArgs);
        } else {
          expr = op;
        }
    }
      ...
      const auto zeroPreservingArgsSets = op->func->zeroPreservingArgs(args);
        for (const auto &zeroPreservingArgs: zeroPreservingArgsSets) {
          taco_iassert(!zeroPreservingArgs.empty());
            if (std::includes(zeroArgs.begin(), zeroArgs.end(), zeroPreservingArgs.begin(), zeroPreservingArgs.end())) {
              expr = IndexExpr();
              return;
          }
        }
  ...
  bool hasNoForAlls(IndexStmt stmt) {
    bool noForAlls = true;
    match(stmt, std::function<void(const ForallNode *)>([&](const ForallNode *op) {
            noForAlls = false;
          }));
    return noForAlls;
  }

  IndexStmt generatePackStmt(TensorVar tensor, std::string otherName, Format otherFormat, std::vector<IndexVar> indexVars, bool otherIsOnRight) {
    const Type type = tensor.getType();
    TensorVar  other(otherName, type, otherFormat);

    const Format format   = tensor.getFormat();
    IndexStmt    packStmt = otherIsOnRight ? (tensor(indexVars) = other(indexVars)) : (other(indexVars) = tensor(indexVars));
  ...
  IndexStmt generatePackCOOStmt(TensorVar tensor, std::vector<IndexVar> indexVars, bool otherIsOnRight) {
    const std::string tensorName = tensor.getName();
    const Format      format     = tensor.getFormat();

    const Format bufferFormat = COO(format.getOrder(), false, true, false, format.getModeOrdering());
