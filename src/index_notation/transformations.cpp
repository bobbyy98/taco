...
namespace taco {
  ...
  Transformation::Transformation(Reorder reorder): transformation(new Reorder(reorder)) {
  }

  Transformation::Transformation(Precompute precompute): transformation(new Precompute(precompute)) {
  }

  Transformation::Transformation(ForAllReplace forallreplace): transformation(new ForAllReplace(forallreplace)) {
  }

  Transformation::Transformation(SetMergeStrategy setmergestrategy): transformation(new SetMergeStrategy(setmergestrategy)) {
  }

  Transformation::Transformation(Parallelize parallelize): transformation(new Parallelize(parallelize)) {
  }

  Transformation::Transformation(AddSuchThatPredicates addsuchthatpredicates): transformation(new AddSuchThatPredicates(addsuchthatpredicates)) {
  }
  ...
  IndexStmt SetMergeStrategy::apply(IndexStmt stmt, string *reason) const {
    ...
    struct SetMergeStrategyRewriter: public IndexNotationRewriter {
      ...
      string           reason;
      SetMergeStrategyRewriter(SetMergeStrategy transformation): transformation(transformation) {
      }
            ...
            MergeLattice lattice = MergeLattice::make(foralli, iterators, provGraph, definedIndexVars);
              for (auto iterator: lattice.iterators()) {
                  if (!iterator.isOrdered()) {
                    reason = "Precondition failed: Variable " + i.getName() + " is not ordered and cannot be galloped.";
                    return;
                }
              }
              if (lattice.points().size() != 1) {
                reason = "Precondition failed: The merge lattice of variable " + i.getName() + " has more than 1 point and cannot be merged by galloping";
                return;
            }

            MergeStrategy strategy = transformation.getMergeStrategy();
            stmt                   = rewrite(foralli.getStmt());
            stmt                   = Forall(node->indexVar, stmt, strategy, node->parallel_unit, node->output_race_strategy, node->unrollFactor);
            return;
        }
  ...
  void SetMergeStrategy::print(std::ostream &os) const {
    os << "mergeby(" << geti() << ", " << MergeStrategy_NAMES[(int) getMergeStrategy()] << ")";
  }
  ...
  Precompute::Precompute(IndexExpr expr, IndexVar i, IndexVar iw, TensorVar workspace): content(new Content) {
  ...
  Precompute::Precompute(IndexExpr expr, std::vector<IndexVar> i_vars, std::vector<IndexVar> iw_vars, TensorVar workspace): content(new Content) {
    content->expr      = expr;
    content->i_vars    = i_vars;
    content->iw_vars   = iw_vars;
    content->workspace = workspace;
  ...
  static Assignment getAssignmentContainingExpr(IndexStmt stmt, IndexExpr expr) {
    ...
    match(stmt, function<void(const AssignmentNode *, Matcher *)>([&assignment, &expr](const AssignmentNode *node, Matcher *ctx) {
  ...
  static IndexStmt eliminateRedundantReductions(IndexStmt stmt, const std::set<TensorVar> *const candidates = nullptr) {
    struct ReduceToAssign: public IndexNotationRewriter {
      ...
      ReduceToAssign(const std::set<TensorVar> *const candidates): candidates(candidates) {
      }
      ...
      void visit(const AssignmentNode *op) {
        const auto result = op->lhs.getTensorVar();
          if (op->op.defined() && util::toSet(op->lhs.getIndexVars()) == availableVars[result] && (!candidates || util::contains(*candidates, result))) {
  ...
  IndexStmt Precompute::apply(IndexStmt stmt, std::string *reason) const {
    ...
    Assignment assignment = getAssignmentContainingExpr(stmt, getExpr());
      if (!assignment.defined()) {
        *reason = "The expression (" + util::toString(getExpr()) + ") " + "is not in " + util::toString(stmt);
        return IndexStmt();
    }

    vector<IndexVar> forallIndexVars;
    match(stmt, function<void(const ForallNode *)>([&](const ForallNode *op) {
    ...
    struct PrecomputeRewriter: public IndexNotationRewriter {
      ...
      Assignment getConsumerAssignment(IndexStmt stmt, TensorVar &ws) {
        ...
        match(stmt, function<void(const AssignmentNode *, Matcher *)>([&](const AssignmentNode *op, Matcher *ctx) {
                a = Assignment(op);
              }),
      ...
      Assignment getProducerAssignment(TensorVar &ws, const std::vector<IndexVar> &i_vars, const std::vector<IndexVar> &iw_vars, const IndexExpr &e, map<IndexVar, IndexVar> substitutions) {
      ...
      bool containsIndexVarScheduled(vector<IndexVar> indexVars, IndexVar indexVar) {
        bool contains = false;
          for (auto &i: indexVars) {
              if (i == indexVar) {
                contains = true;
      ...
      void visit(const ForallNode *node) {
        ...
        match(foralli, function<void(const WhereNode *)>([&](const WhereNode *op) {
                containsWhere = true;
              }));

          if (!containsWhere) {
            vector<IndexVar> forallIndexVars;
            match(foralli, function<void(const ForallNode *)>([&](const ForallNode *op) {
                    forallIndexVars.push_back(op->indexVar);
                  }));
  ...
  void Precompute::print(std::ostream &os) const {
    os << "precompute(" << getExpr() << ", " << getIVars() << ", " << getIWVars() << ", " << getWorkspace() << ")";
  }
  ...
  IndexStmt ForAllReplace::apply(IndexStmt stmt, string *reason) const {
    ...
    struct ForAllReplaceRewriter: public IndexNotationRewriter {
      ...
      ForAllReplaceRewriter(ForAllReplace transformation, string *reason): transformation(transformation), reason(reason) {
      }
      ...
      IndexStmt forallreplace(IndexStmt stmt) {
        IndexStmt replaced = rewrite(stmt);
          ...
          if (replaced == stmt || elementsMatched == -1) {
            *reason = "The pattern of ForAlls: " + util::join(transformation.getPattern()) +
                      " was not found while attempting to replace with: " + util::join(transformation.getReplacement());
            return IndexStmt();
        }
        return replaced;
      }
  ...
  struct ReplaceReductionExpr: public IndexNotationRewriter {
    ...
    ReplaceReductionExpr(const std::map<Access, Access> &substitutions): substitutions(substitutions) {
    }
    using IndexNotationRewriter::visit;
  ...
  IndexStmt scalarPromote(IndexStmt stmt, ProvenanceGraph provGraph, bool isWholeStmt, bool promoteScalar);
      ...
      void visit(const ForallNode *node) {
        ...
        definedIndexVars.insert(foralli.getIndexVar());
          ...
          if (foralli.getIndexVar() == i) {
              ...
              if (parallelize.getOutputRaceStrategy() == OutputRaceStrategy::NoRaces && util::contains(reductionIndexVars, i)) {
            ...
            match(foralli.getStmt(), function<void(const ForallNode *)>([&](const ForallNode *node) {
                    const auto nodeUnderivedAncestors = provGraph.getUnderivedAncestors(node->indexVar);
            ...
            MergeLattice underivedLattice = MergeLattice::make(underivedForall, iterators, provGraph, definedIndexVars);
              ...
              if (parallelize.getOutputRaceStrategy() == OutputRaceStrategy::Temporary && util::contains(reductionIndexVars, underivedForall.getIndexVar())) {
                ...
                match(foralli.getStmt(), function<void(const AssignmentNode *)>([&](const AssignmentNode *node) {
                ...
                IndexStmt precomputed_stmt =
                    forall(i, foralli.getStmt(), foralli.getMergeStrategy(), parallelize.getParallelUnit(), parallelize.getOutputRaceStrategy(), foralli.getUnrollFactor());
                  for (auto assignment: precomputeAssignments) {
                    ...
                    producer = forall(producer_forall.getIndexVar(), producer_forall.getStmt(), foralli.getMergeStrategy(), parallelize.getParallelUnit(),
                                      parallelize.getOutputRaceStrategy(), foralli.getUnrollFactor());
              ...
              if (parallelize.getOutputRaceStrategy() == OutputRaceStrategy::Atomics) {
                ...
                // reducing at end
                IndexStmt body = scalarPromote(foralli.getStmt(), provGraph, false, true);
                stmt = forall(i, body, foralli.getMergeStrategy(), parallelize.getParallelUnit(), parallelize.getOutputRaceStrategy(), foralli.getUnrollFactor());
                return;
            }
  ...
  SetAssembleStrategy::SetAssembleStrategy(TensorVar result, AssembleStrategy strategy, bool separatelySchedulable): content(new Content) {
  ...
  IndexStmt SetAssembleStrategy::apply(IndexStmt stmt, string *reason) const {
    ...
    struct LowerAttrQuery: public IndexNotationRewriter {
      ...
      LowerAttrQuery(TensorVar result, Assemble::AttrQueryResults &queryResults, std::set<TensorVar> &insertedResults):
          result(result),
      ...
      IndexStmt lower(IndexStmt stmt) {
        ...
        temps     = getTemporaries(stmt);
          for (const auto &tmp: temps) {
            tempReplacements[tmp] = TensorVar("q" + tmp.getName(), Type(Bool, tmp.getType().getShape()), tmp.getFormat());
          }
      ...
      void visit(const ForallNode *op) {
        IndexStmt s = rewrite(op->stmt);
          if (s == op->stmt) {
            ...
            stmt = Forall(op->indexVar, s, op->merge_strategy, op->parallel_unit, op->output_race_strategy, op->unrollFactor);
          } else {
            stmt = IndexStmt();
          }
      }
      ...
      void visit(const AssignmentNode *op) {
        ...
        queryResults[resultTensor] = std::vector<std::vector<TensorVar>>(resultTensor.getOrder());
        ...
        std::vector<IndexVar> childCoords;
          ...
          for (size_t i = 0; i < indices.size(); ++i) {
            ...
            childCoords.erase(childCoords.begin());
              ...
              for (const auto &query: modeFormats[i].getAttrQueries(parentCoords, childCoords)) {
                ...
                std::vector<Dimension> queryDims;
                  for (const auto &coord: groupBy) {
                    const auto pos = std::find(groupBy.begin(), groupBy.end(), coord) - groupBy.begin();
                  ...
                  for (const auto &attr: query.getAttrs()) {
                      switch (attr.aggr) {
                          case AttrQuery::COUNT: {
                            ...
                            dedupCoords.insert(dedupCoords.end(), attr.params.begin(), attr.params.end());
                            ...
                            epilog = Assignment(queryResult(groupBy), Cast(dedupTmp(dedupCoords), Int()), Add());
      ...
      void visit(const AccessNode *op) {
          if (util::contains(arguments, op->tensorVar)) {
            expr = Access(op->tensorVar, op->indexVars, op->packageModifiers(), true);
            return;
          } else if (util::contains(temps, op->tensorVar)) {
            expr = Access(tempReplacements[op->tensorVar], op->indexVars, op->packageModifiers());
            return;
        }
    ...
    loweredQueries = eliminateRedundantReductions(loweredQueries, &insertedResults);
    ...
    struct InlineTemporaries: public IndexNotationRewriter {
      ...
      InlineTemporaries(const std::set<TensorVar> &insertedResults, std::set<TensorVar> &inlinedResults):
          insertedResults(insertedResults),
      ...
      void visit(const AssignmentNode *op) {
        ...
        const Access rhsAccess = isa<Access>(op->rhs)                                          ? to<Access>(op->rhs) :
                                 (isa<Cast>(op->rhs) && isa<Access>(to<Cast>(op->rhs).getA())) ? to<Access>(to<Cast>(op->rhs).getA()) :
                                                                                                 Access();
    ...
    struct EliminateRedundantTemps: public IndexNotationRewriter {
      ...
      EliminateRedundantTemps(const std::set<TensorVar> &inlinedResults): inlinedResults(inlinedResults) {
      }
    ...
    loweredQueries = EliminateRedundantTemps(inlinedResults).rewrite(loweredQueries);

    return Assemble(loweredQueries, stmt, queryResults);
  }

  void SetAssembleStrategy::print(std::ostream &os) const {
    os << "assemble(" << getResult() << ", " << AssembleStrategy_NAMES[(int) getAssembleStrategy()] << ")";
  }

  std::ostream &operator<<(std::ostream &os, const SetAssembleStrategy &assemble) {
    assemble.print(os);
    return os;
  }
  ...
  IndexStmt parallelizeOuterLoop(IndexStmt stmt) {
    ...
    Forall forall;
    bool   matched = false;
    match(stmt, function<void(const ForallNode *, Matcher *)>([&forall, &matched](const ForallNode *node, Matcher *ctx) {
            if (!matched)
              forall = node;
            matched = true;
          }));
  ...
  static vector<pair<IndexVar, bool>> varOrderFromTensorLevels(set<pair<IndexVar, pair<int, bool>>> tensorLevelVars) {
    ...
    std::transform(sortedPairs.begin(), sortedPairs.end(), std::back_inserter(varOrder), [](const std::pair<IndexVar, pair<int, bool>> &p) {
  ...
  static vector<IndexVar> topologicallySort(map<IndexVar, set<IndexVar>> hardDeps, map<IndexVar, multiset<IndexVar>> softDeps, vector<IndexVar> originalOrder) {
    ...
    unsigned long    countVars = originalOrder.size();
      while (sortedVars.size() < countVars) {
          ...
          for (size_t i = 0; i < originalOrder.size(); ++i) {
            IndexVar var = originalOrder[i];
              if (!hardDeps.count(var) || hardDeps[var].empty()) {
                const size_t softDepsRemaining = softDeps.count(var) ? softDeps[var].size() : 0;
        ...
        taco_iassert(freeVarPos != std::numeric_limits<size_t>::max()) << "Cycles in iteration graphs must be resolved, through transpose, "
  ...
  IndexStmt reorderLoopsTopologically(IndexStmt stmt) {
    ...
    const auto sortedVars = topologicallySort(hardDeps, collectSoftDeps.softDeps, dagBuilder.indexVarOriginalOrder);
    ...
    struct TopoReorderRewriter: public IndexNotationRewriter {
      ...
      TopoReorderRewriter(const vector<IndexVar> &sortedVars, IndexStmt innerBody, const map<IndexVar, ParallelUnit> forallParallelUnit, const map<IndexVar, OutputRaceStrategy> forallOutputRaceStrategy):
    ...
    TopoReorderRewriter rewriter(sortedVars, dagBuilder.innerBody, dagBuilder.forallParallelUnit, dagBuilder.forallOutputRaceStrategy);
  ...
  IndexStmt scalarPromote(IndexStmt stmt, ProvenanceGraph provGraph, bool isWholeStmt, bool promoteScalar) {
    ...
    struct FindHoistLevel: public IndexNotationVisitor {
      ...
      FindHoistLevel(std::map<Access, const ForallNode *> &hoistLevel, std::map<Access, IndexExpr> &reduceOp, const ProvenanceGraph &provGraph, bool isWholeStmt, bool promoteScalar):
          hoistLevel(hoistLevel),
          reduceOp(reduceOp),
          provGraph(provGraph),
          isWholeStmt(isWholeStmt),
          promoteScalar(promoteScalar) {
      }
      ...
      void visit(const ForallNode *node) {
        ...
        IndexVar i = foralli.getIndexVar();
          ...
          if (foralli.getParallelUnit() == ParallelUnit::GPUWarpReduction || foralli.getParallelUnit() == ParallelUnit::GPUBlockReduction) {
            FindHoistLevel findHoistLevel(hoistLevel, reduceOp, provGraph, false, promoteScalar);
        ...
        std::tie(resultAccesses, std::ignore) = getResultAccesses(foralli);
          for (const auto &resultAccess: resultAccesses) {
            ...
            std::set<IndexVar> resultIndices(resultAccess.getIndexVars().begin(), resultAccess.getIndexVars().end());
              if (std::includes(indices.begin(), indices.end(), resultIndices.begin(), resultIndices.end()) && !util::contains(hoistLevel, resultAccess)) {
      ...
      void visit(const AssignmentNode *op) {
          if (util::contains(hoistLevel, op->lhs) && hoistIndices[op->lhs] == indices) {
    ...
    FindHoistLevel findHoistLevel(hoistLevel, reduceOp, provGraph, isWholeStmt, promoteScalar);
    ...
    struct HoistWrites: public IndexNotationRewriter {
      ...
      HoistWrites(const std::map<Access, const ForallNode *> &hoistLevel, const std::map<Access, IndexExpr> &reduceOp):
          hoistLevel(hoistLevel),
          reduceOp(reduceOp) {
      }
      ...
      void visit(const ForallNode *node) {
        ...
        std::vector<IndexStmt> consumers;
          for (const auto &resultAccess: hoistLevel) {
              if (resultAccess.second == node) {
                ...
                TensorVar val("t" + i.getName() + resultVar.getName(), Type(resultVar.getType().getDataType(), {}));
                body = ReplaceReductionExpr(map<Access, Access>({{resultAccess.first, val()}})).rewrite(body);

                IndexExpr op       = util::contains(reduceOp, resultAccess.first) ? reduceOp.at(resultAccess.first) : IndexExpr();
        ...
        stmt = forall(i, body, foralli.getMergeStrategy(), foralli.getParallelUnit(), foralli.getOutputRaceStrategy(), foralli.getUnrollFactor());
  ...
  static IndexStmt optimizeSpMM(IndexStmt stmt) {
    ...
    Access Caccess = to<Access>(mul.getB());
      ...
      if (Aaccess.getIndexVars().size() != 2 || Baccess.getIndexVars().size() != 2 || Caccess.getIndexVars().size() != 2) {
        return stmt;
    }

      if (!compare(Aaccess.getIndexVars(), {i, j}) || !compare(Baccess.getIndexVars(), {i, k}) || !compare(Caccess.getIndexVars(), {k, j})) {
        return stmt;
    }

    TensorVar A = Aaccess.getTensorVar();
      if (A.getFormat().getModeFormats()[0].getName() != "dense" || A.getFormat().getModeFormats()[1].getName() != "compressed" || A.getFormat().getModeOrdering()[0] != 0 ||
    ...
    TensorVar B = Baccess.getTensorVar();
      if (!B.getFormat().getModeFormats()[0].isOrdered() || !B.getFormat().getModeFormats()[1].isOrdered() || B.getFormat().getModeOrdering()[0] != 0 ||
          B.getFormat().getModeOrdering()[1] != 1) {
        return stmt;
    }

    TensorVar C = Caccess.getTensorVar();
      if (!C.getFormat().getModeFormats()[0].isOrdered() || !C.getFormat().getModeFormats()[1].isOrdered() || C.getFormat().getModeOrdering()[0] != 0 ||
          C.getFormat().getModeOrdering()[1] != 1) {
        return stmt;
    }

    // It's an SpMM statement so return an optimized SpMM statement
    TensorVar w("w", Type(A.getType().getDataType(), {A.getType().getShape().getDimension(1)}), taco::dense);
    return forall(i, where(forall(j, A(i, j) = w(j)), forall(k, forall(j, w(j) += B(i, k) * C(k, j)))));
  }
