...
namespace taco {
  ...
  Kernel::Kernel(IndexStmt stmt, shared_ptr<ir::Module> module, void *evaluate, void *assemble, void *compute): content(new Content) {
  ...
  static inline vector<void *> packArguments(const vector<TensorStorage> &args) {
  ...
  static inline void unpackResults(size_t numResults, const vector<void *> arguments, const vector<TensorStorage> &args) {
  ...
  Kernel compile(IndexStmt stmt) {
    string reason;
    taco_uassert(isConcreteNotation(stmt, &reason)) << "Statement not valid concrete index notation and cannot be compiled. " << reason << endl << stmt;
