...
namespace taco {
  ...
  template <>
  bool isa<Region>(IterationAlgebra alg) {
    return isa<RegionNode>(alg.ptr);
  }

  template <>
  Region to<Region>(IterationAlgebra alg) {
  ...
  template <>
  bool isa<Complement>(IterationAlgebra alg) {
    return isa<ComplementNode>(alg.ptr);
  }

  template <>
  Complement to<Complement>(IterationAlgebra alg) {
  ...
  template <>
  bool isa<Intersect>(IterationAlgebra alg) {
    return isa<IntersectNode>(alg.ptr);
  }

  template <>
  Intersect to<Intersect>(IterationAlgebra alg) {
  ...
  template <>
  bool isa<Union>(IterationAlgebra alg) {
    return isa<UnionNode>(alg.ptr);
  }

  template <>
  Union to<Union>(IterationAlgebra alg) {
  ...
  struct AlgComparer: public IterationAlgebraVisitorStrict {
    ...
    bool binaryCheck(const T *anode, IterationAlgebra b) {
      ...
      return AlgComparer(checkIndexExprs).compare(anode->a, bnode->a) && AlgComparer(checkIndexExprs).compare(anode->b, bnode->b);
    }
