...
namespace taco {
  ...
  std::vector<ir::Expr> IndexVarRelNode::deriveIterBounds(IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const {
    taco_ierror;
    return {};
  }
  ...
  SplitRelNode::SplitRelNode(IndexVar parentVar, IndexVar outerVar, IndexVar innerVar, size_t splitFactor): IndexVarRelNode(SPLIT), content(new Content) {
  ...
  bool SplitRelNode::equals(const SplitRelNode &rel) const {
    return getParentVar() == rel.getParentVar() && getOuterVar() == rel.getOuterVar() && getInnerVar() == rel.getInnerVar() && getSplitFactor() == rel.getSplitFactor();
  }
  ...
  std::vector<ir::Expr> SplitRelNode::deriveIterBounds(taco::IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const {
  ...
  ir::Expr SplitRelNode::recoverVariable(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, ProvenanceGraph provGraph) const {
  ...
  ir::Stmt SplitRelNode::recoverChild(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, bool emitVarDecl, Iterators iterators, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getOuterVar() || indexVar == getInnerVar());
    taco_iassert(variableNames.count(getParentVar()) && variableNames.count(getOuterVar()) && variableNames.count(getInnerVar()));
    Datatype splitFactorType = variableNames[getParentVar()].type();
      if (indexVar == getOuterVar()) {
        ...
        ir::Expr subStmt = ir::Sub::make(variableNames[getParentVar()], ir::Mul::make(variableNames[getOuterVar()], ir::Literal::make(getSplitFactor(), splitFactorType)));
          if (emitVarDecl) {
  ...
  DivideRelNode::DivideRelNode(IndexVar parentVar, IndexVar outerVar, IndexVar innerVar, size_t divFactor): IndexVarRelNode(DIVIDE), content(new Content) {
    content->parentVar = parentVar;
    content->outerVar  = outerVar;
    content->innerVar  = innerVar;
  ...
  bool DivideRelNode::equals(const DivideRelNode &rel) const {
    return getParentVar() == rel.getParentVar() && getOuterVar() == rel.getOuterVar() && getInnerVar() == rel.getInnerVar() && getDivFactor() == rel.getDivFactor();
  }
  ...
  std::vector<ir::Expr> DivideRelNode::deriveIterBounds(taco::IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getOuterVar() || indexVar == getInnerVar());
    taco_iassert(parentIterBounds.size() == 1);
    taco_iassert(parentIterBounds.count(getParentVar()) == 1);
  ...
  ir::Expr DivideRelNode::recoverVariable(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getParentVar());
    taco_iassert(variableNames.count(getParentVar()) && variableNames.count(getOuterVar()) && variableNames.count(getInnerVar()));
  ...
  ir::Stmt DivideRelNode::recoverChild(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, bool emitVarDecl, Iterators iterators, ProvenanceGraph provGraph) const {
  ...
  PosRelNode::PosRelNode(IndexVar i, IndexVar ipos, const Access &access): IndexVarRelNode(POS), content(new Content(i, ipos, access)) {
  }
  ...
  bool PosRelNode::equals(const PosRelNode &rel) const {
    return getParentVar() == rel.getParentVar() && getPosVar() == rel.getPosVar() && getAccess() == rel.getAccess();
  }
  ...
  std::vector<ir::Expr> PosRelNode::deriveIterBounds(taco::IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const {
  ...
  std::vector<ir::Expr> PosRelNode::locateBounds(std::vector<ir::Expr> coordBounds, Datatype boundType, Iterators iterators, ProvenanceGraph provGraph) const {
    Iterator         accessIterator        = getAccessIterator(iterators, provGraph);
    ir::Expr         parentPos             = accessIterator.getParent().getPosVar();
    ModeFunction     segment_bounds        = accessIterator.posBounds(parentPos);
    vector<ir::Expr> binarySearchArgsStart = {getAccessCoordArray(iterators, provGraph),
                                              segment_bounds[0], // arrayStart
                                              segment_bounds[1], // arrayEnd
                                              coordBounds[0]};

    vector<ir::Expr> binarySearchArgsEnd = {getAccessCoordArray(iterators, provGraph),
                                            segment_bounds[0], // arrayStart
                                            segment_bounds[1], // arrayEnd
                                            coordBounds[1]};
  ...
  ir::Expr PosRelNode::recoverVariable(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getParentVar());
  ...
  ir::Stmt PosRelNode::recoverChild(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, bool emitVarDecl, Iterators iterators, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getPosVar());
    ...
    ir::Expr         parentPos        = accessIterator.getParent().getPosVar();
    ModeFunction     segment_bounds   = accessIterator.posBounds(parentPos);
    vector<ir::Expr> binarySearchArgs = {getAccessCoordArray(iterators, provGraph),
                                         segment_bounds[0], // arrayStart
                                         segment_bounds[1], // arrayEnd
  ...
  FuseRelNode::FuseRelNode(IndexVar outerParentVar, IndexVar innerParentVar, IndexVar fusedVar): IndexVarRelNode(FUSE), content(new Content) {
  ...
  bool FuseRelNode::equals(const FuseRelNode &rel) const {
    return getOuterParentVar() == rel.getOuterParentVar() && getInnerParentVar() == rel.getInnerParentVar() && getFusedVar() == rel.getFusedVar();
  }
  ...
  std::vector<ir::Expr> FuseRelNode::deriveIterBounds(taco::IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const {
  ...
  ir::Expr FuseRelNode::recoverVariable(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, ProvenanceGraph provGraph) const {
  ...
  ir::Stmt FuseRelNode::recoverChild(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, bool emitVarDecl, Iterators iterators, ProvenanceGraph provGraph) const {
  ...
  BoundRelNode::BoundRelNode(taco::IndexVar parentVar, taco::IndexVar boundVar, size_t bound, taco::BoundType boundType):
      IndexVarRelNode(BOUND),
      content(new Content) {
    content->parentVar = parentVar;
  ...
  bool BoundRelNode::equals(const BoundRelNode &rel) const {
    return getParentVar() == rel.getParentVar() && getBoundVar() == rel.getBoundVar() && getBound() == rel.getBound() && getBoundType() == rel.getBoundType();
  }
  ...
  std::vector<ir::Expr> BoundRelNode::deriveIterBounds(taco::IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const {
  ...
  ir::Expr BoundRelNode::recoverVariable(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getParentVar());
    taco_iassert(variableNames.count(getBoundVar()) == 1);
    return variableNames[getBoundVar()];
  }

  ir::Stmt BoundRelNode::recoverChild(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, bool emitVarDecl, Iterators iterators, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getBoundVar());
  ...
  PrecomputeRelNode::PrecomputeRelNode(taco::IndexVar parentVar, taco::IndexVar precomputeVar): IndexVarRelNode(PRECOMPUTE), content(new Content) {
    content->parentVar     = parentVar;
  ...
  std::vector<ir::Expr> PrecomputeRelNode::deriveIterBounds(taco::IndexVar indexVar, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, ProvenanceGraph provGraph) const {
  ...
  ir::Expr PrecomputeRelNode::recoverVariable(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators, std::map<IndexVar, std::vector<ir::Expr>> parentIterBounds, std::map<IndexVar, std::vector<ir::Expr>> parentCoordBounds, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getParentVar());
    taco_iassert(variableNames.count(getPrecomputeVar()) == 1);
    return variableNames[getPrecomputeVar()];
  }

  ir::Stmt PrecomputeRelNode::recoverChild(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> variableNames, bool emitVarDecl, Iterators iterators, ProvenanceGraph provGraph) const {
    taco_iassert(indexVar == getPrecomputeVar());
  ...
  ProvenanceGraph::ProvenanceGraph(IndexStmt concreteStmt) {
    ...
    match(concreteStmt, std::function<void(const ForallNode *)>([&](const ForallNode *op) {
  ...
  bool ProvenanceGraph::isRecoverablePrecompute(taco::IndexVar indexVar, std::set<taco::IndexVar> defined, vector<IndexVar> producers, vector<IndexVar> consumers) const {
    vector<IndexVar> childPrecompute;
      ...
      for (const IndexVar &child: getChildren(indexVar)) {
          if (!defined.count(child) && (isFullyDerived(child) || !isRecoverablePrecompute(child, defined, producers, consumers))) {
            return false;
        }
      }
    return true;
  ...
  std::vector<ir::Expr> ProvenanceGraph::deriveIterBounds(IndexVar indexVar, std::vector<IndexVar> derivedVarOrder, std::map<IndexVar, std::vector<ir::Expr>> underivedBounds, std::map<taco::IndexVar, taco::ir::Expr> variableNames, Iterators iterators) const {
  ...
  std::vector<IndexVar> ProvenanceGraph::newlyRecoverableParents(taco::IndexVar indexVar, std::set<taco::IndexVar> previouslyDefined) const {
  ...
  ir::Expr ProvenanceGraph::recoverVariable(taco::IndexVar indexVar, std::vector<IndexVar> definedVarOrder, std::map<IndexVar, std::vector<ir::Expr>> underivedBounds, std::map<taco::IndexVar, taco::ir::Expr> childVariables, Iterators iterators) const {
      if (isFullyDerived(indexVar)) {
        return ir::Expr();
    }
  ...
  ir::Stmt ProvenanceGraph::recoverChild(taco::IndexVar indexVar, std::map<taco::IndexVar, taco::ir::Expr> relVariables, bool emitVarDecl, Iterators iterators) const {
      if (isUnderived(indexVar)) {
        return ir::Stmt();
    }
