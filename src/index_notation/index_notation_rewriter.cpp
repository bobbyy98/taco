  ...
  void IndexNotationRewriter::visit(const CallNode *op) {
    ...
    bool                   rewritten = false;
      ...
      if (rewritten) {
        ...
        expr = new CallNode(op->name, args, op->defaultLowerFunc, newAlg, op->properties, op->regionDefinitions);
      } else {
        expr = op;
      }
  }
...
#define SUBSTITUTE_STMT                             \
  ...
  struct ReplaceRewriter: public IndexNotationRewriter {
    ...
    ReplaceRewriter(const std::map<IndexExpr, IndexExpr> &exprSubstitutions, const std::map<IndexStmt, IndexStmt> &stmtSubstitutions):
  ...
  struct ReplaceIndexVars: public IndexNotationRewriter {
    ...
    ReplaceIndexVars(const std::map<IndexVar, IndexVar> &substitutions): substitutions(substitutions) {
    }
    ...
    void visit(const ForallNode *op) {
      IndexStmt s  = rewrite(op->stmt);
      IndexVar  iv = util::contains(substitutions, op->indexVar) ? substitutions.at(op->indexVar) : op->indexVar;
        if (s == op->stmt && iv == op->indexVar) {
          stmt = op;
        } else {
          stmt = new ForallNode(iv, s, op->merge_strategy, op->parallel_unit, op->output_race_strategy, op->unrollFactor);
        }
    }
  ...
  struct ReplaceTensorVars: public IndexNotationRewriter {
    ...
    ReplaceTensorVars(const std::map<TensorVar, TensorVar> &substitutions): substitutions(substitutions) {
    }
    ...
    void visit(const AccessNode *op) {
      ...
      expr          = (util::contains(substitutions, var)) ? Access(substitutions.at(var), op->indexVars) : op;
    }

    void visit(const AssignmentNode *node) {
      TensorVar var = node->lhs.getTensorVar();
        if (util::contains(substitutions, var)) {
          stmt = Assignment(substitutions.at(var), node->lhs.getIndexVars(), rewrite(node->rhs), node->op);
        } else {
  ...
  IndexExpr replace(IndexExpr expr, const std::map<IndexExpr, IndexExpr> &substitutions) {
    return ReplaceRewriter(substitutions, {}).rewrite(expr);
  }

  IndexExpr replace(IndexExpr expr, const std::map<IndexVar, IndexVar> &substitutions) {
    return ReplaceIndexVars(substitutions).rewrite(expr);
  }

  IndexStmt replace(IndexStmt stmt, const std::map<IndexExpr, IndexExpr> &substitutions) {
    return ReplaceRewriter(substitutions, {}).rewrite(stmt);
  }

  IndexStmt replace(IndexStmt stmt, const std::map<IndexStmt, IndexStmt> &substitutions) {
    return ReplaceRewriter({}, substitutions).rewrite(stmt);
  }

  IndexStmt replace(IndexStmt stmt, const std::map<TensorVar, TensorVar> &substitutions) {
    return ReplaceTensorVars(substitutions).rewrite(stmt);
  }

  IndexStmt replace(IndexStmt stmt, const std::map<IndexVar, IndexVar> &substitutions) {
