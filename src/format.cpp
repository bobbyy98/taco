...
namespace taco {
  ...
  Format::Format(const ModeFormat modeFormat): modeFormatPacks({modeFormat}), modeOrdering({0}) {
  }

  Format::Format(const std::initializer_list<ModeFormatPack> &modeFormatPacks): modeFormatPacks(modeFormatPacks) {
  ...
  Format::Format(const std::vector<ModeFormatPack> &modeFormatPacks): modeFormatPacks(modeFormatPacks) {
    taco_uassert(getOrder() <= INT_MAX) << "Supports only INT_MAX modes";
  ...
  Format::Format(const std::vector<ModeFormatPack> &modeFormatPacks, const std::vector<int> &modeOrdering):
      modeFormatPacks(modeFormatPacks),
      modeOrdering(modeOrdering) {
    taco_uassert(getOrder() <= INT_MAX) << "Supports only INT_MAX modes";
    taco_uassert((size_t) getOrder() == modeOrdering.size()) << "You must either provide a complete mode ordering or none";
  }
  ...
  const std::vector<ModeFormat> Format::getModeFormats() const {
    std::vector<ModeFormat> modeFormats;
      for (auto modeFormatPack: getModeFormatPacks()) {
        modeFormats.insert(modeFormats.end(), modeFormatPack.getModeFormats().begin(), modeFormatPack.getModeFormats().end());
      }
    return modeFormats;
  }
  ...
  bool operator==(const Format &a, const Format &b) {
    ...
    const auto bModeOrdering  = b.getModeOrdering();
      ...
      if (aModeTypePacks.size() != bModeTypePacks.size() || aModeOrdering.size() != bModeOrdering.size()) {
        return false;
    }
  ...
  std::ostream &operator<<(std::ostream &os, const Format &format) {
    return os << "(" << util::join(format.getModeFormatPacks(), ",") << "; " << util::join(format.getModeOrdering(), ",") << ")";
  }
  ...
  ModeFormat ModeFormat::operator()(const std::vector<Property> &properties) const {
  ...
  std::vector<AttrQuery> ModeFormat::getAttrQueries(std::vector<IndexVar> parentCoords, std::vector<IndexVar> childCoords) const {
    taco_iassert(defined());
  ...
  // class ModeTypePack
  ModeFormatPack::ModeFormatPack(const std::vector<ModeFormat> modeFormats): modeFormats(modeFormats) {
      for (const auto &modeFormat: modeFormats) {
        taco_uassert(modeFormat.defined()) << "Cannot have undefined mode type";
      }
  }

  ModeFormatPack::ModeFormatPack(const initializer_list<ModeFormat> modeFormats): modeFormats(modeFormats) {
      for (const auto &modeFormat: modeFormats) {
        taco_uassert(modeFormat.defined()) << "Cannot have undefined mode type";
      }
  }

  ModeFormatPack::ModeFormatPack(const ModeFormat modeFormat): modeFormats({modeFormat}) {
    taco_uassert(modeFormat.defined()) << "Cannot have undefined mode type";
  }
  ...
  const Format COO(int order, bool isUnique, bool isOrdered, bool isAoS, const std::vector<int> &modeOrdering) {
    ...
    ModeFormat::Property ordered = isOrdered ? ModeFormat::ORDERED : ModeFormat::NOT_ORDERED;

    std::vector<ModeFormatPack> modeTypes;
    modeTypes.push_back(Compressed({ordered, (order == 1 && isUnique) ? ModeFormat::UNIQUE : ModeFormat::NOT_UNIQUE}));
      if (order > 1) {
          for (int i = 1; i < order - 1; ++i) {
            modeTypes.push_back(Singleton({ordered, ModeFormat::NOT_UNIQUE}));
          }
        modeTypes.push_back(Singleton({ordered, isUnique ? ModeFormat::UNIQUE : ModeFormat::NOT_UNIQUE}));
    }
    return modeOrdering.empty() ? Format(modeTypes) : Format(modeTypes, modeOrdering);
  }
