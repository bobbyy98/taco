...
namespace taco { namespace parser {
  ...
  EinsumParser::EinsumParser(const std::string &expression, std::vector<TensorBase> &tensors, Format &format, Datatype outType):
      outType(outType),
      format(format),
      tensors(tensors) {
  ...
  std::string EinsumParser::findUniqueIndices(const std::string &subscripts) {
    ...
    std::remove_copy_if(subscripts.begin(), subscripts.end(), std::back_inserter(tempString), [](char chr) {
      return chr == '.' || chr == ',';
    });
  ...
  void EinsumParser::buildResult(std::vector<std::string> subs) {
    ...
    std::vector<int> outShape;
      ...
      if (format.getOrder() != 0 && format.getOrder() != static_cast<int>(outShape.size())) {
        std::ostringstream o;
        o << "Number of dimensions in format (" << format.getOrder() << ") does not match dimensions of output tensor(" << outShape.size() << "),";
        throw ParseError(o.str());
    }
      ...
      if (subscripts.find('.') != std::string::npos) {
        ...
        std::string outEllipse;
          ...
          if (hasOutput) {
            ...
            std::remove_copy_if(sortedOutput.begin(), sortedOutput.end(), std::back_inserter(normalInds), [outEllipse](char chr) {
              return outEllipse.find(chr) != std::string::npos;
            });
