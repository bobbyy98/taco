...
namespace taco { namespace parser {
  ...
  Parser::Parser(string expression, const map<string, Format> &formats, const map<string, Datatype> &dataTypes, const map<string, std::vector<int>> &tensorDimensions, const std::map<std::string, TensorBase> &tensors, int defaultDimension):
              ...
              if (!util::contains(modesWithDefaults, {op->tensorVar, i})) {
                auto dimension = op->tensorVar.getType().getShape().getDimension(i);
                  if (util::contains(*indexVarDimensions, indexVar)) {
                    taco_uassert(indexVarDimensions->at(indexVar) == dimension) << "Incompatible dimensions";
                  } else {
    ...
    struct Rewriter: IndexNotationRewriter {
      ...
      void visit(const AccessNode *op) {
        ...
        taco_uassert(op->indexVars.size() == dimensions.size())
            ...
            << "between tensor accesses or options. Is it order " << dimensions.size() << " or " << op->indexVars.size() << "?";
          ...
          if (dimensionChanged) {
            TensorBase tensor;
              if (util::contains(tensors, op->tensorVar.getName())) {
                ...
                tensor = TensorBase(op->tensorVar.getName(), op->tensorVar.getType().getDataType(), dimensions, op->tensorVar.getFormat());
  ...
  IndexExpr Parser::parseExpr() {
    IndexExpr expr = parseTerm();
      while (content->currentToken == Token::add || content->currentToken == Token::sub) {
  ...
  IndexExpr Parser::parseTerm() {
    IndexExpr term = parseFactor();
      while (content->currentToken == Token::mul || content->currentToken == Token::div) {
          switch (content->currentToken) {
              case Token::mul: {
                consume(Token::mul);
  ...
  const TensorBase &Parser::getTensor(string name) const {
    taco_iassert(name != "");
      if (!hasTensor(name)) {
        taco_uerror << "Parser error: Tensor name " << name << " not found in expression" << endl;
    }
  ...
  string Parser::currentTokenString() {
    return (content->currentToken == Token::identifier) ? content->lexer.getIdentifier() : content->lexer.tokenString(content->currentToken);
  }

  void Parser::consume(Token expected) {
      if (content->currentToken != expected) {
        string error = "Expected \'" + content->lexer.tokenString(expected) + "\' but got \'" + currentTokenString() + "\'";
