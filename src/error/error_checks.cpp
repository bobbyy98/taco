...
namespace taco { namespace error {

  static vector<const AccessNode *> getAccessNodes(const IndexExpr &expr) {
    vector<const AccessNode *> readNodes;
    match(expr, std::function<void(const AccessNode *)>([&](const AccessNode *op) {
            readNodes.push_back(op);
          }));
    return readNodes;
  }

  static string addDimensionError(const IndexVar &var, Dimension dimension1, Dimension dimension2) {
    return "Index variable " + util::toString(var) +
           " is used to index "
           "modes of different dimensions (" +
           util::toString(dimension1) + " and " + util::toString(dimension2) + ").";
  }

  std::pair<bool, string> dimensionsTypecheck(const std::vector<IndexVar> &resultVars, const IndexExpr &expr, const Shape &shape) {
    ...
    std::map<IndexVar, Dimension> indexVarDims;
      for (size_t mode = 0; mode < resultVars.size(); mode++) {
        ...
        auto     dimension = shape.getDimension(mode);
          if (util::contains(indexVarDims, var) && indexVarDims.at(var) != dimension && !(indexVarDims.at(var).isIndexVarSized() && indexVarDims.at(var).getIndexVarSize() == var) &&
  ...
  static void addEdges(vector<IndexVar> indexVars, vector<int> modeOrdering, map<IndexVar, set<IndexVar>> *successors) {
      if (indexVars.size() == 0) {
        return;
    }
  ...
  static bool hasCycle(const IndexVar &var, const map<IndexVar, set<IndexVar>> &successors, set<IndexVar> *visited, set<IndexVar> *marked) {
      if (!util::contains(*visited, var)) {
        visited->insert(var);
        marked->insert(var);

          for (auto &succ: successors.at(var)) {
              if (!util::contains(*visited, succ) && hasCycle(succ, successors, visited, marked)) {
                return true;
  ...
  bool containsTranspose(const Format &resultFormat, const std::vector<IndexVar> &resultVars, const IndexExpr &expr) {
    ...
    match(expr, std::function<void(const AccessNode *)>([&successors](const AccessNode *op) {
            addEdges(op->indexVars, op->tensorVar.getFormat().getModeOrdering(), &successors);
          }));
