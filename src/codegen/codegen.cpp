...
namespace taco { namespace ir {
  ...
  string CodeGen::printContextDeclAndInit(map<Expr, string, ExprCompare> varMap, vector<Expr> localVars, int labels, string funcName) {
    stringstream ret;
    ...
    ret << "  int32_t " << bufCapacityCopyName << " = *" << bufCapacityName << ";" << endl;
    ...
    ret << "    switch (TACO_DEREF(" << stateName << ")) {" << endl;
      for (int i = 0; i <= labels; ++i) {
        ret << "      case " << i << ": goto " << labelPrefix << funcName << i << ";" << endl;
      }
    ret << "    }" << endl;
    ret << "  } else {" << endl;
    ret << "    " << printAlloc("*" + ctxName, "sizeof(" + ctxClassName + ")") << endl;
    ret << "    TACO_DEREF(" << sizeName << ") = sizeof(" << ctxClassName << ");" << endl;
    ret << "  }" << endl;
  ...
  string CodeGen::unpackTensorProperty(string varname, const GetProperty *op, bool is_output_prop) {
    stringstream ret;
    ret << "  ";
    ...
    string tp;
      ...
      if (op->property == TensorProperty::Dimension) {
        tp = "int";
        ret << tp << " " << varname << " = (int)(" << tensor->name << "->dimensions[" << op->mode << "]);\n";
      } else {
        taco_iassert(op->property == TensorProperty::Indices);
        tp      = "int*";
        auto nm = op->index;
  ...
  string CodeGen::packTensorProperty(string varname, Expr tnsr, TensorProperty property, int mode, int index) {
    stringstream ret;
    ret << "  ";
  ...
  string CodeGen::printDecls(map<Expr, string, ExprCompare> varMap, vector<Expr> inputs, vector<Expr> outputs) {
    stringstream          ret;
    ...
    sort(sortedProps.begin(), sortedProps.end(), [&](const GetProperty *a, const GetProperty *b) -> bool {
      ...
      if (a_it == outputs.end())
        a_pos += distance(inputs.begin(), find(inputs.begin(), inputs.end(), a->tensor));
      if (b_it == outputs.end())
        b_pos += distance(inputs.begin(), find(inputs.begin(), inputs.end(), b->tensor));
  ...
  string CodeGen::printPack(map<tuple<Expr, TensorProperty, int, int>, string> outputProperties, vector<Expr> outputs) {
    stringstream                                  ret;
    ...
    sort(sortedProps.begin(), sortedProps.end(), [&](const tuple<Expr, TensorProperty, int, int> &a, const tuple<Expr, TensorProperty, int, int> &b) -> bool {
      // first, use a total order of outputs,inputs
      ...
      for (auto prop: sortedProps) {
        auto var = get<0>(prop).as<Var>();
          if (var->is_parameter) {
            ...
            ret << packTensorProperty(outputProperties[prop], get<0>(prop), get<1>(prop), get<2>(prop), get<3>(prop));
          }
      }
    return ret.str();
  }
  ...
  void CodeGen::resetUniqueNameCounters() {
    uniqueNameCounters = {{"auto", 0},     {"break", 0},           {"case", 0},     {"char", 0},     {"const", 0},
                          {"continue", 0}, {"default", 0},         {"do", 0},       {"double", 0},   {"else", 0},
                          {"enum", 0},     {"extern", 0},          {"float", 0},    {"for", 0},      {"goto", 0},
                          {"if", 0},       {"inline", 0},          {"int", 0},      {"long", 0},     {"register", 0},
                          {"restrict", 0}, {restrictKeyword(), 0}, {"return", 0},   {"short", 0},    {"signed", 0},
                          {"sizeof", 0},   {"static", 0},          {"struct", 0},   {"switch", 0},   {"typedef", 0},
                          {"union", 0},    {"unsigned", 0},        {"void", 0},     {"volatile", 0}, {"while", 0},
                          {"bool", 0},     {"complex", 0},         {"imaginary", 0}};
  }
  ...
  string CodeGen::printFuncName(const Function *func, std::map<Expr, std::string, ExprCompare> inputMap, std::map<Expr, std::string, ExprCompare> outputMap) {
    stringstream ret;
    ...
    bool unfoldOutput = false;
      for (size_t i = 0; i < func->outputs.size(); i++) {
        ...
        taco_iassert(var) << "Unable to convert output " << func->outputs[i] << " to Var";
          if (var->is_parameter) {
            unfoldOutput = true;
            break;
        }
    ...
    bool unfoldInput = false;
      for (size_t i = 0; i < func->inputs.size(); i++) {
        ...
        taco_iassert(var) << "Unable to convert output " << func->inputs[i] << " to Var";
          if (var->is_parameter) {
            unfoldInput = true;
            break;
        }
  ...
  void CodeGen::printYield(const Yield *op, vector<Expr> localVars, map<Expr, string, ExprCompare> varMap, int labelCount, string funcName) {
    int stride = 0;
    ...
    int offset = 0;
      for (auto &coord: op->coords) {
        doIndent();
        stream << "*(" << printType(coord.type(), true) << ")(" << coordsName << " + " << stride << " * " << bufSizeName;
          if (offset > 0) {
            stream << " + " << offset;
        }
        stream << ") = ";
    ...
    doIndent();
    stream << "if (++" << bufSizeName << " == " << bufCapacityCopyName << ") {" << endl;
    indent++;
      for (auto &localVar: localVars) {
        doIndent();
        const string varName = varMap[localVar];
