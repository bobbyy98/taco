...
namespace taco { namespace ir {
  ...
  std::uniform_int_distribution<int> Module::randint = std::uniform_int_distribution<int>(0, chars.length() - 1);
  ...
  void Module::compileToSource(string path, string prefix) {
      if (!moduleFromUserSource) {
        ...
        taco_tassert(target.arch == Target::C99) << "Only C99 codegen supported currently";
        std::shared_ptr<CodeGen> sourcegen = CodeGen::init_default(source, CodeGen::ImplementationGen);
        std::shared_ptr<CodeGen> headergen = CodeGen::init_default(header, CodeGen::HeaderGen);
  ...
  string Module::compile() {
    ...
    string cmd = cc + " " + cflags + " " + prefix + file_ending + " " + shims_file + " " + "-o " + fullpath + " -lm";
  ...
  int Module::callFuncPackedRaw(std::string name, void **args) {
    ...
    static_assert(sizeof(void *) == sizeof(fnptr_t), "Unable to cast dlsym() returned void pointer to function pointer");
