...
namespace taco { namespace ir {
  ...
  protected:
    ...
    virtual void visit(const GetProperty *op) {
        if (!util::contains(inputTensors, op->tensor) && !util::contains(outputTensors, op->tensor)) {
  ...
  CodeGen_C::CodeGen_C(std::ostream &dest, OutputKind outputKind, bool simplify): CodeGen(dest, false, simplify, C), out(dest), outputKind(outputKind) {
  }
  ...
  void CodeGen_C::generateShim(const Stmt &func, stringstream &ret) {
    ...
    const auto returnType = funcPtr->getReturnType();
      ...
      for (auto output: funcPtr->outputs) {
        ...
        auto cast_type = var->is_tensor ? "taco_tensor_t*" : printCType(var->type, var->is_ptr);
