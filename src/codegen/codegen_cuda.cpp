...
namespace taco { namespace ir {
  ...
  namespace {
    ...
    const string gpuAssertMacro =
        ...
        "  return lowerBound;\n"
        "}\n"
        "__global__ void taco_binarySearchBeforeBlock(int * __restrict__ array, int * __restrict__ results, int arrayStart, int arrayEnd, int "
        "values_per_block, int num_blocks) {\n"
        ...
        "}\n"
        "\n"
        "__host__ int * taco_binarySearchBeforeBlockLaunch(int * __restrict__ array, int * __restrict__ results, int arrayStart, int arrayEnd, int "
        "values_per_block, int block_size, int num_blocks){\n"
        ...
        "  return results;\n"
        "}\n"
        "__global__ void taco_binarySearchIndirectBeforeBlock(int * __restrict__ array, int * __restrict__ results, int arrayStart, int arrayEnd, int * "
        "__restrict__ targets, int num_blocks) {\n"
        "  int thread = threadIdx.x;\n"
        "  int block = blockIdx.x;\n"
        "  int idx = block * blockDim.x + thread;\n"
        "  if (idx >= num_blocks+1) {\n"
        ...
        "}\n"
        "\n"
        "__host__ int * taco_binarySearchIndirectBeforeBlockLaunch(int * __restrict__ array, int * __restrict__ results, int arrayStart, int arrayEnd, int * "
        "__restrict__ targets, int block_size, int num_blocks){\n"
        "  int num_search_blocks = (num_blocks + 1 + block_size - 1) / block_size;\n"
  ...
  protected:
    ...
    virtual void visit(const GetProperty *op) {
        if (varMap.count(op) == 0 && !inBlock) {
          auto key = tuple<Expr, TensorProperty, int, int>(op->tensor, op->property, (size_t) op->mode, (size_t) op->index);
    ...
    DeviceFunctionCollector(vector<Expr> inputs, vector<Expr> outputs, CodeGen_CUDA *codeGen): codeGen(codeGen) {
      inDeviceFunction = false;
        for (auto v: inputs) {
          auto var = v.as<Var>();
          taco_iassert(var) << "Inputs must be vars in codegen";
          taco_iassert(scopeMap.count(var) == 0) << "Duplicate input found in codegen";
          scopeMap[var] = var->name;
        }
        for (auto v: outputs) {
          auto var = v.as<Var>();
          taco_iassert(var) << "Outputs must be vars in codegen";
          taco_iassert(scopeMap.count(var) == 0) << "Duplicate output found in codegen";
  ...
  protected:
    ...
    virtual void visit(const Var *op) {
        ...
        } else if (scopeMap.count(op) == 1 && inDeviceFunction && currentParameterSet.count(op) == 0 && (threadIDVars.empty() || op != threadIDVars.back().second) &&
                   (blockIDVars.empty() || op != blockIDVars.back().second) && (warpIDVars.empty() || op != warpIDVars.back().second) &&
  ...
  string CodeGen_CUDA::printDeviceFuncName(const vector<pair<string, Expr>> currentParameters, int index) {
    ...
    string delimiter = "";
      for (size_t i = 0; i < currentParameters.size(); i++) {
        ...
        taco_iassert(var) << "Unable to convert output " << currentParameters[i].second << " to Var";
  ...
  void CodeGen_CUDA::printThreadIDVariable(pair<string, Expr> threadIDVar, Expr start, Expr increment, Expr numThreads) {
    ...
    taco_iassert(var) << "Unable to convert output " << threadIDVar.second << " to Var";
  ...
  void CodeGen_CUDA::printWarpIDVariable(pair<string, Expr> warpIDVar, Expr start, Expr increment, Expr warpSize) {
    ...
    taco_iassert(var) << "Unable to convert output " << warpIDVar.second << " to Var";
  ...
  void CodeGen_CUDA::printBlockIDVariable(pair<string, Expr> blockIDVar, Expr start, Expr increment) {
    ...
    taco_iassert(var) << "Unable to convert output " << blockIDVar.second << " to Var";
  ...
  void CodeGen_CUDA::printDeviceFuncCall(const vector<pair<string, Expr>> currentParameters, Expr blockSize, int index, Expr gridSize) {
    ...
    string delimiter = "";
      for (size_t i = 0; i < currentParameters.size(); i++) {
        taco_iassert(currentParameters[i].second.as<Var>()) << "Unable to convert output " << currentParameters[i].second << " to Var";
        string varName = currentParameters[i].first;
  ...
  CodeGen_CUDA::CodeGen_CUDA(std::ostream &dest, OutputKind outputKind): CodeGen(dest, false, false, CUDA), out(dest), outputKind(outputKind) {
  }
  ...
  void CodeGen_CUDA::visit(const Function *func) {
    ...
    out << printDecls(varFinder.varDecls, func->inputs, func->outputs) << endl;
      ...
      if (emittingCoroutine) {
        out << printContextDeclAndInit(varMap, localVars, numYields, func->name) << endl;
    }
  ...
  void CodeGen_CUDA::visit(const For *op) {
    ...
    auto lit = op->increment.as<Literal>();
      if (lit != nullptr && ((lit->type.isInt() && lit->equalsScalar(1)) || (lit->type.isUInt() && lit->equalsScalar(1)))) {
        stream << "++";
      } else {
        stream << " += ";
        op->increment.accept(this);
  ...
  void CodeGen_CUDA::visit(const GetProperty *op) {
    taco_iassert(varMap.count(op) > 0) << "Property of " << op->tensor << " not found in varMap";
    out << varMap[op];
  }
  ...
  void CodeGen_CUDA::visit(const Sqrt *op) {
    taco_tassert(op->type.isFloat() && op->type.getNumBits() == 64) << "Codegen doesn't currently support non-double sqrt";
    stream << "sqrt(";
    op->a.accept(this);
    stream << ")";
  }
  ...
  void CodeGen_CUDA::generateShim(const Stmt &func, stringstream &ret) {
    ...
    const auto returnType = funcPtr->getReturnType();
      ...
      for (auto output: funcPtr->outputs) {
        ...
        auto cast_type = var->is_tensor ? "taco_tensor_t*" : printCUDAType(var->type, var->is_ptr);
