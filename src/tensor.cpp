...
namespace taco {
  ...
  TensorBase::TensorBase(Datatype ctype): TensorBase(util::uniqueName('A'), ctype) {
  }

  TensorBase::TensorBase(std::string name, Datatype ctype): TensorBase(name, ctype, {}, Format(), Literal::zero(ctype)) {
  }

  TensorBase::TensorBase(Datatype ctype, vector<int> dimensions, ModeFormat modeType, Literal fill):
      TensorBase(util::uniqueName('A'), ctype, dimensions, std::vector<ModeFormatPack>(dimensions.size(), modeType), fill) {
  }
  ...
  TensorBase::TensorBase(std::string name, Datatype ctype, std::vector<int> dimensions, ModeFormat modeType, Literal fill):
      TensorBase(name, ctype, dimensions, std::vector<ModeFormatPack>(dimensions.size(), modeType), fill) {
  }

  TensorBase::TensorBase(Datatype ctype, std::vector<int> dimensions, Literal fill): TensorBase(ctype, dimensions, ModeFormat::compressed, fill) {
  }
  ...
  TensorBase::TensorBase(string name, Datatype ctype, vector<int> dimensions, Format format, Literal fill) {
    ...
    taco_uassert((size_t) format.getOrder() == dimensions.size()) << "The number of format mode types (" << format.getOrder() << ") "
  ...
  void TensorBase::reserve(size_t numCoordinates) {
    size_t newSize = content->coordinateBuffer->size() + numCoordinates * content->coordinateSize;
  ...
  static size_t unpackTensorData(const taco_tensor_t &tensorData, const TensorBase &tensor) {
  ...
  void TensorBase::pack() {
    ...
    const auto helperFuncs = getHelperFunctions(getFormat(), getComponentType(), dimensions);
      ...
      if (order == 0) {
        ...
        taco_tensor_t *bufferStorage =
            init_taco_tensor_t(1, csize, (int32_t *) bufferDim.data(), (int32_t *) bufferModeOrdering.data(), (taco_mode_t *) bufferModeType.data(), fillPtr);
    ...
    taco_tensor_t *          bufferStorage =
        init_taco_tensor_t(order, csize, (int32_t *) dimensions.data(), (int32_t *) permutation.data(), (taco_mode_t *) bufferModeTypes.data(), fillPtr);
    std::vector<int> pos         = {0, (int) numCoordinates};
    bufferStorage->indices[0][0] = (uint8_t *) pos.data();
      for (int i = 0; i < order; ++i) {
  ...
  struct AccessTensorNode: public AccessNode {
    AccessTensorNode(TensorBase tensor, const std::vector<IndexVar> &indices): AccessNode(tensor.getTensorVar(), indices, {}, false), tensor(tensor) {
    }

    AccessTensorNode(TensorBase tensor, const std::vector<std::shared_ptr<IndexVarInterface>> &indices): AccessNode(tensor.getTensorVar()), tensor(tensor) {
        ...
        for (size_t i = 0; i < indices.size(); i++) {
          ...
          IndexVarInterface::match(
              ...
              [&](std::shared_ptr<WindowedIndexVar> wvar) {
                ...
                taco_uassert(hi <= tensor.getDimension(i)) << "slice upper bound must be <= tensor dimension (" << tensor.getDimension(i) << ")";
  ...
  std::shared_ptr<Module> TensorBase::getComputeKernel(const IndexStmt stmt) {
    ...
    const auto computeKernelsReverse = util::ReverseConstIterable<TensorBase::KernelsCache>(computeKernels);
  ...
  void TensorBase::cacheComputeKernel(const IndexStmt stmt, const std::shared_ptr<Module> kernel) {
    computeKernelsMutex.lock();
          ...
          if (new_tensorvar.getId() != -1) {
            auto found = tensorvars.find(new_name);
              if (found != tensorvars.end() && found->second.getId() != -1) {
                ...
                taco_uassert(new_tensorvar.getId() == found_tensorvar.getId()) << error::compile_tensor_name_collision << " " << new_name;
              } else {
  ...
  void TensorBase::compile(taco::IndexStmt stmt, bool assembleWhileCompute) {
    ...
    stmtToCompile               = scalarPromote(stmtToCompile);
      ...
      if (!std::getenv("CACHE_KERNELS") || std::string(std::getenv("CACHE_KERNELS")) != "0") {
  ...
  void TensorBase::compileSource(std::string source) {
    taco_iassert(getAssignment().getRhs().defined()) << error::compile_without_expr;
  ...
  std::shared_ptr<ir::Module> TensorBase::getHelperFunctions(const Format &format, Datatype ctype, const std::vector<int> &dimensions) {
    helperFunctionsMutex.lock();
    const auto helperFunctionsReverse = util::ReverseConstIterable<TensorBase::HelperFuncsCache>(helperFunctions);
      for (const auto &helperFuncs: helperFunctionsReverse) {
          if (std::get<0>(helperFuncs) == format && std::get<1>(helperFuncs) == ctype && std::get<2>(helperFuncs) == dimensions) {
  ...
  ostream &operator<<(ostream &os, const TensorBase &tensor) {
    ...
    os << tensor.getName() << " (" << util::join(dimensionStrings, "x") << ") " << tensor.getFormat() << ":" << std::endl;
        ...
        os << "(" << util::join(ptr, ptr + tensor.getOrder()) << "): ";
          switch (tensor.getComponentType().getKind()) {
            ...
            case Datatype::Float64:
              ...
              break;
            case Datatype::Complex64:
              os << ((std::complex<float> *) (ptr + tensor.getOrder()))[0] << std::endl;
              break;
            case Datatype::Complex128:
              os << ((std::complex<double> *) (ptr + tensor.getOrder()))[0] << std::endl;
              break;
            case Datatype::Undefined:
              taco_ierror;
              break;
  ...
  TensorBase read(string filename, FileType filetype, ModeFormat modetype, bool pack) {
  ...
  TensorBase read(istream &stream, FileType filetype, ModeFormat modetype, bool pack) {
  ...
  void write(string filename, const TensorBase &tensor) {
    string extension = getExtension(filename);
      if (extension == "ttx") {
        ...
        taco_iassert(tensor.getOrder() == 2) << "The .mtx format only supports matrices. Consider using the .ttx format "
                                                "instead";
