...
namespace taco { namespace ir {
  ...
  void IRRewriter::visit(const IfThenElse *op) {
    ...
    Stmt otherwise = rewrite(op->otherwise);
      if (cond == op->cond && then == op->then && otherwise == op->otherwise) {
        ...
        stmt = otherwise.defined() ? IfThenElse::make(cond, then, otherwise) : IfThenElse::make(cond, then);
      }
  }
  ...
  void IRRewriter::visit(const For *op) {
    ...
    Stmt contents  = rewrite(op->contents);
      if (var == op->var && start == op->start && end == op->end && increment == op->increment && contents == op->contents) {
        stmt = op;
      } else {
        stmt = For::make(var, start, end, increment, contents, op->kind, op->parallel_unit, op->unrollFactor, op->vec_width);
      }
  }
