...
namespace taco {
  ...
  struct WorkspaceRewriter: ir::IRRewriter {
    WorkspaceRewriter(std::vector<TensorVar> whereTemps, std::map<TensorVar, std::vector<ir::Expr>> temporarySizeMap):
        whereTemps(whereTemps),
            ...
            for (auto &temp: whereTemps) {
              string gpName = temp.getName() + to_string(op->mode + 1) + "_dimension";
                ...
                if (temp.defined() && gpName == op->name && (temporarySizeMap.find(temp) != temporarySizeMap.end())) {
                  ...
                  taco_iassert((int) tempExprList.size() > op->mode) << "Cannot rewrite workspace (" << op->tensor << ") Dimension GetProperty due to mode (" << op->mode
                                                                     << ") not in expression map (size = " << tempExprList.size() << ")";
