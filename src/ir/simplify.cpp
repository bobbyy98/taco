    ...
    void visit(const Cast *op) {
      Expr a = rewrite(op->a);
        ...
        if (isa<Literal>(a) && to<Literal>(a)->type.isBool()) {
          const auto lit = TypedComponentVal(op->type, to<Literal>(a)->getValue<bool>());
  ...
  ir::Stmt simplify(const ir::Stmt &stmt) {
    ...
    struct FindLoopDependentVars: public IRVisitor {
      ...
      void visit(const Assign *op) {
          if (util::contains(defLevel, op->lhs) && defLevel.at(op->lhs) < loopLevel) {
    ...
    struct Simplifier: public ExpressionSimplifier {
      ...
      Simplifier(const std::set<Expr> &loopDependentVars): loopDependentVars(loopDependentVars) {
      }
      ...
      void visit(const VarDecl *decl) {
        ...
        declarations.insert({decl->var, stmt});
          if (decl->var.type().isInt() && isa<Var>(rhs) && !util::contains(loopDependentVars, decl->var)) {
      ...
      void visit(const Assign *assign) {
        ...
        stmt     = (lhs == assign->lhs && rhs == assign->rhs) ? assign : Assign::make(lhs, rhs, assign->use_atomics);
    ...
    struct RemoveRedundantStmts: public IRRewriter {
      ...
      RemoveRedundantStmts(const std::set<Stmt> &necessaryDecls): necessaryDecls(necessaryDecls) {
      }
    ...
    simplifiedStmt = RemoveRedundantStmts(copyPropagation.necessaryDecls).rewrite(simplifiedStmt);
    ...
    struct RemoveRedundantLoops: public IRRewriter {
      ...
      //     y = lit;
      void visit(const For *op) {
        Expr loopCount = simplify(Div::make(Sub::make(op->end, op->start), op->increment));
                                ...
                                const auto add = to<Add>(assign->rhs);
                                  if (add->a == assign->lhs && isa<Literal>(add->b)) {
                                    const auto inc = to<Literal>(add->b);
                                      if ((inc->type.isInt() && inc->equalsScalar(1)) || (inc->type.isUInt() && inc->equalsScalar(1))) {
