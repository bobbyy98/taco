    ...
    protected:
      ...
      void verify_operand_types_consistent(C op) {
          if (op->a.type() != op->b.type()) {
            messages << "Node: " << (Expr) op << " has operands with different types (" << op->a.type() << " vs. " << op->b.type() << ")\n";
        }
      }
        ...
        auto tp = op->type;
          if (op->a.type() != tp) {
            messages << "Node: " << (Expr) op << " has left operand with different "
                     << "type from result (expected " << tp << " but got " << op->a.type() << ")\n";
        }
          if (op->b.type() != tp) {
            messages << "Node: " << (Expr) op << " has right operand with different "
                     << "type from result (expected " << tp << " but got " << op->b.type() << ")\n";
        }
      }
      ...
      void visit(const IfThenElse *op) {
          if (!op->cond.type().isBool()) {
            messages << "Node: " << (Stmt) op << " has condition " << (Expr) op->cond << " that is not boolean\n";
        }
        op->cond.accept(this);
        op->then.accept(this);
      ...
      void visit(const Case *op) {
          for (auto &c: op->clauses) {
              if (!c.first.type().isBool()) {
                messages << "Node: " << (Stmt) op << " has condition " << (Expr) c.first << " that is not boolean\n";
            }
            c.first.accept(this);
      ...
      void visit(const Switch *op) {
          ...
          if (!op->controlExpr.type().isInt() && !op->controlExpr.type().isUInt()) {
            messages << "Node: " << (Stmt) op << " has a control statement with non-integer type\n";
        }
        op->controlExpr.accept(this);

          for (auto &x: op->cases) {
              if (!(x.first.as<Literal>()) || (!x.first.type().isInt() && !x.first.type().isUInt())) {
                messages << "Node: " << (Stmt) op << " has clauses with non-integer literal switch values\n";
            }
      ...
      void visit(const Load *op) {
          if (op->type != op->arr.type()) {
            messages << "Node: " << (Expr) op << " has type that differs from the target array (" << op->type << " vs. " << op->arr.type() << ")\n";
        }
        op->arr.accept(this);
        op->loc.accept(this);
      }
      ...
      void visit(const Store *op) {
        auto tp = op->arr.type();
          if (tp != op->data.type()) {
            messages << "Node: " << (Stmt) op << " is storing data of different type from array (" << op->data.type() << " vs. " << tp << ")\n";
        }
        op->arr.accept(this);
        op->data.accept(this);
        op->loc.accept(this);
        ...
        auto loopVarType = op->var.type();
          if (op->start.type() != loopVarType) {
            messages << "Node: " << (Stmt) op << " has start with different type from "
                     << "loop variable (expected " << loopVarType << " but got " << op->start.type() << ")\n";
        }
          if (op->end.type() != loopVarType) {
            messages << "Node: " << (Stmt) op << " has end with different type from "
                     << "loop variable (expected " << loopVarType << " but got " << op->end.type() << ")\n";
        }
          if (op->increment.type() != loopVarType) {
            messages << "Node: " << (Stmt) op << " has increment with different type "
                     << "from loop variable (expected " << loopVarType << " but got " << op->increment.type() << ")\n";
        }
          if (!op->var.as<Var>()) {
      ...
      void visit(const Assign *op) {
          if (!op->lhs.as<Var>() && !op->lhs.as<GetProperty>()) {
            messages << "Node: " << (Stmt) op << " must have Var or GetProperty node on lhs\n";
        }
