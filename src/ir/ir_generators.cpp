...
namespace taco { namespace ir {
  ...
  Stmt compoundStore(Expr a, Expr i, Expr val, bool use_atomics, ParallelUnit atomic_parallel_unit) {
    Expr add = (val.type().getKind() == Datatype::Bool) ? Or::make(Load::make(a, i), val) : Add::make(Load::make(a, i), val);
  ...
  Stmt compoundAssign(Expr a, Expr val, bool use_atomics, ParallelUnit atomic_parallel_unit) {
    Expr add = (val.type().getKind() == Datatype::Bool) ? Or::make(a, val) : Add::make(a, val);
