...
namespace taco { namespace ir {
  ...
  template <class T>
  static inline void acceptJoin(IRPrinter *printer, ostream &stream, vector<T> nodes, string sep) {
      if (nodes.size() > 0) {
  ...
  IRPrinter::IRPrinter(ostream &s, bool color, bool simplify): stream(s), indent(0), color(color), simplify(simplify) {
  }
  ...
  void IRPrinter::visit(const Literal *op) {
      ...
      switch (op->type.getKind()) {
        ...
        case Datatype::Float32:
          stream << ((op->getValue<float>() != 0.0) ? util::toString(op->getValue<float>()) : "0.0");
          break;
        case Datatype::Float64:
          stream << ((op->getValue<double>() != 0.0) ? util::toString(op->getValue<double>()) : "0.0");
          break;
  ...
  void IRPrinter::visit(const For *op) {
    doIndent();
    stream << keywordString("for") << " (" << keywordString(util::toString(op->var.type())) << " ";
    op->var.accept(this);
    stream << " = ";
  ...
  void IRPrinter::visit(const Function *op) {
    ...
    if (op->outputs.size() > 0 && op->inputs.size())
      stream << ", ";
    if (op->inputs.size() > 0)
      stream << "Tensor ";
  ...
  bool IRPrinter::needsParentheses(Precedence precedence) {
    ...
    return (precedence >= parentPrecedence || (precedence == Precedence::LAND && parentPrecedence == Precedence::LOR));
  }
