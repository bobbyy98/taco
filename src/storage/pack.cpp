...
#define PACK_NEXT_LEVEL(cend)                                                                                                               \
  {                                                                                                                                         \
      if (i + 1 == modeTypes.size()) {                                                                                                      \
        ...
        valuesIndex = packTensor(dimensions, coords, vals, fill, cbegin, (cend), modeTypes, i + 1, indices, values, dataType, valuesIndex); \
      }                                                                                                                                     \
  }

  /// Count unique entries (assumes the values are sorted)
  static TypedIndexVector getUniqueEntries(TypedIndexVector v, size_t startIndex, size_t endIndex) {
  ...
  /// [0,2] index arrays.
  static int packTensor(const vector<int> &dimensions, const vector<TypedIndexVector> &coords, char *vals, const void *fill, size_t begin, size_t end, const vector<ModeFormat> &modeTypes, size_t i, std::vector<std::vector<TypedIndexVector>> *indices, char *values, Datatype dataType, int valuesIndex) {
  ...
  TensorStorage pack(Datatype componentType, const std::vector<int> &dimensions, const Format &format, const std::vector<TypedIndexVector> &coordinates, const void *values, const Literal &fill) {
    ...
    int         actual_size = packTensor(dimensions, coordinates, (char *) values, fillData, 0, numCoordinates, format.getModeFormats(), 0, &indices, (char *) vals, componentType, 0);
