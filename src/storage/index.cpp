...
namespace taco {
  ...
  Index::Index(const Format &format, const std::vector<ModeIndex> &indices): Index() {
  ...
  ModeIndex Index::getModeIndex(int i) {
    taco_iassert(i < getFormat().getOrder()) << "mode: " << i << endl << "order: " << getFormat().getOrder();
    return content->indices[i];
  }
  ...
  Index makeCSRIndex(size_t numrows, int *rowptr, int *colidx) {
    return Index(CSR, {ModeIndex({makeArray({(int) numrows})}), ModeIndex({makeArray(rowptr, numrows + 1), makeArray(colidx, rowptr[numrows])})});
  }

  Index makeCSRIndex(const vector<int> &rowptr, const vector<int> &colidx) {
    return Index(CSR, {ModeIndex({makeArray({(int) (rowptr.size() - 1)})}), ModeIndex({makeArray(rowptr), makeArray(colidx)})});
  }

  Index makeCSCIndex(size_t numcols, int *colptr, int *rowidx) {
    return Index(CSC, {ModeIndex({makeArray({(int) numcols})}), ModeIndex({makeArray(colptr, numcols + 1), makeArray(rowidx, colptr[numcols])})});
  }

  Index makeCSCIndex(const vector<int> &colptr, const vector<int> &rowidx) {
    return Index(CSC, {ModeIndex({makeArray({(int) (colptr.size() - 1)})}), ModeIndex({makeArray(colptr), makeArray(rowidx)})});
  }
