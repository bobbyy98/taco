...
namespace taco {
  ...
  struct TensorStorage::Content {
    ...
    Content(Datatype componentType, vector<int> dimensions, Format format, Literal fill):
      ...
      taco_uassert(order == format.getOrder()) << "The number of format mode types (" << format.getOrder() << ") "
      ...
      fillValue  = fill;
      tensorData = init_taco_tensor_t(order, componentType.getNumBits(), dimensionsInt32.data(), modeOrdering.data(), modeTypes.data(), fill.getValPtr());
    }
  ...
  TensorStorage::TensorStorage(Datatype componentType, const vector<int> &dimensions, Format format, Literal fillVal):
      ...
      for (int i = 0; i < index.numModeIndices(); i++) {
        const auto &modeIndex = index.getModeIndex(i);
          for (int j = 0; j < modeIndex.numIndexArrays(); j++) {
            ...
            indexSizeInBytes += indexArray.getSize() * indexArray.getType().getNumBytes();
          }
      }
