...
namespace taco {
  ...
  public:
    MergeLatticeBuilder(IndexVar i, Iterators iterators, ProvenanceGraph provGraph, std::set<IndexVar> definedIndexVars, std::map<TensorVar, const AccessNode *> whereTempsToResult = {}):
        ...
        iterators(iterators),
        provGraph(provGraph),
        definedIndexVars(definedIndexVars),
  ...
  private:
    ...
    void visit(const AccessNode *access) {
      ...
      taco_iassert(iterator.hasCoordIter() || iterator.hasPosIter() || iterator.hasLocate()) << "Iterator must support at least one capability";
      ...
      IndexVar posIteratorDescendant;
        ...
        // If this is a position variable then return an iterator over the variable and locate into the access
          ...
          MergePoint point = (!iterator.hasCoordIter() && !iterator.hasPosIter()) ? MergePoint({iterators.modeIterator(i)}, {iterator}, {}) : MergePoint(pointIterators, {}, {});
          lattice = MergeLattice({point});
        }
      ...
      set<set<Iterator>>      regionsToKeep;
        ...
        for (auto &it: regionDefs) {
          ...
          set<Iterator> regionToKeep;
            for (auto idx: region) {
              match(inputs[idx], function<void(const AccessNode *)>([&](const AccessNode *n) {
    ...
    void visit(const CallIntrinsicNode *expr) {
      const auto zeroPreservingArgsSets = expr->func->zeroPreservingArgs(expr->args);
        ...
        for (const auto &zeroPreservingArgsSet: zeroPreservingArgsSets) {
          MergeLattice zeroPreservingLattice({});
            for (const auto zeroPreservingArg: zeroPreservingArgsSet) {
              ...
              zeroPreservingLattice   = unionLattices(zeroPreservingLattice, argLattice);
            }
        ...
        if (!resultIterators.empty()) {
          vector<MergePoint> points;
            for (auto &point: lattice.points()) {
              points.push_back(MergePoint(point.iterators(), point.locators(), vector<Iterator>(resultIterators.begin(), resultIterators.end()), point.isOmitter()));
            }
    ...
    vector<MergePoint> enumerateChildrenPoints(const MergePoint &point, const map<set<Iterator>, MergePoint> &originalPoints, set<set<Iterator>> &seen) {
    ...
    vector<MergePoint> includeMissingProducerPoints(const vector<MergePoint> &points) {
      if (points.empty())
        return points;
    ...
    static vector<MergePoint> includeDimensionIterator(const vector<MergePoint> &points, const Iterator &dimIter) {
      vector<MergePoint> results;
        for (auto &point: points) {
          vector<Iterator> iterators = point.iterators();
            if (!any(iterators, [](Iterator it) {
                  return it.isDimensionIterator();
                })) {
              taco_iassert(point.iterators().size() > 0);
              results.push_back(MergePoint(combine(iterators, {dimIter}), point.locators(), point.results(), point.isOmitter()));
            } else {
            ...
            for (auto &bpoint: sorted_bpoint) {
              ...
              auto bpoint_set = bpoint.tensorRegion();
                ...
                for (auto &it: apoint_set) {
                    if (!std::count(bpoint_set.begin(), bpoint_set.end(), it) && std::count(bpoint_root_set.begin(), bpoint_root_set.end(), it)) {
                      hasIntersection = false;
                  }
                }
                for (auto &it: bpoint_set) {
                    if (!std::count(apoint_set.begin(), apoint_set.end(), it) && std::count(apoint_root_set.begin(), apoint_root_set.end(), it)) {
                      hasIntersection = false;
                  }
                }
              if (hasIntersection)
     ...
     */
    static MergePoint intersectPoints(MergePoint left, MergePoint right, bool locateLeft) {
      vector<Iterator> iterators;
      vector<Iterator> locators;

      tie(iterators, locators) = split((locateLeft ? left : right).iterators(), [](Iterator it) {
        return !it.hasLocate();
      });
      ...
      iterators = (locateLeft) ? combine(iterators, right.iterators()) : combine(left.iterators(), iterators);
      locators  = (locateLeft) ? combine(locators, left.locators(), right.locators()) : combine(left.locators(), locators, right.locators());
    ...
    static bool locateFromLeft(MergeLattice left, MergeLattice right) {
        ...
        if (any(right.iterators(), [](Iterator it) {
              return it.isDimensionIterator();
            })) {
          return false;
      }
        if (any(left.iterators(), [](Iterator it) {
              return it.isDimensionIterator();
            })) {
          return true;
      }
      ...
      size_t leftNumLocates  = count(left.iterators(), [](Iterator it) {
        return it.hasLocate();
      });
      size_t rightNumLocates = count(right.iterators(), [](Iterator it) {
        return it.hasLocate();
      });
      return (leftNumLocates > rightNumLocates);
    }

    static vector<MergePoint> insertDimensionIteratorIfNotOrdered(const vector<MergePoint> &points) {
      vector<MergePoint> results;
        for (auto &point: points) {
          vector<Iterator> iterators = point.iterators();
            if (any(iterators,
                ...
                !any(iterators, [](Iterator it) {
                  return it.isDimensionIterator();
                })) {
              taco_iassert(point.iterators().size() > 0);
              Iterator dimension(iterators[0].getIndexVar());
              results.push_back(MergePoint(combine(iterators, {dimension}), point.locators(), point.results(), point.isOmitter()));
            } else {
              results.push_back(point);
            }
        }
    ...
    static vector<MergePoint> moveLocateSubsetIteratorsToLocateSet(const vector<MergePoint> &points) {
      vector<Iterator> full = filter(points[0].iterators(), [](Iterator it) {
        return it.isFull();
      });
      ...
      vector<MergePoint> result;
        for (auto &point: points) {
          vector<Iterator> locators;
          vector<Iterator> iterators;
          tie(locators, iterators) = split(point.iterators(), [&firstFull](Iterator it) {
            return it.hasLocate() && it != firstFull;
          });
          result.push_back(MergePoint(iterators, combine(point.locators(), locators), point.results(), point.isOmitter()));
        }
      return result;
    }

    static vector<Iterator> deduplicateDimensionIterators(const vector<Iterator> &iterators) {
    ...
    static vector<Iterator> removeDimensionIterators(const vector<Iterator> &iterators) {
    ...
    static vector<MergePoint> flipPoints(const vector<MergePoint> &points) {
    ...
    static set<set<Iterator>> getProducerOrOmitterRegions(const std::vector<MergePoint> &points, bool getOmitters) {
    ...
    static vector<MergePoint> correctPointTypes(const vector<MergePoint> &left, const vector<MergePoint> &right, const vector<MergePoint> &points, bool preserveOmit) {
      vector<MergePoint> result;
    ...
    static vector<MergePoint> correctPointTypesAfterIntersect(const vector<MergePoint> &left, const vector<MergePoint> &right, const vector<MergePoint> &points) {
      return correctPointTypes(left, right, points, false);
    }

    static vector<MergePoint> correctPointTypesAfterUnion(const vector<MergePoint> &left, const vector<MergePoint> &right, const vector<MergePoint> &points) {
      return correctPointTypes(left, right, points, true);
    }

    static vector<MergePoint> removeDuplicatedTensorRegions(const vector<MergePoint> &points, bool preserveOmitters) {
    ...
    static vector<MergePoint> removeUnnecessaryOmitterPoints(const vector<MergePoint> &points) {
      ...
      set<set<Iterator>> removed;
        ...
        for (const auto &point: points) {
          ...
          MergeLattice subLattice = l.subLattice(point);
            ...
            if (util::all(subLattice.points(), [](const MergePoint p) {
                  return p.isOmitter();
                })) {
  ...
  // class MergeLattice
  MergeLattice::MergeLattice(vector<MergePoint> points, set<set<Iterator>> regionsToKeep): points_(points), regionsToKeep(regionsToKeep) {
  }
  ...
  std::vector<MergePoint> MergeLattice::removePointsThatLackFullIterators(const std::vector<MergePoint> &points) {
    vector<MergePoint> result;
    vector<Iterator>   fullIterators = filter(points[0].iterators(), [](Iterator it) {
      return it.isFull();
    });
      for (auto &point: points) {
  ...
  std::vector<MergePoint> MergeLattice::removePointsWithIdenticalIterators(const std::vector<MergePoint> &points) {
    vector<MergePoint> result;
    set<set<Iterator>> producerIteratorSets;
      for (auto &point: points) {
        set<Iterator> iteratorSet(point.iterators().begin(), point.iterators().end());
  ...
  bool MergeLattice::needExplicitZeroChecks() {
      if (util::any(points(), [](const MergePoint &mp) {
            return mp.isOmitter();
          })) {
        return true;
    }
    ...
    sort(lpIterators.begin(), lpIterators.end());
      for (auto &lq: this->points()) {
        ...
        sort(lqIterators.begin(), lqIterators.end());
          if (includes(lpIterators.begin(), lpIterators.end(), lqIterators.begin(), lqIterators.end())) {
  ...
  set<Iterator> MergeLattice::exhausted(MergePoint point) {
    ...
    modeIterators                  = filter(modeIterators, [](const Iterator &it) {
  ...
  bool MergeLattice::exact() const {
    ...
    set<Iterator> uniquelyMergedIterators;
      ...
      if (util::any(points(), [](const MergePoint &m) {
            return m.isOmitter();
          })) {
        return false;
    }
      ...
      for (auto &point: this->points()) {
          if (all(point.iterators(), [](Iterator it) {
                return it.isFull();
              })) {
            return true;
        }
  ...
  bool MergeLattice::anyModeIteratorIsLeaf() const {
    ...
    return util::any(latticeIters, [](const Iterator &it) {
      return it.isModeIterator() && it.isLeaf();
    });
  }
    ...
    const size_t     levelOfPoint = pointRegion.size();
      ...
      for (const auto &mp: points()) {
          if ((mp.tensorRegion().size() > levelOfPoint) && mp.isOmitter()) {
            ...
            set_difference(parentRegion.begin(), parentRegion.end(), pointRegion.begin(), pointRegion.end(), back_inserter(parentItersToOmit));
  ...
  MergePoint::MergePoint(const vector<Iterator> &iterators, const vector<Iterator> &locators, const vector<Iterator> &results, bool omitPoint):
      content_(new Content) {
    taco_uassert(iterators.size() <= 1 || all(iterators,
                                              [](Iterator it) {
                                                return it.hasLocate() || it.isOrdered();
                                              }))
  ...
  std::vector<Iterator> MergePoint::rangers() const {
      ...
      if (any(iterators(), [](Iterator iterator) {
  ...
  std::vector<Iterator> MergePoint::mergers() const {
    ...
    size_t numNotFull = count(iterators(), [](Iterator iterator) {
      return !iterator.isFull() && iterator.isDimensionIterator();
    });
  ...
  const std::set<Iterator> MergePoint::tensorRegion() const {
    std::vector<Iterator> iterators = filter(content_->iterators, [](Iterator it) {
      return !it.isDimensionIterator();
    });
  ...
  ostream &operator<<(ostream &os, const MergePoint &mlp) {
    ...
    if (mlp.iterators().size() > 0)
      os << " ";
    os << "|";
    os << " ";
    os << util::join(mlp.locators(), ", ");
    if (mlp.locators().size() > 0)
      os << " ";
    os << "|";
    if (mlp.results().size() > 0)
      os << " ";
  ...
  bool operator==(const MergePoint &a, const MergePoint &b) {
    if (!compare(a.iterators(), b.iterators()))
      return false;
    if (!compare(a.locators(), b.locators()))
      return false;
    if (!compare(a.results(), b.results()))
      return false;
    if ((a.isOmitter() != b.isOmitter()))
      return false;
    return true;
  }
