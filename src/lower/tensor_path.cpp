...
namespace taco {
  ...
  TensorPath::TensorPath(const vector<IndexVar> &vars, const Access &access): content(new TensorPath::Content(vars, access)) {
  }
  ...
  TensorPathStep::TensorPathStep(const TensorPath &path, int step): path(path), step(step) {
    taco_iassert(step >= 0);
    taco_iassert(step < (int) path.getVariables().size()) << "step: " << step << std::endl << "path: " << path;
  }
  ...
  bool operator<(const TensorPathStep &l, const TensorPathStep &r) {
    return (l.getPath() != r.getPath()) ? l.getPath() < r.getPath() : l.getStep() < r.getStep();
  }

  std::ostream &operator<<(std::ostream &os, const TensorPathStep &step) {
    if (!step.getPath().defined())
      return os << "Step()";
    return os << step.getPath().getAccess().getTensorVar().getName() << (step.getStep() >= 0 ? to_string(step.getStep()) : "root");
  }
