...
namespace taco {
  ...
  static void createCapacityVars(const map<TensorVar, Expr> &tensorVars, map<Expr, Expr> *capacityVars) {
  ...
  static void createReducedValueVars(const vector<Access> &inputAccesses, map<Access, Expr> *reducedValueVars) {
  ...
  static void getDependentTensors(IndexStmt stmt, std::set<TensorVar> &tensors) {
    std::set<TensorVar> prev;
      do {
        prev = tensors;
        match(stmt, function<void(const AssignmentNode *, Matcher *)>([&](const AssignmentNode *n, Matcher *m) {
  ...
  static std::set<Expr> hasSparseInserts(IndexStmt stmt, Iterators iterators, ProvenanceGraph provGraph) {
    std::set<Expr>     ret;
    std::set<IndexVar> definedIndexVars;

    match(stmt, function<void(const ForallNode *, Matcher *)>([&](const ForallNode *op, Matcher *ctx) {
            definedIndexVars.insert(op->indexVar);
            const auto lattice = MergeLattice::make(Forall(op), iterators, provGraph, definedIndexVars);
              if (any(lattice.iterators(),
                      [](Iterator it) {
                        return !it.isFull() && !it.isDimensionIterator();
                      }) ||
                  any(lattice.points()[0].locators(), [](Iterator it) {
                    return !it.isFull();
                  })) {
  ...
  Stmt LowererImplImperative::lower(IndexStmt stmt, string name, bool assemble, bool compute, bool pack, bool unpack) {
    ...
    assembledByUngroupedInsert = util::toSet(getAssembledByUngroupedInsertion(stmt));
    ...
    argumentsIR.insert(argumentsIR.begin(), indexSetArgs.begin(), indexSetArgs.end());
      ...
      for (auto &temp: temporaries) {
        ir::Expr irVar = ir::Var::make(temp.getName(), temp.getType().getDataType(), true, true);
        ...
        match(stmt, function<void(const AssignmentNode *, Matcher *)>([&](const AssignmentNode *n, Matcher *m) {
                m->match(n->rhs);
                  if (!dimension.defined()) {
                    ...
                    int  loc   = (int) distance(ivars.begin(), find(ivars.begin(), ivars.end(), indexVar));
              ...
              function<void(const AccessNode *)>([&](const AccessNode *n) {
                auto indexVars = n->indexVars;
                  if (util::contains(indexVars, indexVar)) {
                    int loc = (int) distance(indexVars.begin(), find(indexVars.begin(), indexVars.end(), indexVar));
    ...
    // Create function
    return Function::make(name, resultsIR, argumentsIR, Block::blanks(Block::make(header), initializeResults, body, finalizeResults, Block::make(footer)));
  }
  ...
  Stmt LowererImplImperative::lowerAssignment(Assignment assignment) {
    ...
    Expr rhs;
      ...
      if (isScalar(result.getType())) {
          if (needComputeAssign) {
              if (!assignment.getOperator().defined()) {
                ...
                bool useAtomics = markAssignsAtomicDepth > 0 && !util::contains(whereTemps, result);
                  if (isa<taco::Add>(assignment.getOperator())) {
                    ...
                    Stmt assign   = Assign::make(var, assignOp, useAtomics, atomicParallelUnit);
              ...
              if (!assignment.getOperator().defined()) {
                computeStmt = Store::make(values, loc, rhs);
                  ...
                  if (isa<taco::Add>(assignment.getOperator())) {
                    ...
                    computeStmt   = Store::make(values, loc, assignOp, markAssignsAtomicDepth > 0 && !util::contains(whereTemps, result), atomicParallelUnit);
      ...
      if (util::contains(guardedTemps, result) && result.getOrder() == 0) {
        ...
        Stmt setGuard = Assign::make(guard, true, markAssignsAtomicDepth > 0, atomicParallelUnit);
    ...
    bool temporaryWithSparseAcceleration = util::contains(tempToIndexList, result);
      ...
      if (temporaryWithSparseAcceleration) {
        taco_iassert(markAssignsAtomicDepth == 0) << "Parallel assembly of sparse accelerator not supported";
  ...
  pair<vector<Iterator>, vector<Iterator>> LowererImplImperative::splitAppenderAndInserters(const vector<Iterator> &results) {
    ...
    vector<Iterator> inserters;
      ...
      for (auto &result: results) {
        ...
        taco_iassert(result.hasAppend() || result.hasInsert()) << "Results must support append or insert";
  ...
  Stmt LowererImplImperative::lowerForall(Forall forall) {
    ...
    bool forallNeedsUnderivedGuards = !hasExactBound && emitUnderivedGuards;
      if (!ignoreVectorize && forallNeedsUnderivedGuards && (forall.getParallelUnit() == ParallelUnit::CPUVector || forall.getUnrollFactor() > 0)) {
    ...
    vector<Stmt> recoverySteps;
      for (const IndexVar &varToRecover: provGraph.newlyRecoverableParents(forall.getIndexVar(), definedIndexVars)) {
          // place pos guard
          if (forallNeedsUnderivedGuards && provGraph.isCoordVariable(varToRecover) && provGraph.getChildren(varToRecover).size() == 1 &&
        ...
        std::vector<ir::Expr> iterBounds = provGraph.deriveIterBounds(varToRecover, definedIndexVarsOrdered, underivedBounds, indexVarToExprMap, iterators);
          if (forallNeedsUnderivedGuards && underivedBounds.count(varToRecover) && !provGraph.hasPosDescendant(varToRecover)) {
            ...
            std::vector<ir::Expr> iterBoundsInner = provGraph.deriveIterBounds(forall.getIndexVar(), definedIndexVarsOrdered, underivedBounds, indexVarToExprMap, iterators);
              for (auto &c: children) {
                  if (provGraph.hasExactBound(c) && provGraph.derivationPath(varToRecover, c).size() == 2) {
                    const auto iterBoundsUnderivedChild = provGraph.deriveIterBounds(c, definedIndexVarsOrdered, underivedBounds, indexVarToExprMap, iterators);
              ...
              if (!hasDirectDivBound) {
                Stmt guard = IfThenElse::make(Gte::make(indexVarToExprMap[varToRecover], underivedBounds[varToRecover][1]), Continue::make());
                recoverySteps.push_back(guard);
            }
        }
    ...
    Stmt preInitValues = initResultArrays(forall.getIndexVar(), resultAccesses, reducedAccesses);
    ...
    Stmt loops;
      ...
      if (caseLattice.iterators().size() == 1 && caseLattice.iterators()[0].isUnique()) {
        ...
        bool canAccelWithSparseIteration = provGraph.isFullyDerived(iterator.getIndexVar()) && iterator.isDimensionIterator() && locators.size() == 1;
          ...
          if (!isWhereProducer && hasPosDescendant && underivedAncestors.size() > 1 && provGraph.isPosVariable(iterator.getIndexVar()) &&
              posDescendant == forall.getIndexVar()) {
            loops = lowerForallFusedPosition(forall, iterator, locators, inserters, appenders, caseLattice, reducedAccesses, recoveryStmt);
          ...
          // Emit dimension coordinate iteration loop
            ...
            loops = lowerForallDimension(forall, point.locators(), inserters, appenders, caseLattice, reducedAccesses, recoveryStmt);
          }
          // Emit position iteration loop
          else if (iterator.hasPosIter()) {
            loops = lowerForallPosition(forall, iterator, locators, inserters, appenders, caseLattice, reducedAccesses, recoveryStmt);
          }
      ...
      // Emit general loops to merge multiple iterators
        ...
        loops = lowerMergeLattice(caseLattice, underivedAncestors[0], forall.getStmt(), reducedAccesses, forall.getMergeStrategy());
      }
    ...
    return Block::blanks(preInitValues, temporaryValuesInitFree[0], loops, temporaryValuesInitFree[1]);
  }
  ...
  Stmt LowererImplImperative::lowerForallCloned(Forall forall) {
              ...
              if (!definedIndexVars.count(child)) {
                ...
                maxChildValues[child] = childBounds[1];
                  ...
                  for (const IndexVar &varToRecover: provGraph.newlyRecoverableParents(child, definedForGuard)) {
                    Expr recoveredValue    = provGraph.recoverVariable(varToRecover, definedIndexVarsOrdered, underivedBounds, minChildValues, iterators);
                    Expr maxRecoveredValue = provGraph.recoverVariable(varToRecover, definedIndexVarsOrdered, underivedBounds, maxChildValues, iterators);
    ...
    taco_uassert(guardCondition.defined()) << "Unable to vectorize or unroll loop over unbound variable " << forall.getIndexVar();
      ...
      for (int i = (int) underivedAncestors.size() - 2; i >= 0; i--) {
        ...
        Iterator rootIterator = posIterator;
          ...
          if (parallelUnitIndexVars.count(ParallelUnit::GPUBlock)) {
                    ...
                    if (child != parallelUnitIndexVars[ParallelUnit::GPUBlock]) {
                      zeroedChildValues[child] = 0;
                        ...
                        for (const IndexVar &varToRecover: provGraph.newlyRecoverableParents(child, zeroDefinedIndexVars)) {
                          Expr recoveredValue = provGraph.recoverVariable(varToRecover, definedIndexVarsOrdered, underivedBounds, zeroedChildValues, iterators);
                          taco_iassert(indexVarToExprMap.count(varToRecover));
            ...
            header.push_back(Allocate::make(blockStarts_temporary, ir::Add::make(parallelUnitSizes[ParallelUnit::GPUBlock], 1)));
            ...
            Expr blockSize;
              if (parallelUnitSizes.count(ParallelUnit::GPUThread)) {
                ...
                match(forall.getStmt(), function<void(const ForallNode *, Matcher *)>([&](const ForallNode *n, Matcher *m) {
                        if (n->parallel_unit == ParallelUnit::GPUThread) {
                          vector<Expr> bounds = provGraph.deriveIterBounds(forall.getIndexVar(), definedIndexVarsMatched, underivedBounds, indexVarToExprMap, iterators);
            ...
            taco_iassert(blockSize.defined());
              ...
              if (i == (int) underivedAncestors.size() - 2) {
                ...
                header.push_back(ir::Assign::make(blockStarts_temporary, ir::Call::make("taco_binarySearchBeforeBlockLaunch", args, getCoordinateVar(underived).type())));
              } else {
                std::vector<Expr> args = {
                    posIteratorLevel.getMode().getModePack().getArray(0),     // array
                    blockStarts_temporary,                                    // results
                ...
                header.push_back(
                    ir::Assign::make(blockStarts_temporary, ir::Call::make("taco_binarySearchIndirectBeforeBlockLaunch", args, getCoordinateVar(underived).type())));
              }
            searchForUnderivedStart.push_back(VarDecl::make(posIteratorLevel.getBeginVar(), ir::Load::make(blockStarts_temporary, indexVarToExprMap[parallelUnitIndexVars[ParallelUnit::GPUBlock]])));
            searchForUnderivedStart.push_back(VarDecl::make(posIteratorLevel.getEndVar(), ir::Load::make(blockStarts_temporary, ir::Add::make(indexVarToExprMap[parallelUnitIndexVars[ParallelUnit::GPUBlock]], 1))));
            ...
            minDefinedIndexVars.erase(forall.getIndexVar());
              ...
              for (IndexVar child: provGraph.getFullyDerivedDescendants(posIterator.getIndexVar())) {
                  if (!minDefinedIndexVars.count(child)) {
                    std::vector<ir::Expr> childBounds = provGraph.deriveIterBounds(child, definedIndexVarsOrdered, underivedBounds, indexVarToExprMap, iterators);
                    minChildValues[child]             = childBounds[0];
        ...
        searchForUnderivedStart.push_back(
            ir::VarDecl::make(posVarUnknown, ir::Call::make("taco_binarySearchBefore", binarySearchArgs, getCoordinateVar(underivedAncestors[i]).type())));
        Stmt locateCoordVar;
  ...
  Stmt LowererImplImperative::lowerForallDimension(Forall forall, vector<Iterator> locators, vector<Iterator> inserters, vector<Iterator> appenders, MergeLattice caseLattice, set<Access> reducedAccesses, ir::Stmt recoveryStmt) {
    ...
    LoopKind kind = LoopKind::Serial;
      ...
      } else if (forall.getParallelUnit() != ParallelUnit::NotParallel && forall.getOutputRaceStrategy() != OutputRaceStrategy::ParallelReduction && !ignoreVectorize) {
        kind = LoopKind::Runtime;
    }

    return Block::blanks(For::make(coordinate, bounds[0], bounds[1], 1, body, kind, ignoreVectorize ? ParallelUnit::NotParallel : forall.getParallelUnit(),
                                   ignoreVectorize ? 0 : forall.getUnrollFactor()),
                         posAppend);
  }

  Stmt LowererImplImperative::lowerForallDenseAcceleration(Forall forall, vector<Iterator> locators, vector<Iterator> inserters, vector<Iterator> appenders, MergeLattice caseLattice, set<Access> reducedAccesses, ir::Stmt recoveryStmt) {
    ...
    return Block::blanks(
        For::make(loopVar, 0, indexListSize, 1, body, kind, ignoreVectorize ? ParallelUnit::NotParallel : forall.getParallelUnit(), ignoreVectorize ? 0 : forall.getUnrollFactor()), posAppend);
  }

  Stmt LowererImplImperative::lowerForallCoordinate(Forall forall, Iterator iterator, vector<Iterator> locators, vector<Iterator> inserters, vector<Iterator> appenders, MergeLattice caseLattice, set<Access> reducedAccesses, ir::Stmt recoveryStmt) {
    taco_not_supported_yet;
    return Stmt();
  }

  Stmt LowererImplImperative::lowerForallPosition(Forall forall, Iterator iterator, vector<Iterator> locators, vector<Iterator> inserters, vector<Iterator> appenders, MergeLattice caseLattice, set<Access> reducedAccesses, ir::Stmt recoveryStmt) {
    Expr coordinate        = getCoordinateVar(forall.getIndexVar());
    Stmt declareCoordinate = Stmt();
    Stmt strideGuard       = Stmt();
    Stmt boundsGuard       = Stmt();
      if (provGraph.isCoordVariable(forall.getIndexVar())) {
        Expr coordinateArray = iterator.posAccess(iterator.getPosVar(), coordinates(iterator)).getResults()[0];
    ...
    Stmt loop = Block::make(strideGuard, declareCoordinate, boundsGuard, body);
      if (iterator.isBranchless() && iterator.isCompact() && (iterator.getParent().isRoot() || iterator.getParent().isUnique())) {
        ...
        loop = For::make(iterator.getPosVar(), startBound, endBound, 1, loop, kind, ignoreVectorize ? ParallelUnit::NotParallel : forall.getParallelUnit(),
                         ignoreVectorize ? 0 : forall.getUnrollFactor());
      }
  ...
  Stmt LowererImplImperative::lowerForallFusedPosition(Forall forall, Iterator iterator, vector<Iterator> locators, vector<Iterator> inserters, vector<Iterator> appenders, MergeLattice caseLattice, set<Access> reducedAccesses, ir::Stmt recoveryStmt) {
    Expr coordinate        = getCoordinateVar(forall.getIndexVar());
    Stmt declareCoordinate = Stmt();
      if (provGraph.isCoordVariable(forall.getIndexVar())) {
        Expr coordinateArray = iterator.posAccess(iterator.getPosVar(), coordinates(iterator)).getResults()[0];
...
#if TACO_ASSERTS
        bool hasIteratorAncestor = provGraph.getPosIteratorAncestor(iterator.getIndexVar(), &posIteratorVar);
        taco_iassert(hasIteratorAncestor);
#else  /* !TACO_ASSERTS */
        provGraph.getPosIteratorAncestor(iterator.getIndexVar(), &posIteratorVar);
#endif /* TACO_ASSERTS */
    ...
    return Block::blanks(boundsCompute,
                         Block::make(Block::make(searchForUnderivedStart),
                                     For::make(indexVarToExprMap[iterator.getIndexVar()], startBound, endBound, 1, Block::make(declareCoordinate, body), kind,
                                               ignoreVectorize ? ParallelUnit::NotParallel : forall.getParallelUnit(), ignoreVectorize ? 0 : forall.getUnrollFactor())),
                         posAppend);
  }

  Stmt LowererImplImperative::lowerMergeLattice(MergeLattice caseLattice, IndexVar coordinateVar, IndexStmt statement, const std::set<Access> &reducedAccesses, MergeStrategy mergestrategy) {
    ...
    vector<Iterator> appenders  = filter(loopLattice.results(), [](Iterator it) {
      return it.hasAppend();
    });
    ...
    auto modeIteratorsNonMergers = filter(loopLattice.points()[0].iterators(), [mergers](Iterator it) {
    ...
    return Block::blanks(iteratorVarInits, mergeLoops, appendPositions);
  }

  Stmt LowererImplImperative::lowerMergePoint(MergeLattice pointLattice, ir::Expr coordinate, IndexVar coordinateVar, IndexStmt statement, const std::set<Access> &reducedAccesses, bool resolvedCoordDeclared, MergeStrategy mergeStrategy) {
    ...
    std::vector<ir::Stmt> indexSetStmts;
      for (auto &iter: filter(iterators, [](Iterator it) {
             return it.hasIndexSet();
           })) {
        ...
        auto shiftDown =
            ir::Block::make(ir::Assign::make(iter.getCoordVar(), indexSetIter.getPosVar()), ir::Assign::make(indexSetIter.getCoordVar(), indexSetIter.getPosVar()));
        ...
        vector<Expr> iterGallopArgs     = {iter.getMode().getModePack().getArray(1), ivar, iterBounds[1], setMatch};
        ...
        vector<Expr> indexGallopArgs    = {indexSetIter.getMode().getModePack().getArray(1), indexVar, indexIterBounds[1], setMatch};
        auto         incr               = ir::Block::make(ir::Assign::make(ivar, ir::Call::make("taco_gallop", iterGallopArgs, ivar.type())),
                                    ir::Assign::make(indexVar, ir::Call::make("taco_gallop", indexGallopArgs, indexVar.type())), ir::Continue::make());
    ...
    // Deduplication loops
    auto dupIters           = filter(iterators, [](Iterator it) {
      return !it.isUnique() && it.hasPosIter();
    });
    bool alwaysReduce       = (mergers.size() == 1 && mergers[0].hasPosIter());
    Stmt deduplicationLoops = reduceDuplicateCoordinates(coordinate, dupIters, alwaysReduce);

    // One case for each child lattice point lp
    Stmt caseStmts = lowerMergeCases(coordinate, coordinateVar, statement, pointLattice, reducedAccesses, mergeStrategy);
    ...
    return While::make(checkThatNoneAreExhausted(rangers), Block::make(loadPosIterCoordinates, ir::Block::make(indexSetStmts), resolvedCoordinate, loadLocatorPosVars, deduplicationLoops, caseStmts, incIteratorVarStmts));
  }
      ...
      if (mergers.size() == 1) {
        Iterator merger = mergers[0];
          if (merger.hasPosIter()) {
            ...
            ModeFunction posAccess     = merger.posAccess(merger.getPosVar(), coordinates(merger));
            ...
            return Block::make(posAccess.compute(), windowVarDecl, stride, resolution, guard);
  ...
  Stmt LowererImplImperative::lowerMergeCases(ir::Expr coordinate, IndexVar coordinateVar, IndexStmt stmt, MergeLattice caseLattice, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy) {
    vector<Stmt> result;
    ...
    tie(appenders, inserters) = splitAppenderAndInserters(loopLattice.results());
      ...
      if (loopLattice.iterators().size() == 1) {
        ...
        Stmt body = lowerForallBody(coordinate, stmt, {}, inserters, appenders, loopLattice, reducedAccesses, mergeStrategy);
        result.push_back(body);
      } else if (loopLattice.exact() && isa<Assignment>(stmt) && returnsTrue(stmt.as<Assignment>().getRhs())) {
        // All cases require the same computation, so no conditionals needed
        taco_iassert(!loopLattice.points()[0].isOmitter());
        Stmt body = lowerForallBody(coordinate, stmt, {}, inserters, appenders, MergeLattice({loopLattice.points()[0]}), reducedAccesses, mergeStrategy);
        result.push_back(body);
        ...
        vector<pair<Expr, Stmt>> cases;
          for (MergePoint point: loopLattice.points()) {
            ...
            coordComparisons = filter(coordComparisons, [](const Expr &e) {
              return e.defined();
            });

            // Construct case body
            IndexStmt zeroedStmt = zero(stmt, getExhaustedAccesses(point, loopLattice));
            Stmt      body       = lowerForallBody(coordinate, zeroedStmt, {}, inserters, appenders, MergeLattice({point}), reducedAccesses, mergeStrategy);
              if (coordComparisons.empty()) {
                Stmt body = lowerForallBody(coordinate, stmt, {}, inserters, appenders, MergeLattice({point}), reducedAccesses, mergeStrategy);
                result.push_back(body);
                break;
            }
  ...
  std::vector<Iterator> LowererImplImperative::getModeIterators(const std::vector<Iterator> &iters) {
    ...
    return filter(iters, [](const Iterator &it) {
      return it.isModeIterator();
    });
  }

  std::vector<ir::Stmt> LowererImplImperative::constructInnerLoopCasePreamble(ir::Expr coordinate, IndexVar coordinateVar, MergeLattice lattice, map<Iterator, Expr> &iteratorToConditionMap) {
    vector<Stmt> result;
  ...
  vector<Stmt> LowererImplImperative::lowerCasesFromMap(map<Iterator, Expr> iteratorToCondition, ir::Expr coordinate, IndexStmt stmt, const MergeLattice &lattice, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy) {
    vector<Iterator> appenders;
    vector<Iterator> inserters;
    ...
    vector<pair<Expr, Stmt>> cases;
      for (MergePoint point: lattice.points()) {
        ...
        function<Expr(const Iterator &it)> getNegatedComparison = [&](const Iterator &it) {
          return ir::Neg::make(iteratorToCondition.at(it));
        };
    ...
    inputs                  = getModeIterators(inputs);
      ...
      if (!lattice.exact() &&
          ...
          hasNoForAlls(stmt) && any(inputs, [](Iterator it) {
            return it.isFull();
          })) {
        ...
        vector<Stmt> stmts;
          for (auto &it: inserters) {
              if (it.isFull()) {
                ...
                Stmt      initialization = lowerForallBody(coordinate, initStmt, {}, inserters, appenders, MergeLattice({}), reducedAccesses, mergeStrategy);
  ...
  Stmt LowererImplImperative::lowerMergeCasesWithExplicitZeroChecks(ir::Expr coordinate, IndexVar coordinateVar, IndexStmt stmt, MergeLattice lattice, const std::set<Access> &reducedAccesses, MergeStrategy mergeStrategy) {
    vector<Stmt> result;
      if (lattice.points().size() == 1 && lattice.iterators().size() == 1) {
        ...
        Stmt body = lowerForallBody(coordinate, stmt, {}, inserters, appenders, lattice, reducedAccesses, mergeStrategy);
        result.push_back(body);
      } else if (lattice.exact() && isa<Assignment>(stmt) && returnsTrue(stmt.as<Assignment>().getRhs())) {
        // All cases require the same computation, so no conditionals needed
        vector<Iterator> appenders;
        vector<Iterator> inserters;
        tie(appenders, inserters) = splitAppenderAndInserters(lattice.results());
        taco_iassert(!lattice.points()[0].isOmitter());
        Stmt body = lowerForallBody(coordinate, stmt, {}, inserters, appenders, MergeLattice({lattice.points()[0]}), reducedAccesses, mergeStrategy);
        result.push_back(body);
  ...
  Stmt LowererImplImperative::lowerForallBody(Expr coordinate, IndexStmt stmt, vector<Iterator> locators, vector<Iterator> inserters, vector<Iterator> appenders, MergeLattice caseLattice, const set<Access> &reducedAccesses, MergeStrategy mergeStrategy) {
    // Inserter positions
    ...
    return Block::make(initVals, declInserterPosVars, declLocatorPosVars, body, appendCoords, incr);
  }
  ...
  Expr LowererImplImperative::getTemporarySize(Where where) {
    ...
    std::vector<IndexVar> indexVars       = temporaryAccess.getIndexVars();
      ...
      if (util::all(indexVars, [&](const IndexVar &var) {
            return provGraph.isUnderived(var);
          })) {
  ...
  vector<Stmt> LowererImplImperative::codeToInitializeDenseAcceleratorArrays(Where where, bool parallel) {
    ...
    const Expr alreadySetArr = ir::Var::make(bitGuardName, bitGuardType, true, false);
    ...
    const Expr        indexListArr  = ir::Var::make(indexListName, indexListType, true, false);
  ...
  std::pair<bool, bool> LowererImplImperative::canAccelerateDenseTemp(Where where) {
    ...
    auto                  it         = std::find_if(resultVars.begin(), resultVars.end(), [&](const auto &resultVar) {
      return resultVar == tempVar[0] || provGraph.isDerivedFrom(tempVar[0], resultVar);
    });
  ...
  vector<Stmt> LowererImplImperative::codeToInitializeLocalTemporaryParallel(Where where, ParallelUnit parallelUnit) {
    ...
    Expr values;
      if (util::contains(needCompute, temporary) && needComputeValues(where, temporary)) {
        // Declare local temporary workspace array
        values          = ir::Var::make(temporary.getName(), temporary.getType().getDataType(), true, false);
  ...
  vector<Stmt> LowererImplImperative::codeToInitializeTemporaryParallel(Where where, ParallelUnit parallelUnit) {
    ...
    Expr values;
      if (util::contains(needCompute, temporary) && needComputeValues(where, temporary)) {
        values       = ir::Var::make(temporaryAll.getName(), temporaryAll.getType().getDataType(), true, false);
  ...
  Stmt LowererImplImperative::lowerWhere(Where where) {
    ...
    std::tie(accelerateDenseWorkSpace, sortAccelerator) = canAccelerateDenseTemp(where);
    ...
    match(where.getConsumer(), std::function<void(const AssignmentNode *)>([&](const AssignmentNode *op) {
    ...
    Stmt consumer = lower(where.getConsumer());
      ...
      if (util::contains(needCompute, temporary) && !isScalar(temporary.getType()) && !accelerateDenseWorkSpace) {
        ...
        Expr p              = Var::make("p" + temporary.getName(), Int());
        Expr values         = ir::Var::make(temporary.getName(), temporary.getType().getDataType(), true, false);
        Expr size           = getTemporarySize(where);
  ...
  Stmt LowererImplImperative::lowerAssemble(Assemble assemble) {
      ...
      if (generateAssembleCode() && assemble.getQueries().defined()) {
        ...
        const auto        queryAccesses = getResultAccesses(assemble.getQueries()).first;
          for (const auto &queryAccess: queryAccesses) {
            ...
            Expr       values      = ir::Var::make(queryResult.getName(), queryResult.getType().getDataType(), true, false);
            ...
            taco_iassert(util::all(indexVars, [&](const auto &var) {
              return provGraph.isUnderived(var);
            }));
            Expr size = 1;
            ...
            const bool zeroInit = isNonFullyInitialized(getTensorVar(queryResult)) || util::contains(getResultAccesses(assemble.getQueries()).second, queryAccess);
              if (zeroInit) {
                Expr sizeOfElt    = Sizeof::make(queryResult.getType().getDataType());
                Expr callocValues = ir::Call::make("calloc", {size, sizeOfElt}, queryResult.getType().getDataType());
    ...
    vector<Access> resultAccesses;
    set<Access>    reducedAccesses;
    std::tie(resultAccesses, reducedAccesses) = getResultAccesses(assemble.getCompute());
    ...
    std::vector<Stmt> initAssembleStmts;
      for (const auto &resultAccess: resultAccesses) {
              ...
              if (generateAssembleCode()) {
                ...
                std::vector<AttrQueryResult> queryResults;
                  for (const auto &queryResultVar: queryResultVars) {
                    queryResults.emplace_back(getTensorVar(queryResultVar), getValuesArray(queryResultVar));
                  }
                  ...
                  if (resultIterator.hasSeqInsertEdge()) {
                    ...
                    Stmt insertEdgeLoop = resultIterator.getSeqInsertEdge(resultIterator.getParent().getPosVar(), coords, queryResults);
                    auto locateCoords   = coords;
                      for (auto iter = resultIterator.getParent(); !iter.isRoot(); iter = iter.getParent()) {
                          if (iter.hasLocate()) {
                            Expr dim = GetProperty::make(resultTensorVar, TensorProperty::Dimension, resultModeOrdering[iter.getMode().getLevel() - 1]);
                            Expr pos = iter.getPosVar();
                            Stmt initPos   = VarDecl::make(pos, iter.locate(locateCoords)[0]);
                            insertEdgeLoop = For::make(coords.back(), 0, dim, 1, Block::make(initPos, insertEdgeLoop));
                          } else {
                            taco_not_supported_yet;
                          }
        ...
        const bool zeroInit  = isNonFullyInitialized(resultTensorVar) || util::contains(reducedAccesses, resultAccess);
          if (generateAssembleCode()) {
              if (zeroInit && generateComputeCode()) {
                const auto type         = resultTensor.getType().getDataType();
                Expr       sizeOfElt    = Sizeof::make(type);
                Expr       callocValues = ir::Call::make("calloc", {prevSize, sizeOfElt}, type);
    ...
    return Block::blanks(queries, initAssemble, compute, finalizeAssemble, freeQueryResults);
  }
  ...
  Expr LowererImplImperative::lowerAdd(Add add) {
    ...
    return (add.getDataType().getKind() == Datatype::Bool) ? ir::Or::make(a, b) : ir::Add::make(a, b);
  }
  ...
  Expr LowererImplImperative::lowerMul(Mul mul) {
    ...
    return (mul.getDataType().getKind() == Datatype::Bool) ? ir::And::make(a, b) : ir::Mul::make(a, b);
  }
  ...
  ir::Expr LowererImplImperative::getValuesArray(TensorVar var) const {
    return (util::contains(temporaryArrays, var)) ? temporaryArrays.at(var).values : GetProperty::make(getTensorVar(var), TensorProperty::Values);
  }
  ...
  set<Access> LowererImplImperative::getExhaustedAccesses(MergePoint point, MergeLattice lattice) const {
  ...
  vector<Expr> LowererImplImperative::coordinates(vector<Iterator> iterators) {
    taco_iassert(all(iterators, [](Iterator iter) {
      return iter.defined();
    }));
    vector<Expr> result;
  ...
  Stmt LowererImplImperative::initResultArrays(vector<Access> writes, set<Access> reducedAccesses) {
    std::vector<Stmt> result;
      for (auto &write: writes) {
          if (write.getTensorVar().getOrder() == 0 || isAssembledByUngroupedInsertion(write.getTensorVar())) {
            continue;
        }
        ...
        Expr parentSize = 1;
          if (generateAssembleCode()) {
              for (const auto &iterator: iterators) {
                ...
                initArrays.push_back(init);
                  ...
                  // position variable)
                  if (iterator.hasAppend() && (iterator.isLeaf() || !iterator.getChild().isBranchless())) {
              ...
              if (generateComputeCode()) {
                ...
                Expr allocSize   = isValue(parentSize, 0) ? DEFAULT_ALLOC_SIZE : parentSize;
                initArrays.push_back(VarDecl::make(capacityVar, allocSize));
                initArrays.push_back(Allocate::make(valuesArr, capacityVar, false /* is_realloc */, Expr() /* old_elements */, clearValuesAllocation));
            }
          ...
          if (generateComputeCode() && iterators.back().hasInsert() && !isValue(parentSize, 0) && (isNonFullyInitialized(tensor) || util::contains(reducedAccesses, write))) {
      ...
      for (auto &write: writes) {
        ...
        Expr parentSize = 1;
          ...
          if (!generateComputeCode()) {
            ...
            Expr tensor    = getTensorVar(write.getTensorVar());
            Expr valuesArr = GetProperty::make(tensor, TensorProperty::Values);
            result.push_back(Allocate::make(valuesArr, parentSize, false /* is_realloc */, Expr() /* old_elements */, clearValuesAllocation));
        }
      }
    return result.empty() ? Stmt() : Block::blanks(result);
  }
  ...
  Stmt LowererImplImperative::defineScalarVariable(TensorVar var, bool zero) {
    ...
    Expr     init                = (zero) ? ir::Literal::zero(type) : Load::make(GetProperty::make(tensorVars.at(var), TensorProperty::Values));
    tensorVars.find(var)->second = varValueIR;
    return VarDecl::make(varValueIR, init);
  }

  static vector<Iterator> getIteratorsFrom(IndexVar var, const vector<Iterator> &iterators) {
    vector<Iterator> result;
    bool             found = false;
      for (Iterator iterator: iterators) {
        if (var == iterator.getIndexVar())
          found = true;
          if (found) {
  ...
  Stmt LowererImplImperative::initResultArrays(IndexVar var, vector<Access> writes, set<Access> reducedAccesses) {
      if (!generateAssembleCode()) {
        return Stmt();
    }
            ...
            Iterator initIterator;
              ...
              if (initIterator.defined()) {
                ...
                result.push_back(atLeastDoubleSizeIfFull(values, capacityVar, size));
                  ...
                  if (isNonFullyInitialized(tensor) || util::contains(reducedAccesses, write)) {
  ...
  Stmt LowererImplImperative::resizeAndInitValues(const std::vector<Iterator> &appenders, const std::set<Access> &reducedAccesses) {
      if (!generateComputeCode()) {
        return Stmt();
    }
  ...
  Stmt LowererImplImperative::initValues(Expr tensor, Expr initVal, Expr begin, Expr size) {
    ...
    LoopKind parallel = (isa<ir::Literal>(size) && to<ir::Literal>(size)->getIntValue() < (1 << 10)) ? LoopKind::Serial : LoopKind::Static_Chunked;
      if (should_use_CUDA_codegen() && util::contains(parallelUnitSizes, ParallelUnit::GPUBlock)) {
        return ir::VarDecl::make(
            ir::Var::make("status", Int()),
  ...
  Stmt LowererImplImperative::declLocatePosVars(vector<Iterator> locators) {
    vector<Stmt> result;
      for (Iterator &locator: locators) {
        ...
        bool doLocate = true;
          for (Iterator ancestorIterator = locator.getParent(); !ancestorIterator.isRoot() && ancestorIterator.hasLocate(); ancestorIterator = ancestorIterator.getParent()) {
          ...
          if (doLocate) {
            Iterator locateIterator = locator;
              ...
              do {
                ...
                Stmt declarePosVar = VarDecl::make(locateIterator.getPosVar(), locate.getResults()[0]);
            ...
            } while (locateIterator.hasLocate() && accessibleIterators.contains(locateIterator));
        }
      }
    return result.empty() ? Stmt() : Block::make(result);
  }

  Stmt LowererImplImperative::reduceDuplicateCoordinates(Expr coordinate, vector<Iterator> iterators, bool alwaysReduce) {
    vector<Stmt> result;
      for (Iterator &iterator: iterators) {
        ...
        Expr   tensorVals = GetProperty::make(tensorVar, TensorProperty::Values);
          ...
          if (reducedVal.defined()) {
            Expr reducedValInit = alwaysReduce ? Load::make(tensorVals, iterVar) : ir::Literal::zero(reducedVal.type());
        ...
        ModeFunction posAccess = iterator.posAccess(segendVar, coordinates(iterator));
        ...
        result.push_back(While::make(And::make(withinBounds, isDuplicate), Block::make(dedupStmts)));
      }
    return result.empty() ? Stmt() : Block::make(result);
  }
  ...
  Stmt LowererImplImperative::codeToInitializeIteratorVar(Iterator iterator, vector<Iterator> iterators, vector<Iterator> rangers, vector<Iterator> mergers, Expr coordinate, IndexVar coordinateVar) {
    vector<Stmt> result;
    taco_iassert(iterator.hasPosIter() || iterator.hasCoordIter() || iterator.isDimensionIterator());
              ...
              if (any(rangers, [](Iterator it) {
                ...
                Expr binarySearchTarget = provGraph.deriveCoordBounds(definedIndexVarsOrdered, underivedBounds, indexVarToExprMap, this->iterators)[coordinateVar][0];
                  if (binarySearchTarget != underivedBounds[coordinateVar][0]) {
                    ...
                    result.push_back(VarDecl::make(iterVar, ir::Call::make("taco_binarySearchAfter", binarySearchArgs, iterVar.type())));
                  } else {
  ...
  Stmt LowererImplImperative::codeToIncIteratorVars(Expr coordinate, IndexVar coordinateVar, vector<Iterator> iterators, vector<Iterator> mergers, MergeStrategy strategy) {
      if (iterators.size() == 1) {
        ...
        return iterators[0].isLeaf() ? Stmt() : Assign::make(ivar, iterators[0].getSegendVar());
    }
    ...
    auto levelIterators = filter(iterators, [](Iterator it) {
      return !it.isDimensionIterator();
    });
    ...
    auto modeIterators = filter(iterators, [](Iterator it) {
      return it.isDimensionIterator();
    });
  ...
  Stmt LowererImplImperative::codeToLoadCoordinatesFromPosIterators(vector<Iterator> iterators, bool declVars) {
    ...
    Stmt loadPosIterCoordinates;
      if (iterators.size() > 1) {
        ...
        auto         posIters = filter(iterators, [](Iterator it) {
          return it.hasPosIter();
        });
          for (auto &posIter: posIters) {
            taco_tassert(posIter.hasPosIter());
            ModeFunction posAccess = posIter.posAccess(posIter.getPosVar(), coordinates(posIter));
          ...
          if (generateAssembleCode() || isLastAppender(appender)) {
            ...
            Stmt appendCode = Block::make(appendStmts);
              if (appenderChild.defined() && appenderChild.hasAppend()) {
                ...
                Expr shouldAppend = Lt::make(appenderChild.getBeginVar(), appenderChild.getPosVar());
    ...
    vector<Stmt> result;
      if (generateAssembleCode()) {
          for (Iterator appender: appenders) {
              if (appender.isBranchless() || isAssembledByUngroupedInsertion(appender.getTensor())) {
                continue;
            }
  ...
  Expr LowererImplImperative::generateValueLocExpr(Access access) const {
    ...
    Iterator it = getIterators(access).back();
      ...
      if (!provGraph.isUnderived(it.getIndexVar()) && !access.getIndexVars().empty() && util::contains(indexVarToExprMap, access.getIndexVars().front()) && !it.hasPosIter() &&
          access.getIndexVars().front() == it.getIndexVar()) {
  ...
  Expr LowererImplImperative::checkThatNoneAreExhausted(std::vector<Iterator> iterators) {
    ...
    vector<Expr> result;
      for (const auto &iterator: iterators) {
        taco_iassert(!iterator.isFull()) << iterator << " - full iterators do not need to partake in merge loop bounds";
        Expr iterUnexhausted = Lt::make(iterator.getIteratorVar(), iterator.getEndVar());
        result.push_back(iterUnexhausted);
      }

    return (!result.empty()) ? taco::ir::conjunction(result) : Lt::make(iterators[0].getIteratorVar(), iterators[0].getEndVar());
  }

  Expr LowererImplImperative::generateAssembleGuard(IndexExpr expr) {
    class GenerateGuard: public IndexExprVisitorStrict {
    public:
      GenerateGuard(const std::set<TensorVar> &guardedTemps, const std::map<TensorVar, Expr> &tempToGuard):
          guardedTemps(guardedTemps),
          tempToGuard(tempToGuard) {
      }
    ...
    private:
      ...
      void visit(const AccessNode *node) {
        expr = (util::contains(guardedTemps, node->tensorVar) && node->tensorVar.getOrder() == 0) ? tempToGuard.at(node->tensorVar) : true;
      }
  ...
  bool LowererImplImperative::hasStores(Stmt stmt) {
    ...
    struct FindStores: IRVisitor {
      ...
      FindStores(const std::map<TensorVar, Expr> &tensorVars, const std::map<TensorVar, Expr> &tempToBitGuard):
          tensorVars(tensorVars),
          tempToBitGuard(tempToBitGuard) {
      }
  ...
  Stmt LowererImplImperative::upperBoundGuardForWindowPosition(Iterator iterator, ir::Expr access) {
    taco_iassert(iterator.isWindowed());
    return ir::IfThenElse::make(ir::Gte::make(access, ir::Div::make(ir::Sub::make(iterator.getWindowUpperBound(), iterator.getWindowLowerBound()), iterator.getStride())), ir::Break::make());
  }
  ...
  Stmt LowererImplImperative::strideBoundsGuard(Iterator iterator, ir::Expr access, bool incrementPosVar) {
    Stmt cont = ir::Continue::make();
      ...
      if (incrementPosVar) {
        cont = ir::Block::make({ir::Assign::make(iterator.getPosVar(), ir::Add::make(iterator.getPosVar(), ir::Literal::make(1))), cont});
    }
    // The guard makes sure that the coordinate being accessed is along the stride.
    return ir::IfThenElse::make(ir::Neq::make(ir::Rem::make(ir::Sub::make(access, iterator.getWindowLowerBound()), iterator.getStride()), ir::Literal::make(0)), cont);
  }
