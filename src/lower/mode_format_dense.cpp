...
namespace taco {
  ...
  DenseModeFormat::DenseModeFormat(const bool isOrdered, const bool isUnique, const bool isZeroless):
      ModeFormatImpl("dense", true, isOrdered, isUnique, false, true, isZeroless, true, false, false, true, true, false, false, false, true) {
  }
  ...
  ModeFunction DenseModeFormat::locate(ir::Expr parentPos, std::vector<ir::Expr> coords, Mode mode) const {
    Expr pos = ir::Add::make(ir::Mul::make(parentPos, getWidth(mode)), coords.back());
    return ModeFunction(Stmt(), {pos, true});
  }

  Stmt DenseModeFormat::getInsertCoord(Expr p, const std::vector<Expr> &i, Mode mode) const {
    return Stmt();
  }

  Expr DenseModeFormat::getWidth(Mode mode) const {
    return (mode.getSize().isFixed() && mode.getSize().getSize() < 16) ? (int) mode.getSize().getSize() : getSizeArray(mode.getModePack());
  }

  Stmt DenseModeFormat::getInsertInitCoords(Expr pBegin, Expr pEnd, Mode mode) const {
    return Stmt();
  }

  Stmt DenseModeFormat::getInsertInitLevel(Expr szPrev, Expr sz, Mode mode) const {
    return Stmt();
  }

  Stmt DenseModeFormat::getInsertFinalizeLevel(Expr szPrev, Expr sz, Mode mode) const {
    return Stmt();
  }
  ...
  ModeFunction DenseModeFormat::getYieldPos(Expr parentPos, std::vector<Expr> coords, Mode mode) const {
    return locate(parentPos, coords, mode);
  }

  vector<Expr> DenseModeFormat::getArrays(Expr tensor, int mode, int level) const {
