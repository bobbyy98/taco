...
namespace taco {
  ...
  SingletonModeFormat::SingletonModeFormat(): SingletonModeFormat(false, true, true, false, false) {
  }

  SingletonModeFormat::SingletonModeFormat(bool isFull, bool isOrdered, bool isUnique, bool isZeroless, bool isPadded, long long allocSize):
      ModeFormatImpl("singleton", isFull, isOrdered, isUnique, true, true, isZeroless, isPadded, false, true, false, false, true, false, true, true),
      allocSize(allocSize) {
  }

  ModeFormat SingletonModeFormat::copy(std::vector<ModeFormat::Property> properties) const {
    ...
    const auto singletonVariant = std::make_shared<SingletonModeFormat>(isFull, isOrdered, isUnique, isZeroless, isPadded);
    return ModeFormat(singletonVariant);
  }

  ModeFunction SingletonModeFormat::posIterBounds(Expr parentPos, Mode mode) const {
    return ModeFunction(Stmt(), {parentPos, ir::Add::make(parentPos, 1)});
  }

  ModeFunction SingletonModeFormat::posIterAccess(ir::Expr pos, std::vector<ir::Expr> coords, Mode mode) const {
  ...
  Stmt SingletonModeFormat::getAppendCoord(Expr pos, Expr coord, Mode mode) const {
    Expr idxArray = getCoordArray(mode.getModePack());
    Expr stride   = (int) mode.getModePack().getNumModes();
    Expr offset   = (int) mode.getPackLocation();
    Expr loc      = ir::Add::make(ir::Mul::make(pos, stride), offset);
  ...
  Stmt SingletonModeFormat::getAppendInitLevel(Expr parentSize, Expr size, Mode mode) const {
      if (mode.getPackLocation() != (mode.getModePack().getNumModes() - 1)) {
        return Stmt();
    }
  ...
  Stmt SingletonModeFormat::getAppendFinalizeLevel(Expr parentSize, Expr size, Mode mode) const {
    return Stmt();
  }
  ...
  Stmt SingletonModeFormat::getInitCoords(Expr prevSize, std::vector<AttrQueryResult> queries, Mode mode) const {
    Expr crdArray = getCoordArray(mode.getModePack());
    return Allocate::make(crdArray, prevSize, false, Expr(), isPadded);
  }

  ModeFunction SingletonModeFormat::getYieldPos(Expr parentPos, std::vector<Expr> coords, Mode mode) const {
    return ModeFunction(Stmt(), {parentPos});
  }

  Stmt SingletonModeFormat::getInsertCoord(Expr parentPos, Expr pos, std::vector<Expr> coords, Mode mode) const {
    Expr crdArray = getCoordArray(mode.getModePack());
    Expr stride   = (int) mode.getModePack().getNumModes();
    Expr offset   = (int) mode.getPackLocation();
  ...
  std::vector<Expr> SingletonModeFormat::getArrays(Expr tensor, int mode, int level) const {
    std::string arraysName = util::toString(tensor) + std::to_string(level);
    return {Expr(), GetProperty::make(tensor, TensorProperty::Indices, level - 1, 1, arraysName + "_crd")};
  }
  ...
  bool SingletonModeFormat::equals(const ModeFormatImpl &other) const {
    return ModeFormatImpl::equals(other) && (dynamic_cast<const SingletonModeFormat &>(other).allocSize == allocSize);
  }
