...
namespace taco {
  ...
  static tuple<vector<IndexVar>, set<IndexVar>, map<IndexVar, set<IndexVar>>, map<IndexVar, set<IndexVar>>> getGraph(const vector<TensorPath> &tensorPaths) {
    ...
    return tuple<vector<IndexVar>, set<IndexVar>, map<IndexVar, set<IndexVar>>, map<IndexVar, set<IndexVar>>> {vertices, sources, successors, predecessors};
  }
  ...
  IterationForest::IterationForest(const vector<TensorPath> &paths) {
    ...
    sort(levelOrderedVars.begin(), levelOrderedVars.end(), [](pair<int, IndexVar> a, pair<int, IndexVar> b) {
    ...
    map<IndexVar, int> vertexPositions;
      ...
      for (auto &vertex: vertices) {
        ...
        sort(childPositions.begin(), childPositions.end(), [](pair<int, IndexVar> a, pair<int, IndexVar> b) {
          return b.first > a.first;
        });
  ...
  const IndexVar &IterationForest::getParent(const IndexVar &var) const {
    taco_iassert(hasParent(var)) << "Attempting to get the parent of " << var << " which has no no parent";
    return parents.at(var);
  }

  const std::vector<IndexVar> &IterationForest::getChildren(const IndexVar &var) const {
