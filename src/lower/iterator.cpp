...
namespace taco {
  ...
  struct Iterator::Content {
    ...
    struct Window {
      ...
      ir::Expr hi;
      ir::Expr stride;
      Window(ir::Expr _lo, ir::Expr _hi, ir::Expr _stride, ir::Expr _windowVar): windowVar(_windowVar), lo(_lo), hi(_hi), stride(_stride) {};
    };
  ...
  Iterator::Iterator(IndexVar indexVar, Expr tensor, Mode mode, Iterator parent, string name, bool useNameForPos): content(new Content) {
    content->indexVar = indexVar;
  ...
  ModeFunction Iterator::coordAccess(const std::vector<ir::Expr> &coords) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->coordIterAccess(getParent().getPosVar(), coords, getMode());
  }
  ...
  ModeFunction Iterator::posAccess(const ir::Expr &pos, const std::vector<ir::Expr> &coords) const {
    taco_iassert(defined() && content->mode.defined());
  ...
  ModeFunction Iterator::locate(const std::vector<ir::Expr> &coords) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->locate(getParent().getPosVar(), coords, getMode());
  }
  ...
  Stmt Iterator::getInsertInitCoords(const Expr &pBegin, const Expr &pEnd) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getInsertInitCoords(pBegin, pEnd, getMode());
  }

  Stmt Iterator::getInsertInitLevel(const Expr &szPrev, const Expr &sz) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getInsertInitLevel(szPrev, sz, getMode());
  }

  Stmt Iterator::getInsertFinalizeLevel(const Expr &szPrev, const Expr &sz) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getInsertFinalizeLevel(szPrev, sz, getMode());
  }
  ...
  Stmt Iterator::getAppendEdges(const Expr &pPrev, const Expr &pBegin, const Expr &pEnd) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getAppendEdges(pPrev, pBegin, pEnd, getMode());
  }
  ...
  Stmt Iterator::getAppendInitEdges(const Expr &pPrevBegin, const Expr &pPrevEnd) const {
    taco_iassert(defined() && content->mode.defined());
    return content->mode.getModeFormat().impl->getAppendInitEdges(pPrevBegin, pPrevEnd, content->mode);
  }

  Stmt Iterator::getAppendInitLevel(const Expr &szPrev, const Expr &sz) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getAppendInitLevel(szPrev, sz, getMode());
  }

  Stmt Iterator::getAppendFinalizeLevel(const Expr &szPrev, const Expr &sz) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getAppendFinalizeLevel(szPrev, sz, getMode());
  }
  ...
  Stmt Iterator::getSeqInitEdges(const Expr &prevSize, const std::vector<AttrQueryResult> &queries) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getSeqInitEdges(prevSize, queries, getMode());
  }

  Stmt Iterator::getSeqInsertEdge(const Expr &parentPos, const std::vector<Expr> &coords, const std::vector<AttrQueryResult> &queries) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getSeqInsertEdge(parentPos, coords, queries, getMode());
  }

  Stmt Iterator::getInitCoords(const Expr &prevSize, const std::vector<AttrQueryResult> &queries) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getInitCoords(prevSize, queries, getMode());
  }
  ...
  ModeFunction Iterator::getYieldPos(const Expr &parentPos, const std::vector<Expr> &coords) const {
    taco_iassert(defined() && content->mode.defined());
    return getMode().getModeFormat().impl->getYieldPos(parentPos, coords, getMode());
  }

  Stmt Iterator::getInsertCoord(const Expr &parentPos, const Expr &pos, const std::vector<Expr> &coords) const {
    taco_iassert(defined() && content->mode.defined());
  ...
  bool operator==(const Iterator &a, const Iterator &b) {
    ...
    return (a.getIndexVar() == b.getIndexVar() && a.getTensor() == b.getTensor() && a.getParent() == b.getParent());
  }
  ...
  Iterators::Iterators(): content(new Content) {
  }

  Iterators::Iterators(IndexStmt stmt): Iterators(stmt, createIRTensorVars(stmt)) {
  }

  Iterators::Iterators(IndexStmt stmt, const map<TensorVar, Expr> &tensorVars): Iterators() {
    ...
    match(
        stmt, function<void(const ForallNode *, Matcher *)>([&](auto n, auto m) {
    ...
    match(stmt, function<void(const AccessNode *)>([&](auto n) {
  ...
  void Iterators::createAccessIterators(Access access, Format format, Expr tensorIR, ProvenanceGraph provGraph, const map<TensorVar, Expr> &tensorVars) {
    TensorVar tensorConcrete = access.getTensorVar();
    taco_iassert(tensorConcrete.getOrder() == format.getOrder()) << tensorConcrete << ", Format" << format;
    ...
    ModeFormat parentModeType;
      for (ModeFormatPack modeTypePack: format.getModeFormatPacks()) {
        ...
        ModePack modePack(modeTypePack.getModeFormats().size(), modeTypePack.getModeFormats()[0], tensorIR, modeNumber, level);
        ...
        int pos = 0;
          for (auto &modeType: modeTypePack.getModeFormats()) {
            ...
            Mode mode(tensorIR, dim, level, modeType, modePack, pos, parentModeType);
  ...
  Iterator Iterators::levelIterator(ModeAccess modeAccess) const {
    ...
    taco_iassert(util::contains(content->levelIterators, modeAccess)) << "Cannot find " << modeAccess << " in " << util::join(content->levelIterators) << "\n"
    ...
    vector<Iterator> result;
      for (auto &iterator: iterators) {
          if (iterator.hasInsert()) {
            taco_iassert(iterator.hasLocate()) << "Iterators with insert must also have locate";
