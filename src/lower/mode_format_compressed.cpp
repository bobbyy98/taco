...
namespace taco {
  ...
  CompressedModeFormat::CompressedModeFormat(): CompressedModeFormat(false, true, true, false) {
  }

  CompressedModeFormat::CompressedModeFormat(bool isFull, bool isOrdered, bool isUnique, bool isZeroless, long long allocSize):
      ModeFormatImpl("compressed", isFull, isOrdered, isUnique, false, true, isZeroless, false, false, true, false, false, true, true, true, false),
      allocSize(allocSize) {
  }
  ...
  ModeFormat CompressedModeFormat::copy(vector<ModeFormat::Property> properties) const {
    ...
    const auto compressedVariant = std::make_shared<CompressedModeFormat>(isFull, isOrdered, isUnique, isZeroless);
    return ModeFormat(compressedVariant);
  }

  std::vector<AttrQuery> CompressedModeFormat::attrQueries(vector<IndexVar> parentCoords, vector<IndexVar> childCoords) const {
    std::vector<IndexVar> groupBy(parentCoords.begin(), parentCoords.end() - 1);

    std::vector<IndexVar> aggregatedCoords = {parentCoords.back()};
      if (!isUnique) {
        aggregatedCoords.insert(aggregatedCoords.end(), childCoords.begin(), childCoords.end());
    }
  ...
  ModeFunction CompressedModeFormat::posIterBounds(Expr parentPos, Mode mode) const {
    ...
    Expr pend   = Load::make(getPosArray(mode.getModePack()), ir::Add::make(parentPos, 1));
    return ModeFunction(Stmt(), {pbegin, pend});
  }

  ModeFunction CompressedModeFormat::coordBounds(Expr parentPos, Mode mode) const {
    Expr pend     = Load::make(getPosArray(mode.getModePack()), ir::Add::make(parentPos, 1));
    Expr coordend = Load::make(getCoordArray(mode.getModePack()), ir::Sub::make(pend, 1));
    return ModeFunction(Stmt(), {0, coordend});
  }

  ModeFunction CompressedModeFormat::posIterAccess(ir::Expr pos, std::vector<ir::Expr> coords, Mode mode) const {
  ...
  Stmt CompressedModeFormat::getAppendEdges(Expr pPrev, Expr pBegin, Expr pEnd, Mode mode) const {
    Expr       posArray       = getPosArray(mode.getModePack());
    ModeFormat parentModeType = mode.getParentModeType();
    Expr       edges          = (!parentModeType.defined() || parentModeType.hasAppend()) ? pEnd : ir::Sub::make(pEnd, pBegin);
  ...
  Stmt CompressedModeFormat::getAppendInitEdges(Expr pPrevBegin, Expr pPrevEnd, Mode mode) const {
  ...
  Stmt CompressedModeFormat::getAppendInitLevel(Expr szPrev, Expr sz, Mode mode) const {
    const bool szPrevIsZero = isa<ir::Literal>(szPrev) && to<ir::Literal>(szPrev)->equalsScalar(0);
    ...
    initStmts.push_back(Store::make(posArray, 0, 0));
      ...
      if (mode.getParentModeType().defined() && !mode.getParentModeType().hasAppend() && !szPrevIsZero) {
        Expr pVar     = Var::make("p" + mode.getName(), Int());
  ...
  Stmt CompressedModeFormat::getAppendFinalizeLevel(Expr szPrev, Expr sz, Mode mode) const {
    ModeFormat parentModeType = mode.getParentModeType();
      if ((isa<ir::Literal>(szPrev) && to<ir::Literal>(szPrev)->equalsScalar(1)) || !parentModeType.defined() || parentModeType.hasAppend()) {
        return Stmt();
    }
  ...
  Stmt CompressedModeFormat::getSeqInitEdges(Expr prevSize, std::vector<AttrQueryResult> queries, Mode mode) const {
    Expr posArray = getPosArray(mode.getModePack());
    return Block::make({Allocate::make(posArray, ir::Add::make(prevSize, 1)), Store::make(posArray, 0, 0)});
  }

  Stmt CompressedModeFormat::getSeqInsertEdge(Expr parentPos, std::vector<Expr> coords, std::vector<AttrQueryResult> queries, Mode mode) const {
    Expr posArray = getPosArray(mode.getModePack());
  ...
  Stmt CompressedModeFormat::getInitCoords(Expr prevSize, std::vector<AttrQueryResult> queries, Mode mode) const {
    Expr posArray = getPosArray(mode.getModePack());
    Expr crdArray = getCoordArray(mode.getModePack());
  ...
  ModeFunction CompressedModeFormat::getYieldPos(Expr parentPos, std::vector<Expr> coords, Mode mode) const {
  ...
  Stmt CompressedModeFormat::getInsertCoord(Expr parentPos, Expr pos, std::vector<Expr> coords, Mode mode) const {
    taco_iassert(mode.getPackLocation() == 0);
    Expr crdArray = getCoordArray(mode.getModePack());
    Expr stride   = (int) mode.getModePack().getNumModes();
  ...
  Stmt CompressedModeFormat::getFinalizeYieldPos(Expr prevSize, Mode mode) const {
    ...
    Stmt resetLoop = For::make(pVar, 0, prevSize, 1, Store::make(posArr, ir::Sub::make(prevSize, pVar), Load::make(posArr, ir::Sub::make(ir::Sub::make(prevSize, pVar), 1))));
    return Block::make(resetLoop, Store::make(posArr, 0, 0));
  }

  vector<Expr> CompressedModeFormat::getArrays(Expr tensor, int mode, int level) const {
    std::string arraysName = util::toString(tensor) + std::to_string(level);
    return {GetProperty::make(tensor, TensorProperty::Indices, level - 1, 0, arraysName + "_pos"), GetProperty::make(tensor, TensorProperty::Indices, level - 1, 1, arraysName + "_crd")};
  }
  ...
  bool CompressedModeFormat::equals(const ModeFormatImpl &other) const {
    return ModeFormatImpl::equals(other) && (dynamic_cast<const CompressedModeFormat &>(other).allocSize == allocSize);
  }
