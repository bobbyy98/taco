...
namespace taco {
  ...
  ir::Stmt lower(IndexStmt stmt, std::string name, bool assemble, bool compute, bool pack, bool unpack, Lowerer lowerer) {
    string reason;
    taco_iassert(isLowerable(stmt, &reason)) << "Not lowerable, because " << reason << ": " << stmt;
  ...
  bool isLowerable(IndexStmt stmt, std::string *reason) {
    ...
    string r;
      if (!isConcreteNotation(stmt, &r)) {
        *reason = "the index statement is not in concrete index notation, because " + r;
        return false;
    }
