...
namespace taco {
  ...
  struct IterationGraph::Content {
    Content(IterationForest iterationForest, const vector<IndexVar> &freeVars, TensorPath resultTensorPath, vector<TensorPath> tensorPaths, map<IndexExpr, TensorPath> mapAccessNodesToPaths, IndexExpr expr):
  ...
  IterationGraph IterationGraph::make(Assignment assignment) {
    ...
    match(expr, function<void(const AccessNode *)>([&](const AccessNode *op) {
            auto type = op->tensorVar.getType();
            taco_iassert((size_t) type.getShape().getOrder() == op->indexVars.size())
                << "Tensor access " << IndexExpr(op) << " but tensor format only has " << type.getShape().getOrder() << " modes.";
    ...
    IterationForest forest = IterationForest(util::combine({resultPath}, tensorPaths));

    // Create the iteration graph
    IterationGraph iterationGraph = IterationGraph();
    iterationGraph.content        = make_shared<IterationGraph::Content>(forest, freeVars, resultPath, tensorPaths, accessNodesToPaths, expr);
  ...
  const std::vector<IndexVar> &IterationGraph::getChildren(const IndexVar &var) const {
  ...
  const TensorPath &IterationGraph::getTensorPath(const IndexExpr &operand) const {
  ...
  IndexVarType IterationGraph::getIndexVarType(const IndexVar &var) const {
    return (util::contains(content->freeVars, var)) ? IndexVarType::Free : IndexVarType::Sum;
  }
  ...
  void IterationGraph::printAsDot(std::ostream &os) {
    ...
    auto & resultVars = resultPath.getVariables();
      if (resultVars.size() > 0) {
        os << "\n root -> " << resultVars[0] << " [style=dashed label=\"" << resultName << "\"]";
    }
      ...
      for (auto &path: getTensorPaths()) {
        ...
        auto & vars = path.getVariables();
          for (size_t i = 1; i < vars.size(); i++) {
            os << "\n " << vars[i - 1] << " -> " << vars[i] << " [label=\"" << name << "\"]";
          }
      }

      for (size_t i = 1; i < resultVars.size(); i++) {
        os << "\n " << resultVars[i - 1] << " -> " << resultVars[i] << " [style=dashed label=\"" << resultName << "\"]";
      }
    os << "\n}\n";
    os.flush();
  }
